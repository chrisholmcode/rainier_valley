...
- item_name_raw comes ONLY from an actual per-line Description entry. Do NOT copy pre-printed category section labels (e.g., "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Dairy") into item_name_raw. If a category row has no line item written/printed against it, leave `item_name_raw` blank for that row. If you do capture a printed category label as the description, copy it EXACTLY and in full, verbatim as printed ("Canned/Dry Goods", not "Canned") — never truncate at a slash or space. NOTE: a document laid out as pre-printed category labels with blank/handwritten quantities is almost certainly the `grocery_rescue` slip, not this AGENCY ORDER manifest — re-check the "Not this supplier" section before extracting.
