...
- **Rows with a non-empty Pounds cell:**
  ...
  > 🔎 **Pounds-cell verification (read before committing a number):**
  > - **Count the digits.** Read the ENTIRE contiguous digit run left-to-right and re-check the leading digit before writing the value — do not truncate. A cell that reads "145" must not be emitted as "45"; a "51" must not be emitted as "50". If the digit count is uncertain, put every candidate reading in `quantity_raw`, lower `confidence` to ≤ 0.6, and add a `source_warning`.
  > - **Row alignment / no spurious values.** A number counts for THIS row only if it sits inside this row's Pounds cell. If a value appears to belong to an adjacent row, is a stray mark, or you cannot confidently tie it to this specific row, do NOT guess — treat this row as empty (`approx_weight = null`, `quantity = null`, `quantity_raw = null`, notes = "no value confidently aligned to this row"). The "never leave blank" rule applies only when a numeral is legibly written **inside this row's cell**; it does not authorize inventing a value for a row that is actually blank.
