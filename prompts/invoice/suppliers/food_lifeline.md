...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). Read the Gross Weight value digit-by-digit from the far-right column, transcribing EVERY digit exactly — do not drop a leading digit (e.g., read "117", not "17") or add a spurious one, and do not transpose digits. The Gross Weight column can be crowded against the page margin or overlap handwritten storage codes (e.g., "F-1", "C-3"); those codes are NOT part of the weight. Sanity-check: approx_weight should be roughly Quantity × a plausible per-case weight — if your reading is off by a factor of ~10 or is implausibly small/large for the quantity, re-examine the digits before committing.
