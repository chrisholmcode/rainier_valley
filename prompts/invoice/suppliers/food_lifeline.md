...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **Never leave `approx_weight` blank when the slip shows any pounds figure.** If there is no printed "Gross Weight" column but the document has a handwritten **Pounds** value (typically a small integer like 14, 24, 70, 79), map that Pounds number directly to `approx_weight`. NOTE: a handwritten Donor/Address/Agency/Date form with a Pounds column belongs to `grocery_rescue` (reroute per the "Not this supplier" section); apply this fallback only if it has not been rerouted.
