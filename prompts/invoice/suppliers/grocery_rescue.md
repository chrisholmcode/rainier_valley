...
- **Rows with a non-empty Pounds cell:**
  ...
  > **On rescue forms there is NO separate unit count — `quantity` is NOT a count of items/cases; it is just a mechanical copy of the pound value.** As a MANDATORY final step on every populated row, after you set `approx_weight`, set `quantity` to the exact same value. Never leave `quantity` blank because "the Pounds cell only shows a weight, not a count" — the weight IS the quantity here. If `approx_weight` is non-null and `quantity` is null, that row is invalid; fix it before emitting.
