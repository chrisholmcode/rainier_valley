...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Row-cell alignment check (do this per row before writing a weight).** A number counts for a row ONLY if it sits in *that* row's Pounds cell on the same horizontal line as the row label. On densely filled grids it is easy to (a) pull a neighboring row's value into a blank row, or (b) leave the correct value on the wrong row. Before emitting, verify each weight is horizontally aligned with its own row label. If a row's Pounds cell is genuinely empty, keep it `null` even if a nearby cell has a number — **do not borrow a value from an adjacent row to satisfy "never leave blank"** (that rule applies only when a numeral is in THIS row's own cell).
  - **Magnitude / truncation check.** After reading a value, re-scan the cell for a faint or partially-cut leading digit before finalizing (e.g., a written "145" can be misread as "45" if the leading "1" is light or near the cell border). If the leading digit is uncertain, emit your best full reading, lower `confidence` to ≤ 0.6, and add a `source_warning`: `"approx_weight leading digit unclear: read as <value> — verify no leading digit was dropped"`. Grocery-rescue Pounds are commonly two-to-three digits; a suspiciously small value next to larger rows is a cue to re-check for a dropped leading digit.
