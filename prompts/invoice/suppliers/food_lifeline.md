...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - The Ship Date is printed in **US MM/DD/YYYY** order. Read the **month** (first segment) and **day** (second segment) exactly as printed — transcribe them verbatim and do NOT shift, round, or add a month. Example: printed "07/07/2026" => `2026-07-07` (NOT `2026-08-07`). Before finalizing, re-check that the month digits in your output match the first segment of the printed Ship Date character-for-character.
