...
- BEFORE populating approx_weight: confirm the source is the PRINTED manifest column literally labeled **Gross Weight**. If the weight comes from a handwritten **Pounds** column (or any hand-filled Donor/Address/Agency/Date form), STOP — this is `grocery_rescue`, not food_lifeline; do not extract approx_weight here, re-route per the routing note above. Only map the printed **Gross Weight** value to approx_weight; never map a handwritten Pounds figure.
