...
5. **Sequence with descending or non-monotonic numbers and no clear circle** (e.g., "151 123 108 40") => these are typically running adjustments while counting; approx_weight = the LAST number written **that is not itself crossed out or struck through** (40 in this case). Note "running tally; taking last value" in line notes and lower confidence to 0.6.
   - **Crossed-out / abandoned final token:** if the *last* thing written in the cell is itself crossed out, struck, overwritten, or clearly abandoned, do NOT emit it as the weight. Fall back to the last **clean** (non-crossed) number. If EVERY candidate number in the cell is crossed out or struck, the row has **no accepted value** — set `approx_weight = null`, `quantity = null`, `quantity_raw` = the crossed-out digits verbatim, note "all tally values crossed out — no accepted weight", set confidence ≤ 0.6, and add a source_warning so a reviewer can confirm or clear it. Do NOT invent a weight just to avoid a blank.
   - **Leading-digit re-check:** grocery-rescue pounds are frequently 3 digits. After reading the accepted number, re-scan the LEFT edge of the cell for a faint, smudged, or partly-cropped leading digit (e.g. a thin "1" that turns "45" into "145"). If a leading stroke may be present, report both readings in `quantity_raw`, prefer the larger plausible value, lower confidence to ≤ 0.6, and add a source_warning.
