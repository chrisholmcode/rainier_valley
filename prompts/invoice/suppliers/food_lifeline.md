...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). This value comes ONLY from the printed **Gross Weight** column of an AGENCY ORDER manifest. If the weights are in a handwritten per-category **Pounds** column (not a printed Gross Weight column), this is NOT a food_lifeline manifest — STOP, set `supplier = "grocery_rescue"`, and follow that prompt. Do NOT populate approx_weight from a handwritten Pounds column here, and do NOT leave approx_weight blank while still treating the document as food_lifeline.
