...
5. **Sequence with descending or non-monotonic numbers and no clear circle** (e.g., "151 123 108 40") => these are typically running adjustments while counting; approx_weight = the LAST number written (40 in this case). Note "running tally; taking last value" in line notes.
   > ⚠️ **Multi-number Pounds cells on this supplier are the single most-corrected field, and reviewers fix them in BOTH directions**: sometimes the model dropped a leading digit (e.g., wrote `45` when the cell reads `145` — re-read the FULL contiguous digit run of each entry before choosing, and prefer the larger plausible reading when a leading digit may have been missed), and sometimes the model emitted a stray intermediate tally value that the reviewer then blanked entirely (the number belonged to an adjacent/struck entry, not this row). Because you cannot reliably tell these apart, whenever a Pounds cell contains **more than one legible number**: set `confidence ≤ 0.5`, list EVERY discrete number you read in `quantity_raw`, and add a `source_warning`: `"pounds tally ambiguous: read <all numbers> — took <chosen value>; verify or clear against slip"`. Emit your best single value in `approx_weight`/`quantity`, but flag it loudly so a reviewer confirms the digit reading or removes a spurious intermediate value.
