...
- donor_org: capture the donor/source code printed on the manifest verbatim, including any hyphenated suffix (e.g., "SWY-GEN" = Safeway General, "QFC-MI" = QFC Michigan/store code). Look for a Donor or Source field near the header or a per-line donor code. Only set `donor_org = null` if no such code appears anywhere on the document.
