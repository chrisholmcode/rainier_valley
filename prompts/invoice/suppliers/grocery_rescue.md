...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **`quantity_raw` is a DIGITS-ONLY verbatim transcription.** Emit only the numerals and the spaces/line-breaks separating distinct numbers. Do NOT add annotation words or symbols such as "(circled)", "(boxed)", "crossed out", "/", or "or" — a circled 51 with a competing 66 is `quantity_raw = "51 66"`, NOT `"51 (circled) / 66"`. All commentary about which value was circled/final/crossed belongs in `notes`, never in `quantity_raw`.
    - **Transcribe every digit of each number exactly — no dropping and no splitting.** Do not drop a leading digit (a legible "145" is `"145"`, never `"45"`), and do not read one contiguous number as two (or two as one). If a leading digit is faint or you are unsure of a digit count, transcribe your best reading of ALL digits, lower confidence to ≤ 0.6, and add a `source_warning` — do not silently emit a shorter number.
