...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **quantity_raw is a LITERAL transcription of the ink — digits and separators (spaces/line-breaks) ONLY.** Do **NOT** embed interpretive annotations such as `(circled)`, `(boxed)`, `/`, "or", or any words. Those observations belong in `notes`, never in `quantity_raw`. Example: a cell where "51" is circled → `quantity_raw = "51"` (NOT `"51 (circled)"`), and put "51 circled = final count" in `notes`.
    - **Re-read multi-digit numbers digit-by-digit before writing them here.** Do not drop a leading digit or split one number into two — e.g., a cell reading "145" is `quantity_raw = "145"`, not `"45"`. If a leading digit is faint but plausibly present, include it, lower confidence to ≤ 0.6, and add a source_warning noting the uncertain leading digit.
