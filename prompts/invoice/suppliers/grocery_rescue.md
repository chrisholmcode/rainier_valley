...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in. The `<delivery_date>` segment MUST be the exact resolved delivery_date string, character-for-character — do not reformat, re-pad, or re-guess the month here. If the delivery_date month digit was flagged as unclear (per the single-digit month legibility warning above), do NOT silently emit a confident month in this ID: carry the SAME uncertainty forward — set slip confidence ≤ 0.7 and add a `source_warning`: `"invoice_or_order_number month digit unverified — mirrors ambiguous delivery_date; please verify against slip"`. Never let this synthesized ID assert a more precise/confident date than the delivery_date field it is built from.
