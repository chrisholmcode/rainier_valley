...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - **supplier is ALWAYS `"grocery_rescue"` for this form — even though the slip shows a prominent "FOOD LIFELINE" logo upper-left.** That logo is the broker's branding, NOT the supplier value. NEVER emit `supplier = "food_lifeline"` here; `food_lifeline` is a different bucket reserved for printed Food Lifeline **Agency Order** manifests.
