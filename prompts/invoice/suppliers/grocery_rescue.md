...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is ambiguous (could be "7" or "8", could be "5" vs another digit, ink smudged, or only partially legible), do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`. **Do not treat a single-digit month as "clean/high-confidence" just because you settled on one digit; single-digit months are the most-corrected reading on this supplier.**
  - **Month must be a valid, zero-padded two-digit value 01–12.** Always pad a single-digit month (May → `05`, never `5` and never `0`). Before emitting, self-check that the month component parses to an integer 1–12; if your reading yields `0`, `00`, or a bare single digit, you have misread the stroke — re-examine and add the month-unclear `source_warning` above.
