...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **Pounds fallback for quantity:** If a line has NO dedicated Quantity column and the only numeric amount is a "Pounds" (or Gross Weight) figure, set `quantity` to that Pounds value (e.g., a line reading "16 lbs" => quantity = 16) and also record it in `approx_weight`. Set `unit = "lb"`. NEVER leave `quantity` blank when a Pounds/weight number is present — the Pounds figure is the quantity of last resort.
