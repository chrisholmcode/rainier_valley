...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Month legibility warning:** if the handwritten month is ambiguous in ANY way — smudged, overwritten, partially legible, or a leading/second digit that could plausibly be a different value (e.g., "7" vs "8", but also any digit in the 0–9 range where the stroke is unclear) — do NOT commit a confident month. Emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`. When you cannot confidently distinguish the month digit, prefer flagging over guessing: it is better to route the slip to review than to repeatedly emit a wrong month (this field is the single most-corrected date component on this supplier).
