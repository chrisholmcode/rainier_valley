...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If there is no "ACR-" style Agency Order No on the slip, use the store/manifest order identifier printed elsewhere on the document instead — e.g., a QFC store number like "QFC-MI-2026-07-10". Never leave this field blank when any order/manifest number is visible; capture it verbatim.
