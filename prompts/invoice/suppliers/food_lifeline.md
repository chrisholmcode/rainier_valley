...
- donor_org => capture the donor code printed/stamped on the manifest verbatim (e.g., "SWY-GEN", "QFC-MI"). These are retail-chain donor codes in `CHAIN-DIVISION` form (SWY = Safeway, QFC = QFC, etc.). Look for a "Donor" / "Donor Code" / "Source" label near the header or a stamped code on the slip. Only set `donor_org = null` if no such code is present anywhere on the document.
