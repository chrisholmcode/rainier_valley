...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **Use the EXACT resolved `delivery_date` string — do not re-read the date from the slip a second time.** The ID's date component must be byte-for-byte identical to the final `delivery_date` you emitted, so the two fields can never disagree.
  - **If `delivery_date` carries a single-digit / month-digit legibility warning (or its confidence is ≤ 0.7 per the single-digit month rule), the month in the resolved date is NOT trusted — do NOT silently commit a guessed month into this unique key.** In that case, set the ID's month component to `0` as a placeholder (e.g. `QFC-MI-2026-0-28`), keep confidence ≤ 0.7, and add a `source_warning`: `"invoice_or_order_number month placeholder — delivery_date month unverified, reviewer must confirm"`. Do not emit a specific but unverified month (e.g. `QFC-MI-2026-08-28`) — a wrong month silently corrupts the unique key.
