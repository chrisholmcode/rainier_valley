...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). Pounds column => quantity_raw (the numeric pounds for the line, verbatim, e.g. "24", "170"). If a document has a Pounds column but you have NOT re-routed it to `grocery_rescue`, still populate `quantity_raw` from that Pounds value — never leave `quantity_raw` blank when a Pounds figure is present.
