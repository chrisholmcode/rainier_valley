...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. Output format is strictly `YYYY-MM-DD` with month and day ALWAYS two digits, zero-padded (e.g. `2026-05-28`, `2026-07-01`). Month MUST be `01`–`12` and day `01`–`31` — never emit a single-digit, empty, or `0` month/day component (e.g. `2026-0-28` and `2026-5-28` are invalid). If you cannot read a valid month or day, set delivery_date=null and add a source_warning rather than emitting a malformed date. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
