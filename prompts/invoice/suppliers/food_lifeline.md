...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "Agency Order No" / "ACR-" value is present, capture whatever order or invoice number IS printed on the slip verbatim, even if it uses a different prefix or format (e.g., a store/donor-sourced number like "QFC-MI-2026-07-10"). Do NOT leave this field blank when any order/invoice number is visible.
