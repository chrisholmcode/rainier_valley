...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **`quantity_raw` is a LITERAL transcription — digits and the spaces/line-breaks between them ONLY.** Do NOT put annotation words or symbols in this field: no `(circled)`, `(boxed)`, `(crossed out)`, no `/` connector, no commentary. A cell with a circled "51" → `quantity_raw = "51"` (put "circled as final" in `notes`), NOT `"51 (circled)"` or `"51 (circled) / 66"`; if two distinct numbers are legible, separate them with a single space (e.g. `"51 66"`). Before emitting, re-verify every digit against the slip — pay special attention to leading digits (read "145", not "45") and easily-confused pairs (50 vs 51).
