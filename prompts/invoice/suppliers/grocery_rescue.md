...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **Do NOT re-read or re-parse the date when building this ID.** Concatenate the *already-resolved* `delivery_date` string verbatim — the month/day segment in invoice_or_order_number MUST be byte-for-byte identical to the `delivery_date` value you emitted. Never independently zero-pad, "correct," or re-derive the month here (e.g., do not turn a resolved `2026-5-28` into `2026-05-28`, and never emit a month that differs from delivery_date).
  - **If the month digit was ambiguous / smudged / single-digit** (i.e., you triggered the "single-digit month legibility warning" above), carry the SAME lowered confidence (≤ 0.7) onto this field and add a source_warning `"invoice_or_order_number month digit unclear — verify against slip; must match delivery_date"`. A recurring reviewer correction on this supplier has been the month segment of this ID (e.g., model wrote `SWY-GEN-2026-05-28` but the slip's month was a single ambiguous digit); do not silently assume "05".
