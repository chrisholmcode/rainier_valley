...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Digit-count check (prevents dropped leading digits):** after reading the final value, re-scan the cell left-to-right and confirm you captured EVERY leading digit — a value that looks like "45" is often "145", and "51" can be "151". If the digit count you emit is fewer than the digits visibly present, you have dropped a digit; re-read. When the final read is at all uncertain (smudged, overwritten, ambiguous digit count, or you cannot decide whether the cell resolves to a value at all), emit your best single reading, set confidence ≤ 0.6, and add a source_warning `"approx_weight uncertain: read as <your reading> — verify digit count and final tally value against slip"`. Do NOT silently blank a cell that contains legible writing, and do NOT emit a confident value for a mark you are not sure is a weight.
