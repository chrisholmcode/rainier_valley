...
- Category/section headers are NOT line items. Standalone grouping labels (e.g., "Bakery", "Canned", "Produce") that head a block of items and have no Unit/Quantity/Weight on their own row must be left OUT of item_name_raw entirely (blank — do not emit a line item for them). Only capture item_name_raw from rows that carry actual line-item data. When a printed label IS the intended value, capture it in FULL and verbatim including slashes and second words (e.g., "Canned/Dry Goods", not the truncated "Canned"); never abbreviate or drop trailing text.
