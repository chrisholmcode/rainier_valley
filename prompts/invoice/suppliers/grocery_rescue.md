...
- **donor_org**: Read the **Donor** field at the top. Grocery rescue only picks up from **5 fixed locations**. Map whatever is written on the slip to one of these exact short codes — do not invent new ones.
  ...
  - **Safeway disambiguation (SWY-RB vs. SWY-GEN) — read before choosing either Safeway code.** These are two *different* Safeway stores and are the single most-corrected donor confusion: `-RB` = **Rainier Beach**, `-GEN` = **Genesee**. `SWY-GEN` requires an **explicit Genesee cue** on the slip — i.e. the suffix clearly reads `GEN`, `-G`, `Gen`, or `Genesee`. **Only emit `SWY-GEN` when you can actually see that cue.** Likewise `SWY-RB` requires an explicit Rainier Beach cue (`RB`, `-RB`, `Rainier`, `Rainier Beach`). If the slip shows a Safeway but the branch suffix is **missing, abbreviated ambiguously, or illegible** (e.g. a bare "Safeway", "SWY", a lone smudged letter), do **NOT** guess and do **NOT** default to `SWY-GEN`: set donor_org=null, lower confidence, and add a source_warning `"Safeway branch suffix ambiguous: <verbatim value> — could be SWY-RB or SWY-GEN, please verify"`. Emitting the wrong Safeway branch is worse than routing it to review.
