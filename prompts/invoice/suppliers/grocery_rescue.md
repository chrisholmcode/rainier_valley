...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` is ALWAYS `"grocery_rescue"` for this form — NEVER `"food_lifeline"`.** The slip's upper-left logo reads "FOOD LIFELINE" because Food Lifeline is the broker printed on the form, but that logo is NOT the supplier value. Do not copy the logo text into the `supplier` field. The `food_lifeline` supplier is reserved exclusively for printed Food Lifeline Agency Order manifests, which this hand-filled rescue form is not.
