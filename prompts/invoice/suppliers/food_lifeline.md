...
- Description => item_name_raw verbatim. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
  - CATEGORY/SECTION BANNERS: Some manifests group items under category headers (e.g., "Bakery", "Produce", "Canned/Dry Goods", "Frozen"). These banner rows are NOT line items — they usually have no Item No., Unit, Quantity, or Gross Weight. On a banner row itself, leave `item_name_raw` **blank** (do not copy the banner into it). Only capture actual product Descriptions into `item_name_raw`. When you DO transcribe a banner elsewhere (e.g., as a category label), copy it EXACTLY as printed — preserve slashes and the full phrase (write "Canned/Dry Goods", not "Canned"; do not truncate at the slash).
