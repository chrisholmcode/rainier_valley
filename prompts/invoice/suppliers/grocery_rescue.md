...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit legibility warning:** the *day* digit is misread as often as the month — adjacent digits like 5/6, 1/7, 3/8, 0/6/8 are easily confused in handwriting. If the day digit is at all ambiguous (smudged, could be an adjacent value), emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be off by one — please verify against slip"`. Because `invoice_or_order_number` is synthesized directly from `delivery_date`, a one-day misread silently corrupts the shipment key — do not guess the day when it is unclear.
