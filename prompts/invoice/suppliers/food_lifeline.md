...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - The Ship Date is printed in **MM/DD/YYYY** order. Read the FIRST number as the month and the SECOND as the day. Transcribe exactly what is printed — do NOT advance, round, or infer the month. Example: a Ship Date printed "07/07/2026" => `2026-07-07` (NOT `2026-08-07`). Double-check the month digit against the printed value before emitting.
