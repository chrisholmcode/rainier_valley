...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Do not over-commit an illegible month.** If, after your best effort, the month digit is truly unreadable (smudged, partially formed, could plausibly be more than one value), you MUST set `confidence ≤ 0.5` and add the `source_warning` above — never emit a confident, clean single-month reading (like "05") for a digit you cannot actually distinguish. A reviewer is expected to hand-correct this slip; a fabricated-precise month that "looks right" is worse than a flagged uncertain one, because it will be silently accepted. When the month is flagged unclear, still populate delivery_date with your best-guess YYYY-MM-DD so downstream keys resolve, but keep confidence low and the warning present.
