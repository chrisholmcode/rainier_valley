...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Per-cell read discipline (two most common errors on this supplier):**
    1. **Do not truncate leading digits.** Count the digits in the cell before writing the number. Grocery-rescue weights are frequently 3 digits (100–199 lb). If you read a 2-digit value (e.g., "45", "56") on a row whose handwriting spans a wide cell or shows a faint leading stroke, re-examine for a dropped leading "1" and prefer the 3-digit reading; note the ambiguity in `quantity_raw` and lower confidence.
    2. **Never carry a value across rows.** Each row's `approx_weight` must come ONLY from that row's own Pounds cell. If a cell is genuinely empty, emit `approx_weight = null` (skeleton-row rule) even if an adjacent row has a value — do NOT copy or repeat a neighboring row's number. A value must be visibly present in that row's cell to be extracted.
