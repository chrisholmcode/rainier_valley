...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **quantity_raw is a LITERAL DIGIT TRANSCRIPTION — never annotate it.** Do NOT add interpretive words or symbols such as `(circled)`, `(boxed)`, `(crossed out)`, or an "X / Y" split. Emit only the digits and the whitespace/line-breaks between them (e.g., a value circled as final that also shows an earlier "66" → `quantity_raw = "50 66"`, NOT `"50 (circled) / 66"`). All circle/box/tally reasoning belongs in `approx_weight` and `notes`, never in `quantity_raw`.
    - **Re-read leading digits before finalizing.** Handwritten hundreds are easy to clip — a "145" can look like "45", a "156" like "56". After transcribing, re-check whether a leading digit precedes the number you read; if a faint or partial leading digit is plausible, include it, lower `confidence` to ≤ 0.7, and add a `source_warning` noting the ambiguity.
