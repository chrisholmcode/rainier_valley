...
- IMPORTANT: A printed AGENCY ORDER manifest has a **Quantity** column (case counts) AND a separate **Gross Weight** column — it does NOT have a column simply labeled **Pounds**. If the document you are reading has a column headed "Pounds" (handwritten per-category weights such as 14, 16, 21, 170), this is NOT a printed manifest: it is the handwritten grocery-rescue slip. Re-route: set `supplier = "grocery_rescue"` and follow that supplier's prompt, where the Pounds value maps to `quantity`. Never leave `quantity` blank because a value appeared under a "Pounds" header.
