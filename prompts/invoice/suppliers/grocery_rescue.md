...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit legibility warning:** the handwritten *day* is misread as often as the month on these slips (e.g., a "6" read as a "5", or an "8" as a "3"). If the day digit is at all ambiguous (looped/closed tops, ink smudge, adjacent digits touching), do NOT silently pick the lower or rounder value — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be ±1 — please verify against slip"`. Because `invoice_or_order_number` embeds this date, an unclear day means the synthesized key is also unverified and should be flagged for review.
