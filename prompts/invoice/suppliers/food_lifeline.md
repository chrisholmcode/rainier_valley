...
- HANDWRITTEN POUNDS SLIPS (Pounds column, no printed Quantity column): if the document has a handwritten/printed **Pounds** column and NO separate printed Quantity column, read each line's Pounds value as `quantity` and set `unit = "lb"`. Read every line's Pounds figure — do not leave `quantity` blank when a Pounds value is present. Transcribe the digits exactly as written (e.g., 79 not 70); if a handwritten digit is ambiguous, capture the most likely value and flag it in line `notes`.
