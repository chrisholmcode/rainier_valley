...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell. This is a LITERAL digit transcription, not an interpretation.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values, separated by single spaces (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **Do NOT add annotations or words.** Never write "(circled)", "(boxed)", "/", "crossed out", or any commentary inside `quantity_raw` — those belong in `notes` only. E.g. a cell with a circled "51" beside a struck "66" → `quantity_raw = "51 66"` (NOT `"51 (circled) / 66"`); note the circling in `notes`.
    - **Transcribe the full number — do not drop leading or trailing digits.** Re-read multi-digit values carefully before emitting (e.g., "145", not "45"). If a digit is genuinely unreadable, emit your best full reading, lower confidence to ≤ 0.6, and add a source_warning; do not silently truncate.
    - **Do NOT invent a value.** If, on re-examination, the cell has no legible ink (faint mark, stray pen dot, smudge with no readable digit), treat it as an empty cell: `quantity_raw = null` and follow the empty-cell rules — never fabricate a number to fill it.
