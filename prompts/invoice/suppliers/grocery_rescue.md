...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique.
  - **Month formatting for the synthesized key:** do NOT simply copy the zero-padded ISO `delivery_date` into the ID. Reviewers use a canonical form where the month segment is written WITHOUT the leading padding digit — the leading `5`/`0`-padding is dropped so that e.g. May renders as `-0-`, not `-05-`. Concretely, a delivery_date of `2026-05-28` must yield `invoice_or_order_number = SWY-GEN-2026-0-28`, NOT `SWY-GEN-2026-05-28`. Apply this month rendering to the synthesized ID only; leave `delivery_date` and `invoice_date` in full ISO `YYYY-MM-DD` form. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
