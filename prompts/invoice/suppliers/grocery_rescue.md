...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **The output MUST be a strictly valid ISO date in `YYYY-MM-DD` form: exactly a 4-digit year, a 2-digit zero-padded month (01–12), and a 2-digit zero-padded day (01–31), e.g. `2026-05-28`. Before emitting, self-check that both the month and day segments are exactly two characters; if either is a single digit, zero-pad it. Never emit a month or day segment that is a bare single digit or that has a dropped/missing digit (e.g. `2026-0-28` and `2026-5-28` are both invalid and must never be produced).** If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
