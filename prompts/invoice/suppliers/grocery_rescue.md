...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **The output MUST be a strictly valid, fully zero-padded ISO date: exactly `YYYY-MM-DD` with a two-digit month `01`–`12` and a two-digit day `01`–`31` (e.g., `2026-05-28`, never `2026-5-28`, never `2026-0-28`, never a month of `0`/`00`). If you cannot resolve a real month (1–12) and day, do NOT emit a partial/zeroed date — set both dates to null, lower confidence, and add a source_warning `"invoice_date unresolved: <verbatim value>"`.** If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
