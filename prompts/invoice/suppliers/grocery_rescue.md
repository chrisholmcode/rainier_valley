...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** This is a PURE, MECHANICAL transcription of the Pounds cell — NOT an interpretation. Copy every visible digit/number exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **Do NOT put interpretive annotations in `quantity_raw`.** Never write markers like `(circled)`, `(boxed)`, `(crossed out)`, or which value is "final" here — put those observations in `notes`. `quantity_raw` contains digits and spaces only (e.g., transcribe a circled 51 above a 66 as `quantity_raw = "51 66"`, and record "51 circled as final" in `notes`).
    - **Re-read every number edge-to-edge before writing it, so you do not drop a leading digit.** A faint or narrow leading digit is easy to miss — e.g. "145" misread as "45", or "156" misread as "56". If a cell looks like a 2-digit number but the row's neighbors are 3-digit, or the value seems implausibly small, re-examine for a dropped leading digit before committing. If still unsure, report both readings in `quantity_raw`, lower confidence to 0.6, and add a `source_warning`.
