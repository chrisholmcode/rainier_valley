...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to strict `YYYY-MM-DD` where **both the month and the day are always two digits, zero-padded** (e.g., May 28 → `2026-05-28`, never `2026-5-28` or `2026-0-28`). The month component MUST be a valid `01`–`12` and the day `01`–`31`; a bare single-digit or `0` month is invalid and must never be emitted. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
