...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - Read the Gross Weight value digit-by-digit and transcribe EVERY digit exactly — do not drop a leading/trailing digit or infer a "rounder" number. Common misreads on these manifests: reading "73" as "7", "117" as "17", "17" as "117". If unsure of a digit, prefer the value as printed rather than a plausible-looking alternative.
  - Align each Gross Weight to its OWN row: read across horizontally from the Description/Item No. on the same line. Do not borrow a weight from an adjacent line item. When multiple line items are present, verify each row's weight independently before moving to the next.
