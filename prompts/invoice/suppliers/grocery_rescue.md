...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit off-by-one warning (known recurring error):** the handwritten DAY digit on these slips is frequently misread by one (e.g., a "6" read as "5", or vice versa). Because `invoice_or_order_number` is synthesized directly from this date, a one-day slip silently corrupts the shipment ID. Re-examine the day digit carefully against the source before finalizing. If the day digit is at all ambiguous (adjacent digits like 5/6, 0/6, 8/6, or a smudged/curved stroke), do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be off by one — please verify against slip"`.
