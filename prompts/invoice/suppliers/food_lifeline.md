...
- donor_org: Capture the printed donor/retailer source code verbatim (e.g., "SWY-GEN" for Safeway-General, "QFC-MI" for QFC-Michigan). These short hyphenated codes identify the retail chain donating the product and usually appear near the item source or header. Set `donor_org = null` ONLY when no such code is present on the manifest.
