...
- donor_org: read the printed **donor / source code** on the manifest and capture it verbatim (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC). These are retail-donor source codes and are frequently present on these slips. Only set `donor_org = null` when no donor/source code is printed anywhere on the document. Do NOT leave this blank by default.
