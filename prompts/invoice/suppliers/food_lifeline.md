...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If there is NO "Agency Order No" field but the slip shows another printed order/reference/pickup code — e.g. a store-source identifier like "QFC-MI-2026-07-10" (store code + location + date) — capture that value verbatim into `invoice_or_order_number` instead. Never leave this field blank when any such order/reference number is visible on the slip.
