...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - CAUTION: transcribe the **month exactly as printed** — do NOT shift it forward. A recurring error on this supplier is reading the month one higher than printed (e.g., outputting `2026-08-07` when the Ship Date reads July → `2026-07-07`). Read the month digits/name literally from the Ship Date field; never infer, round up, or add a month. If the Ship Date is printed in numeric `MM/DD/YYYY` or `DD/MM/YYYY` form, resolve the ambiguity from the printed month and keep that exact month value in the output.
