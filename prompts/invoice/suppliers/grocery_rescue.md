...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **The output MUST be a fully valid, strictly zero-padded date: exactly four year digits, a two-digit month in the range 01–12, and a two-digit day (e.g. `2026-05-28`). Month `00`/`0` and any missing or single-digit month component are invalid and must never be emitted — if you cannot produce a valid two-digit month in 01–12, set the date to null, lower confidence, and add a source_warning rather than emitting a malformed value.** If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
