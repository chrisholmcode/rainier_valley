...
- Description => item_name_raw verbatim. Capture the ENTIRE Description cell exactly as printed, including multi-word category strings joined by a slash or spanning two lines (e.g., "Canned/Dry Goods" — do NOT truncate to "Canned", and do NOT drop the "/Dry Goods" portion).
  - Category/section HEADER rows: some manifests print grouping labels (e.g., "Bakery", "Canned/Dry Goods", "Produce") on their own row with NO Item No., Unit, Quantity, or Gross Weight. These are headers, not line items — do NOT emit a line item for them and leave `item_name_raw` blank / omit the row. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
