...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Transcribe the printed month digit-for-digit; do NOT infer or shift it. A recurring error on this supplier is writing the month one higher than printed (e.g., reading a July Ship Date as `-08-`). Read the month field carefully and, if the manifest shows a nearby date (e.g., an order/print date), confirm the Ship Date month is consistent rather than a month later.
  - Example: printed Ship Date "07/07/2026" => invoice_date = "2026-07-07" (NOT "2026-08-07").
