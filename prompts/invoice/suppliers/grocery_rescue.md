...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to strict `YYYY-MM-DD`: the month MUST be a two-digit value in the range `01`–`12` (never `0`, `00`, or a single digit) and the day MUST be two digits `01`–`31`. Emit only a syntactically valid ISO calendar date — e.g. write `2026-05-28`, never `2026-5-28` or `2026-0-28`. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
