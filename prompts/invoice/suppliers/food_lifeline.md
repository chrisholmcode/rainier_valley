...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). approx_weight comes ONLY from the printed **Gross Weight** column of the AGENCY ORDER manifest. If the document has no printed Gross Weight column but instead a hand-filled per-category **Pounds** column (small tallies like 14, 16, 21, 24 written by hand), this is NOT this supplier — stop and re-route to `supplier = "grocery_rescue"`; do not populate approx_weight from a handwritten Pounds value here.
