...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Transcribe the **month** digit-for-digit exactly as printed in the Ship Date field. Do NOT infer, pad, or "correct" the month — if the printed month reads as a single glyph or an ambiguous stroke, transcribe only the digit(s) you can actually see rather than assuming a two-digit month like "08". Re-check the month against the raw printed characters before finalizing; the recurring error on this supplier is over-reading the month (e.g., writing "08" when the slip shows "07", or adding a spurious leading digit).
