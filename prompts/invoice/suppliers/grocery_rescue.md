...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** This field is a **pure verbatim transcription of the digits actually inked in the Pounds cell** — it is NOT the interpreted final count and NOT an annotation field. Copy every visible digit/number exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **Do NOT add interpretive words or markup.** Never write "(circled)", "(boxed)", "(crossed out)", or a "/" separating alternate guesses inside `quantity_raw`. Put circle/box/crossout reasoning and your final-value choice in `notes`/`approx_weight` — keep `quantity_raw` to the raw numerals and the spaces/line-breaks between them only. (E.g., a cell where "51" is circled → `quantity_raw = "51"`, NOT `"51 (circled)"`; a cell showing "50" circled with "66" nearby → `quantity_raw = "50 66"`, NOT `"50 (circled) / 66"`.)
    - **Transcribe the FULL number — do not drop a leading digit.** Apply the digit-boundary caution from the Running-tally rule here: a contiguous run of digits is ONE number. A cell reading "145" must be transcribed "145", never "45"; re-check the leftmost digit before emitting.
    - **Only genuinely empty cells get blank.** If any numeral is legible, `quantity_raw` must be non-null. If the cell has no ink at all, `quantity_raw = null` (do not invent a reading such as "51" for an empty row).
