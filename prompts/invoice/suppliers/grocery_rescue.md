...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Anchor each value to its OWN row.** A Pounds value belongs to a row only if the digits sit inside that row's Pounds cell. Do NOT pull a number up or down from an adjacent row's cell, and do NOT read a stray tick/gridline/bleed-through as a value. If the cell for THIS row contains no digit clearly inside its own boundary, treat it as empty (approx_weight = null) — do not fabricate a weight. Over-emitting a value on a truly-blank row is a common correction; a confidently-blank row is better than a guessed one.
  - **Verify the leading digit on multi-digit reads.** Before committing a 2- or 3-digit weight, re-scan the left edge of the number for a leading "1" (or other digit) you may have clipped — dropping the hundreds/tens digit (e.g., reading "45" when the cell says "145") is a recurring error. If the leading digit is faint or cut off, report both readings in quantity_raw, prefer the fuller value, and lower confidence to ≤ 0.6.
