...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity = **REQUIRED — mechanical copy of `approx_weight`, done as the LAST step on every row.** `quantity` is NOT a package/item count and is never independent of `approx_weight`; it is a literal duplicate. Set `quantity = approx_weight` byte-for-byte whenever `approx_weight` is non-null — including when the value is `0`, `1`, or any small number. A legible `0` in the Pounds cell is a REAL value (approx_weight = 0, quantity = 0, quantity_raw = "0"), NOT an empty cell — only a cell with no writing at all is null. The Pounds cell is both the weight and the billed quantity. Example: Pounds cell reads "17" → `approx_weight = 17`, `quantity = 17`. **Never emit a row where `approx_weight` is non-null but `quantity` is blank/null.**
