...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **Strict format:** the output MUST match `^\d{4}-\d{2}-\d{2}$` — a 4-digit year, a 2-digit zero-padded month (01–12), and a 2-digit zero-padded day (01–31). Never emit a single-digit or empty month/day component (`2026-0-28` and `2026-5-28` are INVALID; the correct form is `2026-05-28`). If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
