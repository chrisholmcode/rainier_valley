...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "Agency Order No" / "ACR-" value is present, fall back to any other printed invoice or order number on the slip — including store-style codes such as "QFC-MI-2026-07-10" (format `<STORE>-<REGION>-<YYYY-MM-DD>`). Never leave this field blank when any such number appears anywhere on the document.
