...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). **In this key, the date portion must use the month digit EXACTLY as written on the slip — do NOT zero-pad a single-digit month, and do NOT reuse the zero-padded `delivery_date` string here.** The `delivery_date` field itself stays fully padded YYYY-MM-DD, but the shipment key uses the verbatim month digit. Worked example: a slip with donor `SWY-GEN` and a handwritten Date whose month digit is a bare single digit and day "28" → `delivery_date = 2026-0?-28` (padded), but `invoice_or_order_number = SWY-GEN-2026-0-28` (verbatim single-digit month, NOT `SWY-GEN-2026-05-28`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
