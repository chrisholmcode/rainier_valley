...
- Category/section header rows (e.g., "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Meat") are group labels, NOT line items. They typically span the row with no Item No., Unit, Quantity, or Gross Weight. Do NOT emit a line item for a header row — leave `item_name_raw` blank / omit the row. If a row genuinely is an item and its Description IS a category-style phrase, capture the FULL label verbatim including any second half after a slash (e.g., "Canned/Dry Goods", not "Canned").
