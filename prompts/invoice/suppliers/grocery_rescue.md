...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. ⚠️ The date component of this ID is copied verbatim from the resolved `delivery_date`, so any month-reading error propagates straight into the shipment key. Before emitting, re-read the handwritten month digit. On **SWY-GEN** slips the month digit is frequently misread as "5"/"05" when it is actually a different (often single-digit / "0"-leading) value — do NOT default to "05". If the month digit is at all ambiguous, apply the single-digit month legibility warning (confidence ≤ 0.7 + source_warning) AND set invoice_or_order_number to your best reading rather than confidently emitting a padded "05". If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
