...
- **Rows with a non-empty Pounds cell:**
  ...
  >
  > **Reading discipline (two most common miss-reads):**
  > 1. **Count the digits before committing.** Handwritten grocery-rescue weights are frequently 2–3 digits. A leading "1" is easy to drop (reading "145" as "45") and adjacent digits are easy to confuse (reading "51" as "50"). After you read a value, re-look at the cell and confirm the digit count and each digit. If any digit is uncertain, emit your best full-length reading, set `confidence` ≤ 0.6, and add a `source_warning`: `"pounds value hard to read: read as <value> — please verify digit count against slip"`. Never truncate to a shorter number when in doubt.
  > 2. **"Never leave blank" applies ONLY to cells that actually contain writing.** Do NOT project or invent a number onto a row whose Pounds cell has no writing — a truly empty cell must be extracted as blank per the blank-cell rule below (`approx_weight = quantity = quantity_raw = null`). If you cannot point to a specific ink mark in *this* cell, it is blank. Do not carry a value over from another row.
