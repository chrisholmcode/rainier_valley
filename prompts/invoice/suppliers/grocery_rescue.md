...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **The output MUST be a valid calendar date with a four-digit year and ZERO-PADDED two-digit month (01–12) and day (01–31) — e.g. `2026-05-28`. Never emit a single-digit or "0" month/day (e.g. `2026-0-28` is invalid) and never emit a month outside 01–12.** If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
