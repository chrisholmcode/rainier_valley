...
- donor_org: Capture the printed **donor/source code** verbatim — a short uppercase code of the form `<STORE>-<REGION/DEPT>` (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC Michigan). This code identifies the donating retailer and typically appears near the item/source columns or in a Donor field. Preserve the hyphen and casing exactly. Only set `donor_org = null` if no such donor code appears anywhere on the manifest.
