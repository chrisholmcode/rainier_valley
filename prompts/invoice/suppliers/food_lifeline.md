...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **Fallback when there is NO printed "Quantity" column but there IS a "Pounds" column** (this happens when a Pounds-column slip is being extracted here): use the numeric Pounds value for that line as `quantity` (e.g., a Pounds value of "16" => quantity = 16), set unit = "lb", and set approx_weight to the same Pounds value. Never leave `quantity` blank when a Pounds value is legible. (If this is actually the handwritten Donor/Address/Agency/Date form, prefer routing to `grocery_rescue` per "Not this supplier" below.)
