...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. Always output a fully well-formed, zero-padded date: the month MUST be two digits 01–12 and the day MUST be two digits 01–31 (e.g., May 28 → `2026-05-28`, never `2026-5-28` and never `2026-0-28`). Before emitting, validate that the string matches `\d{4}-\d{2}-\d{2}` with a real calendar month/day; if it does not, re-read the slip rather than emitting a malformed value. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
