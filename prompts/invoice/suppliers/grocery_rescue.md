...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Month cross-check (mandatory):** whenever the month you read does NOT match the month implied by `Today's date`, do NOT emit it with high confidence. Re-examine the month digit; if you still read a conflicting month, lower confidence to ≤ 0.6 and add a `source_warning`: `"delivery_date month (<read month>) disagrees with upload/Today's date month (<today month>) — please verify against slip"`. These forms are filled the day of pickup, so a month mismatch almost always means a misread digit. Never silently emit a delivery_date whose month conflicts with the upload date.
