...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is ambiguous OR anything less than crisply, unambiguously legible (could be "7" or "8", could be "5" vs "0"/"6"/"8", ink smudged, partially formed, or you are filling in a plausible value rather than reading a clear digit), do NOT silently commit a clean month. Emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`. **A syntactically perfect `YYYY-MM-DD` output is NOT evidence you read the month correctly — when in doubt, always attach this warning and lower confidence rather than emitting a confident date.**
