...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). If no printed **Gross Weight** column is present but a handwritten/printed **Pounds** value appears for the line (typically a small 1–3 digit integer, e.g., 14, 24, 70), populate `approx_weight` with that value. Never leave `approx_weight` blank when any per-line pounds figure is legible on the slip — if this document is the handwritten Donor/Address/Agency/Date form, re-check the routing to `grocery_rescue` first, but still capture the visible pounds into `approx_weight`.
