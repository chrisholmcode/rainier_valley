...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no `ACR-`prefixed Agency Order No is present, capture whatever printed order/reference identifier appears instead — including a store/donor-code + date reference such as "QFC-MI-2026-07-10" (store prefix like QFC/etc., optional site code, then a YYYY-MM-DD date). Copy it verbatim. Do NOT leave this field blank when any such order/reference number is visible on the slip.
