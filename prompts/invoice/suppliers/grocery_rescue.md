...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **Output format is strict: the month and day MUST each be exactly two digits, zero-padded, and the month MUST be a valid 01–12 value and the day a valid 01–31 value (e.g., May → `05`, the 8th → `08`).** Never emit a single-digit or truncated month/day (e.g., `2026-0-28`, `2026-5-28`, or `2026-00-28` are all INVALID). Before emitting, re-read the resolved value and confirm it is a well-formed ISO-8601 date; if the month cannot be read as a valid 01–12 value, set both date fields to null, lower confidence to ≤ 0.7, and add a source_warning `"invoice_date month unreadable: <verbatim value> — please verify against slip"` rather than emitting a malformed date. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
