...
- Description => item_name_raw verbatim. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
  - Do NOT treat category/section band labels as item names. Rows that are bare category headings grouping the lines beneath them — e.g. "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Frozen", "Dairy" — are NOT line items; they have no Item No./Unit/Quantity of their own. Skip these rows entirely (leave `item_name_raw` blank / emit no line for them).
  - When you DO capture `item_name_raw`, copy the ENTIRE printed cell text verbatim — never truncate a multi-word label to its first token (e.g. capture "Canned/Dry Goods" in full, not "Canned").
