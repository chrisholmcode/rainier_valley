...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **The ID must reflect the SAME delivery_date you resolved above, including any legibility caveats — never emit a month in the ID that is more certain than the delivery_date it derives from.** If the handwritten month digit is smudged/ambiguous (see the single-digit month legibility warning above), do NOT silently pick a confident month here: carry the same lowered confidence and `source_warning`, and re-examine the month digit before committing. Recurring case on this supplier: SWY-GEN slips whose month digit was NOT clearly legible as "05" were being written as `SWY-GEN-2026-05-28` — when the month digit is ambiguous, do not fabricate "05"; flag it for the reviewer instead.
