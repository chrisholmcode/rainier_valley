...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` is ALWAYS the literal string `"grocery_rescue"` — never `"food_lifeline"`.** The big "FOOD LIFELINE" logo/text in the upper-left is the broker's letterhead, NOT the supplier value. Do not copy it into the `supplier` field. `food_lifeline` is a different bucket (printed Agency Order manifests) and must never be emitted as the supplier for these grocery-rescue slips.
