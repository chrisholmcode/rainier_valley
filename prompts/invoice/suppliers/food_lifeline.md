...
- Description => item_name_raw verbatim.
  - CAUTION: If the "line items" are actually per-category section labels (e.g., "Bakery", "Produce", "Canned/Dry Goods", "Meat", "Dairy") next to a Pounds column rather than named product descriptions, this is likely the handwritten grocery-rescue slip — re-check the "Not this supplier" section and set `supplier = "grocery_rescue"` if the Donor/Address/Agency/Date form is present.
  - If you do capture a category label as item_name_raw, copy it EXACTLY and in FULL — never truncate a multi-word/slash label (write "Canned/Dry Goods", not "Canned"; write "Bakery" only if that is the complete printed label).
  - If a category row has no item and no weight recorded, leave item_name_raw BLANK — do not emit the category heading as an invented line item. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
