...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is ambiguous (could be "7" or "8", ink smudged, only partially legible, or you had to guess between two candidate digits), do NOT silently pick one and do NOT emit a clean confident month such as "05". Emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`. Reviewers repeatedly correct the month on this supplier — a confidently zero-padded month (e.g. "05") on a slip whose month digit is not crisply legible is the exact failure they are fixing. Only emit a confident, un-warned month when the month digit is unmistakably legible; otherwise you MUST attach the warning and lower confidence.
