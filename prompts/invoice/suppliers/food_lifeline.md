...
- HANDWRITTEN POUNDS: If a line has NO printed Quantity value but DOES have a handwritten number in a "Pounds" column (common on food_lifeline slips), transcribe that handwritten pounds number into `quantity` verbatim. Read the full digits carefully (e.g., a leading-obscured "79" is not "70"); do not leave `quantity` blank when a Pounds figure is present. Only route the whole document to `grocery_rescue` when it is the Donor/Address/Agency/Date form described in "Not this supplier"; otherwise keep it here and fill quantity from Pounds.
