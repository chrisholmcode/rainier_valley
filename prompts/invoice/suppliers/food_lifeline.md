...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). Read the Ship Date digit-by-digit EXACTLY as printed; do NOT infer or adjust the month. On these manifests the Ship Date is printed MM/DD/YYYY (US order) — the FIRST number is the month, the SECOND is the day. Reviewers have repeatedly found the month transcribed one higher than what is printed (e.g., outputting 2026-08-07 for a slip that reads 07/07/2026); before finalizing, re-read the printed month character-for-character and confirm it matches the digit on the slip.
