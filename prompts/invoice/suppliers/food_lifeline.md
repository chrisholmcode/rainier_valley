...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). This Gross-Weight mapping applies ONLY to the printed AGENCY ORDER manifest.
  - approx_weight comes from the printed **Gross Weight** column ONLY. If instead you see a handwritten per-category **Pounds** column, this is the `grocery_rescue` document, not this supplier — re-check `supplier`.
  - Never invent, infer, or carry a weight over from another row. Set `approx_weight` to BLANK for any row whose weight cell is empty, illegible, struck through, or is a running subtotal/total rather than a real line weight.
