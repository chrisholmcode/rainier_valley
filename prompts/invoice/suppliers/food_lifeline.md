...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - WEIGHT GUARD: This `Gross Weight => approx_weight` rule applies ONLY to the printed AGENCY ORDER manifest with the columnar layout above. If the document is the HANDWRITTEN Donor / Address / Agency / Date slip with a per-category **Pounds** column (rows like Produce / Meat / Bakery / Non-Perishable), STOP — this is `supplier = "grocery_rescue"`, not food_lifeline. Do NOT read a single Pounds cell into `approx_weight` here, and do not leave a manifest weight blank by trying to reconcile multiple per-category rows. Re-route to `grocery_rescue` and follow that prompt.
