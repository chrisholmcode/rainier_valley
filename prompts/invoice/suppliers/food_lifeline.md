Supplier: Food Lifeline (AGENCY ORDER — printed manifest). NOTE: hand-filled grocery rescue slips are now a separate supplier (`grocery_rescue`) — if this document is the handwritten Donor / Address / Agency / Date form with a Pounds column, set `supplier = "grocery_rescue"` and follow that supplier's prompt instead. **Decisive test — do this FIRST, before extracting any weights:** if the weight figures are HANDWRITTEN and/or appear in a column labeled "Pounds" (as opposed to a printed "Gross Weight" column inside the Item No./Description/Unit/Quantity/Cubic Feet/Unit Fee/Total Fee table), this is NOT food_lifeline. Do NOT populate `approx_weight` here — reroute to `supplier = "grocery_rescue"`. Only extract weights under this prompt when the printed "Gross Weight" column is present in the manifest table.
