...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity = **REQUIRED — HARD RULE: after you set `approx_weight`, immediately copy it into `quantity`. If `approx_weight` is non-null, `quantity` MUST be non-null and byte-for-byte equal — never emit a row where `approx_weight` has a value but `quantity` is blank.** The Pounds cell is both the weight and the billed quantity. Examples: Pounds cell reads "17" → `approx_weight = 17`, `quantity = 17`. Pounds cell reads "0" → `approx_weight = 0`, `quantity = 0` (zero is a real, legible value — see below). Pounds cell reads "42" → `quantity = 42` (extract the FULL number; do not truncate "42" to "4").
