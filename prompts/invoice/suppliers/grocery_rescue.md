...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = a **verbatim transcription** of the Pounds cell — NOT the resolved/final weight. Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **Do NOT put the running-tally/final-value result here.** `approx_weight`/`quantity` hold the *resolved* number; `quantity_raw` holds *exactly what is written*. If a cell shows "84" but you resolve the weight to 64 (or vice versa), `quantity_raw` must be the string you actually see on the slip, not the resolved number.
    - **Do NOT invent a number.** Only populate `quantity_raw` when the Pounds cell actually contains legible writing. If the cell is blank, contains only hatching/smudges, or you cannot make out any digit with confidence, set `quantity_raw = null` (and treat the row per the blank-cell rules) — never guess a value into this field.
    - If a digit is genuinely ambiguous to read (e.g., 4 vs 6, 8 vs 6, 5 vs 7), transcribe your best reading, record both interpretations in a `source_warning`, and lower confidence to ≤ 0.6 rather than silently committing to one reading.
