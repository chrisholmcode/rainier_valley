...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to `YYYY-M-DD` **for this supplier**: the year is 4 digits, the **month is emitted WITHOUT a leading zero** (a single-digit month stays a single digit — reviewers consistently strip the zero-pad on this supplier's dates, e.g. `2026-05-28` should be `2026-5-28`), and the day keeps its normal two-digit form. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
