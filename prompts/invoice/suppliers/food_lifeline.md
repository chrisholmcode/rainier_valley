...
- Pounds column => quantity_raw. Transcribe the handwritten/printed number in the **Pounds** column verbatim as digits (e.g., "14" => 14, "79" => 79). Never leave `quantity_raw` blank when a Pounds figure is written on the slip — if the line has a weight in the Pounds column, that number IS the quantity_raw. Read handwritten digits carefully (a "9" vs "0", a "7" vs "1"); when the digit is genuinely ambiguous, transcribe your best single-digit reading rather than dropping the value.
