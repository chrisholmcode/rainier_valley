...
- item_name_raw must be the COMPLETE Description cell text, verbatim — do not truncate at the first word or drop a second half after a slash. Capture compound/slash-joined labels in full (e.g., "Canned/Dry Goods", not "Canned"). If a row shows only a bare category grouping header (e.g., a standalone "Bakery" or "Canned" with no Unit/Quantity/Weight values), it is a SECTION HEADER, not a line item — do not emit a line for it (leave item_name_raw blank / skip the row).
