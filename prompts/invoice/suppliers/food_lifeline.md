...
- donor_org: Read the donor/source code printed on the manifest (e.g., "SWY-GEN", "QFC-MI"). These are retail donor codes (SWY = Safeway, QFC = QFC/Quality Food Centers) with a trailing program/region suffix. Capture the code VERBATIM including the suffix. Only set donor_org = null if no such donor code is present anywhere on the document.
