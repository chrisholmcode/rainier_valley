...
- Description => item_name_raw verbatim.
  - EXCEPTION — category/section labels are NOT item names. If the Description cell contains only a category grouping word/phrase with no specific product (e.g., "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Dairy", "Frozen"), it is a section header, not a line item: leave `item_name_raw` blank and do not emit a line for it. Only rows naming a specific product (e.g., "TEFAP FB Chicken Drumsticks (1115795) FB") become line items. When a category label IS the printed value and it is abbreviated/truncated, transcribe the FULL label exactly as printed (e.g., "Canned/Dry Goods", not "Canned"). When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
