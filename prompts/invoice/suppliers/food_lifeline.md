...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - Pounds fallback: if the line has a **Pounds** value but NO separate printed "Quantity" column, use the Pounds number as `quantity` (and set `unit = "lb"`). Also set `approx_weight` to the same value. Never leave `quantity` blank when a Pounds figure is present. (If the whole document is the handwritten Donor/Address/Agency/Date slip, it should be `grocery_rescue` instead — see "Not this supplier".)
