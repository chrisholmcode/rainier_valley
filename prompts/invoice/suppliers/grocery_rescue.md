...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here. **quantity_raw is a PURE VERBATIM TRANSCRIPTION: it must contain ONLY the digits and the natural separators (spaces / line breaks / slashes) you actually see — never any interpretive words or markup.** Do NOT write "(circled)", "(boxed)", "(crossed out)", "/ 66", or any commentary inside quantity_raw. Which number is circled/final is recorded in `notes` and reflected in `approx_weight`, NOT in quantity_raw. Example: a cell where "51" is circled and "66" is written above it → `quantity_raw = "66 51"` (both numbers, no annotation), `approx_weight = 51`, `notes = "51 circled as final; 66 earlier"`. Before writing quantity_raw, re-read the cell digit-by-digit and confirm you have not dropped a leading digit (e.g. do not read "145" as "45"); if a leading digit is faint but present, include it and lower confidence.
