...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If there is no "Agency Order No" field but the slip carries a store/PO-style reference (e.g., a QFC/store code combined with a date such as "QFC-MI-2026-07-10"), capture that reference verbatim into `invoice_or_order_number`. Never leave this field blank when any such order/reference number is visible.
