...
- Description => item_name_raw verbatim. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
  - Category/section header rows are NOT line items. Some manifests group items under category banners like "Bakery", "Canned", or "Canned/Dry Goods". A header row has NO Item No., Unit, Quantity, or Gross Weight of its own — it only labels the rows beneath it. Do NOT emit a bare category label (e.g., "Bakery", "Canned") as `item_name_raw`; skip the header row entirely (produce no line for it). If a category grouping is relevant, record it in the line's `notes` (e.g., "category: Canned/Dry Goods"), and when you do capture such a label, copy the FULL printed text verbatim (e.g., "Canned/Dry Goods", not truncated "Canned"). Only rows with an actual product Description in the Description column become line items with `item_name_raw`.
