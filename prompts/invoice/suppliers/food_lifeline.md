...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). The Ship Date is printed as **MM/DD/YYYY** — transcribe the month digit-for-digit exactly as printed; do NOT shift, round, or infer the month. Double-check the month specifically (a recurring error is emitting a month one greater than printed, e.g. reading "07/07/2026" but writing 2026-08-07 — the correct output is 2026-07-07).
