...
- **Handwritten Pounds column fallback:** If this document is being processed as `food_lifeline` but has a handwritten **Pounds** column instead of a printed Quantity column (e.g., a rescue slip that was not re-routed to `grocery_rescue`), read each handwritten pound value into `quantity` for that line — never leave `quantity` blank when a Pounds value is written. Transcribe the handwritten digits exactly, reading each digit carefully (e.g., a trailing "9" vs "0"); do not round to a "cleaner" number.
