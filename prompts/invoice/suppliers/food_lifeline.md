...
- Description => item_name_raw verbatim. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
  - Do NOT emit a line item for **category/section header rows** — bare category labels (e.g., "Bakery", "Produce", "Meat", "Dairy") that group the items below them and have no Item No., Quantity, Unit, or Weight of their own. These are layout headers, not products; skip them (leave `item_name_raw` blank / produce no line).
  - When the description IS a real line item, capture the FULL label verbatim including slashes and multi-word category-style names — e.g. capture "Canned/Dry Goods" in full, not just "Canned". Do not truncate at the first word or at a "/".
