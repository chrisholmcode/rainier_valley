...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **Both the month and day MUST be two zero-padded digits, the month MUST be in the range 01–12, and the day in 01–31 (e.g., `2026-05-28`, never `2026-5-28` or `2026-0-28`).** If a resolved value would fall outside these ranges or be missing a digit, it is malformed — re-read the source, emit your best zero-padded reading, and add a `source_warning` noting the ambiguity rather than emitting an invalid string. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
