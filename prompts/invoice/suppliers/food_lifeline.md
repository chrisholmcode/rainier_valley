...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "Agency Order No" field is present, fall back to any other printed order/reference/PO number on the slip — this includes store- or program-prefixed identifiers such as "QFC-MI-2026-07-10". Never leave this field blank when a reference number is legible anywhere on the document; only null it out if genuinely no number appears.
