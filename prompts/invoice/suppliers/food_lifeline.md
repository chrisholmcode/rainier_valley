...
- Quantity column => quantity. If the line item has NO discrete printed Quantity column but instead reports the amount in a **Pounds** column (a common variant of this slip), use that Pounds value as `quantity` (e.g., a line reading 16 lb => quantity = 16). Never leave `quantity` blank when a Pounds value is present. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
