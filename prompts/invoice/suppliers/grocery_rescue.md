...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **Always emit a well-formed, zero-padded ISO date: the month component MUST be `01`–`12` and the day component MUST be `01`–`31` (e.g., May is `05`, never `5` and never `0`). A value like `2026-0-28` or any zero/blank month or day is invalid — never output it. If your reading would produce an out-of-range or non-zero-padded component, re-examine the slip and correct it before emitting.** If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
