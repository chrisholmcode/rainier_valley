...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **`quantity_raw` is digits-only — no annotation words or markup.** It is a plain transcription of the numerals (with spaces between distinct numbers), NOT a place to describe how they were annotated. Do NOT write "(circled)", "(boxed)", "(crossed out)", "/", or any word — put that observation in `notes` instead. Example: a cell showing a circled 51 → `quantity_raw = "51"` (NOT `"51 (circled)"`), and `notes` = "51 circled as final". A cell showing "50 (circled) then 66 written below" → `quantity_raw = "50 66"`, `notes` = "50 circled".
    - **Transcribe the FULL number — never drop a leading digit.** Before writing the value, count the digits you see left-to-right. A faint or tightly-spaced leading digit is still part of the number: "145" is three digits, not "45". If a digit is uncertain, emit your best full reading, lower confidence to ≤ 0.6, and add a source_warning rather than dropping it.
    - Only leave `quantity_raw` blank when the Pounds cell contains **absolutely no writing**. If any numeral is legible you must transcribe it — do not bail to blank because the cell is messy.
