...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If there is no ACR-style Agency Order No, capture any other printed reference/order number visible on the slip verbatim — including store- or donor-prefixed codes that embed a date (e.g., "QFC-MI-2026-07-10"). Never leave this field blank when any such reference number is present on the document.
