...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **Date component format in the synthesized key:** do NOT zero-pad the month in the ID. Strip any leading zero from the month so a single-digit month appears as one digit. Example: a pickup resolving to delivery_date `2026-05-28` → `invoice_or_order_number = SWY-GEN-2026-5-28` (NOT `SWY-GEN-2026-05-28`). The `delivery_date` field itself stays fully zero-padded ISO (`2026-05-28`); only the ID's month digit is un-padded.
