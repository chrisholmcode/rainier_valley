...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. **The `<delivery_date>` segment must be the full, zero-padded `YYYY-MM-DD` exactly as resolved — month and day ALWAYS two digits (e.g. `05`, `08`), never a single digit and never truncated. Note that both the donor_org code (e.g. `SWY-GEN`) and the date contain hyphens; append the date verbatim without dropping any leading zeros. Example: donor_org `SWY-GEN` + delivery_date `2026-05-28` → `SWY-GEN-2026-05-28` (NOT `SWY-GEN-2026-5-28` or `SWY-GEN-2026-0-28`).** If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
