...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` is ALWAYS the literal string `"grocery_rescue"` on these slips — NEVER `"food_lifeline"`.** The "FOOD LIFELINE" logo/branding printed on the form is the *broker* who runs the pickup; it is NOT the supplier bucket. Reading "Food Lifeline" off the page and emitting it as `supplier` is the single most common error on this supplier. The `food_lifeline` supplier value is reserved exclusively for printed Food Lifeline **Agency Order** manifests, which this handwritten grocery-rescue form is not.
