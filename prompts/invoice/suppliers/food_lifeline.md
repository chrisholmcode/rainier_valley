...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - Read the Gross Weight column carefully digit-by-digit; do not guess ambiguous glyphs. Common misreads on these manifests are a dropped or added leading digit (e.g., 17 vs 117, 7 vs 73) and confused glyphs (1↔7, 0↔4↔5).
  - Cross-check: approx_weight is the LINE TOTAL, so it should be roughly Quantity × a plausible per-case weight. If your read weight does not divide sensibly by Quantity, re-read the digits before committing (a value that implies an implausible per-case weight is usually a mis-transcription).
