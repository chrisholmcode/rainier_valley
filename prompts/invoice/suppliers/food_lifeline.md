...
- Description => item_name_raw verbatim.
  - Some slips group lines under printed CATEGORY labels (e.g., "Bakery", "Canned/Dry Goods", "Produce") rather than naming a specific product. If a line's only text is such a category label, transcribe it EXACTLY and IN FULL as printed — never abbreviate or truncate (e.g., write "Canned/Dry Goods", not "Canned"). If the label is only a section HEADING with no quantity/weight on that line (i.e., no actual item), leave `item_name_raw` blank rather than copying the heading text. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
