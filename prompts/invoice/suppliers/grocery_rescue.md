...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Per-row read discipline (grocery_rescue Pounds column is the most-corrected field):** Before assigning a value to any row, (a) transcribe the FULL contents of that row's Pounds cell into `quantity_raw` first, then derive `approx_weight`/`quantity` from it — never emit a numeric value you did not first record in `quantity_raw`. (b) **Read the entire number, including leading digits.** Grocery-rescue pounds are commonly three digits (100–200+); if you read a two-digit value (e.g. "45", "50"), double-check that there is no leading digit you clipped — prefer the fuller reading (e.g. "145") and note the ambiguity in a `source_warning`. (c) **Do not carry a value across rows.** A number belongs to a row only if it sits inside that row's Pounds cell; if a cell is empty, that row is blank (`approx_weight = null`) even when adjacent rows have values — never repeat a neighbor's number. If you find yourself emitting the same numeric value on two adjacent rows, re-examine — one of them is likely blank.
