...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - FORMAT: The output MUST be a strict, zero-padded `YYYY-MM-DD` (each of month and day is exactly two digits, e.g., `2026-07-07`). Never emit a single-digit or empty month/day segment (e.g., `2026-0-07` and `2026-7-7` are invalid — pad to `2026-07-07`).
  - READING CARE: Read the month digits directly off the printed Ship Date; do NOT round or infer. The printed "07" (July) is frequently misread as "08" (August) on these manifests — verify the second month digit before writing it.
