...
- **Rows with a non-empty Pounds cell:**
  ...
  > - **A hand-written `0` is a REAL value, not an empty cell.** If the Pounds cell contains a written zero, extract `approx_weight = 0`, `quantity = 0`, `quantity_raw = "0"`. Only the "empty Pounds cell (blank, no writing)" branch below may produce null — and it applies ONLY when the cell contains no writing at all.
  > - **Never emit a sentinel, placeholder, or negative value** (e.g. `-1`, any negative number, `"N/A"`). If a numeral is present but hard to read, emit your best-guess reading, lower `confidence` (≤ 0.6), and add a `source_warning` — do NOT substitute `-1` or any placeholder. Use `null` ONLY when the cell is genuinely empty (no writing).
