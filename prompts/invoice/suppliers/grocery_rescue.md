...
- Common fields for every row:
  ...
  - unit = "lb".
  - category = per the table above. **Derive category ONLY from THIS row's own `item_name_raw`, never from its position in the emitted list.** Do not let the category drift relative to the row label — a common error is shifting the whole column up/down by one so, e.g., the Frozen Foods row gets "dairy" (the row above it) or the Meat row gets "frozen". Before finalizing, re-read each row's label and confirm the pairing exactly matches the predefined table: Bakery/Canned/Dry/Coffee Kiosk→shelf_stable, Dairy/Juice/Alt. Dairy→dairy, Frozen Foods→frozen, Meat→meat_protein, Nonfood→non_food, Non-Meat Protein→dairy, Prepared/Perishable & Produce→produce. If a label and its category don't line up, you have shifted rows — realign before emitting.
