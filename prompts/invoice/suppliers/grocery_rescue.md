...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is at all ambiguous — could be "7" or "8", ink smudged, a stray stroke, or only partially legible (this is the single most-corrected part of the date on this supplier) — do NOT silently pick one. Emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`. Whenever the month is written as a single handwritten digit, treat it as verify-worthy: prefer flagging for review over emitting a confident month.
  - **Month value sanity guard:** the resolved month MUST be in range 01–12. Never emit a date whose month component is "0", "00", or otherwise outside 01–12. If your reading produces an out-of-range or implausible month, re-examine the digit, emit your best in-range reading, lower confidence to ≤ 0.6, and add the month-digit `source_warning` above.
