...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **The "FOOD LIFELINE" logo on the slip is NOT the supplier.** These forms carry Food Lifeline branding, but `supplier` MUST be `"grocery_rescue"` — never `"food_lifeline"`. `food_lifeline` is reserved only for printed **Agency Order** manifests, which these handwritten grocery-rescue donation forms are not. Do not copy the logo text into the supplier field.
