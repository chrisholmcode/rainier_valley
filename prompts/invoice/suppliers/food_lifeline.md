...
- If this slip has NO printed "Quantity" column but instead a handwritten **Pounds** column (grocery-rescue-style layout), the value in the Pounds column is the `quantity` for that line. Read the handwritten number carefully digit-by-digit (e.g., 79 not 70) and never leave `quantity` blank when a Pounds figure is present. (If the document is fully the Donor/Address/Agency/Date handwritten form, prefer routing to `supplier = "grocery_rescue"` per the note above.)
