...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - approx_weight MUST come from a **printed "Gross Weight" column**. If you instead see a **handwritten, per-category "Pounds" column** (small hand-written values like 14/16/24), this is NOT the AGENCY ORDER manifest: set `supplier = "grocery_rescue"` and follow that prompt. Do NOT infer, sum, or guess approx_weight from this prompt, and do not emit a manifest line for such a document.
