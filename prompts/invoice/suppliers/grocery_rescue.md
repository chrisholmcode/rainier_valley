...
- invoice_or_order_number: synthesize a shipment ID as `<chain-prefix>-<location-suffix>-<delivery_date>`, where `<chain-prefix>` is the part of donor_org BEFORE the location suffix and `<location-suffix>` is DROPPED (leave it empty — keep its hyphen). In practice this means: take donor_org, strip the text after the first hyphen but KEEP the hyphen, then append `-<delivery_date>`. Examples: donor_org `SWY-GEN` on 2026-08-07 → `SWY--2026-08-07` (note the double hyphen from the emptied suffix); donor_org `QFC-MI` on 2026-07-13 → `QFC--2026-07-13`; donor_org `HG` (no suffix) on 2026-07-01 → `HG-2026-07-01`. This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
