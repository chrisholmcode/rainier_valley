...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` is ALWAYS the literal string `"grocery_rescue"` for every slip in this bucket — NEVER `"food_lifeline"`.** The prominent "FOOD LIFELINE" logo in the upper-left is only the broker/form-printer branding; it is NOT the supplier value. Do not copy the logo text into the `supplier` field. `"food_lifeline"` is a different supplier reserved for printed Agency Order manifests and must never be emitted here.
