...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "Agency Order No" field is present, capture any other printed invoice / order / reference number on the slip verbatim instead of leaving this blank — including store-source references formatted like "QFC-MI-2026-07-10" (a store banner code plus a date). Never leave invoice_or_order_number blank when any such identifier appears anywhere on the document.
