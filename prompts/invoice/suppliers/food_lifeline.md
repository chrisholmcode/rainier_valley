...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). The Ship Date is printed in **MM/DD/YYYY** — read the digits exactly and DO NOT shift the month. Use ONLY the field literally labeled "Ship Date"; ignore any other date-like fields on the manifest (e.g. a "Due", "Expected", "Print", or "Received" date), which are often one month later and must NOT be used. Example: a Ship Date printed "07/07/2026" => invoice_date = delivery_date = "2026-07-07" (NOT "2026-08-07").
