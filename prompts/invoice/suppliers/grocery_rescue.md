...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Never emit a malformed or zero month/day.** The resolved date MUST be a real calendar date with a two-digit month `01`–`12` and a two-digit day `01`–`31` (e.g., `2026-08-12`, never `2026-0-12`, `2026-00-12`, or `2026-8-1`). If a digit is illegible, commit to your single best legible reading (zero-padded) and use the month-/day-legibility warnings — do NOT output an empty or `0` month/day. Because `invoice_or_order_number` is synthesized directly from this value, a malformed date here silently corrupts the shipment ID too.
  - **Day-digit transposition warning:** the day is as error-prone as the month. If the two day digits could plausibly be read in either order (e.g., "03" vs "31", "13" vs "31", "02" vs "20"), do NOT assume the larger value and do NOT misread a leading-zero single-digit day ("03") as a two-digit day ("31"). Prefer the reading consistent with `Today's date` (grocery rescue slips are filled the day of pickup, so the day is normally at or before the upload day and within a few days of it). Emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be transposed — please verify against slip"`.
