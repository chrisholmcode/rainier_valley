...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Read the Ship Date digit-by-digit, paying special attention to the **month**: on these printed manifests a "7" is frequently misread as "8". Do not guess — transcribe exactly what is printed, and never output an invalid month (e.g., a single "0"). If the month is ambiguous, prefer the digit consistent with the rest of the manifest's date fields.
