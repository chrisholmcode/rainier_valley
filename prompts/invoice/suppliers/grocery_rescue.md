...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **supplier is ALWAYS `grocery_rescue` for this form — NEVER `food_lifeline`.** The "FOOD LIFELINE" logo/branding on the slip is the broker's name, NOT the supplier value. Do not copy the logo text into the supplier field. `food_lifeline` is a separate supplier reserved for printed Agency Order manifests and must never be emitted from this handwritten grocery-rescue form, regardless of how prominently "Food Lifeline" appears on the slip.
