...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to strict `YYYY-MM-DD` with the month and day ALWAYS zero-padded to two digits (`MM` and `DD` — e.g., May 28, 2026 → `2026-05-28`, never `2026-5-28` or `2026-0-28`). Emit exactly 10 characters: 4-digit year, hyphen, 2-digit month, hyphen, 2-digit day. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
