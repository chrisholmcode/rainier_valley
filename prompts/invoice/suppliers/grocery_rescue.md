...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - **⚠️ supplier is ALWAYS the literal string `grocery_rescue` for these slips — NEVER `food_lifeline`.** The "FOOD LIFELINE" logo printed at the top is the broker's branding and is EXPECTED on every one of these forms; it is NOT the supplier value. Do not copy the logo text into the `supplier` field. The value `food_lifeline` is reserved exclusively for printed Food Lifeline **Agency Order** manifests, which these handwritten rescue forms are not.
