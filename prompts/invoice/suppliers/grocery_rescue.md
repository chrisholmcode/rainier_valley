...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit legibility warning (off-by-one day):** the day digit is just as error-prone as the month, and because `invoice_or_order_number` is synthesized directly from the date, a misread day silently corrupts the shipment key. Handwritten day digits are frequently confused (esp. **5 vs 6**, 1 vs 7, 0 vs 8, 3 vs 8). If the day digit is at all ambiguous, do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be off by one — please verify against slip"`. This warning must propagate to the synthesized invoice_or_order_number as well.
