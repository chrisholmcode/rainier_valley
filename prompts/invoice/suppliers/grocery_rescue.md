...
- **Rows with a non-empty Pounds cell:**
  ...
  > ⚠️ **Anchor each Pounds value to its own row+column cell before parsing.** A grocery-rescue table is dense and hand-filled; numbers frequently sit near column borders. Before writing `approx_weight` for a row, verify the number is physically inside the **Pounds (lb)** column of **that row** — not bleeding in from the Pick Up Temp / Drop Off Temp columns to its left. Refrigeration/temperature readings are typically **~30–55** (e.g., 38, 41, 51); if the only number you see for a row sits under a Temp column, that row's Pounds cell is **blank** (approx_weight = null), do NOT copy the temp value into Pounds.
  > **Also read the full width of the Pounds cell:** capture the leading digit(s) — do not truncate a 3-digit weight to 2 digits (e.g., "145" must not become "45"). If a leading digit is faint or clipped at the cell edge, prefer the fuller reading, put every legible digit in `quantity_raw`, and lower confidence to ≤ 0.6.
