...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Read the Ship Date's month carefully. This is the most-corrected field on this supplier: the model frequently misreads the month digit (e.g., outputting `08`/August when the manifest actually shows `07`/July). Before finalizing, re-examine the printed month character and do not "round up" or infer a later month — transcribe exactly what is printed.
  - Output must be a fully zero-padded, valid date: `YYYY-MM-DD` with `MM` in the range `01`–`12` and `DD` in `01`–`31`. Never emit a single-digit or empty month segment (e.g., `2026-0-07` and `2026-8-07` are invalid).
