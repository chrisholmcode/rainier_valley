...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - approx_weight MUST come ONLY from a **printed "Gross Weight" column**. If the only weight figures are small handwritten numbers in a **"Pounds" column** (typically single/double digits like 14, 16, 21, 24 per row), this is NOT an AGENCY ORDER manifest — STOP and re-route to `supplier = "grocery_rescue"` and follow that prompt instead. Do not guess or leave approx_weight blank on this prompt; hand the document off.
