...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Zero-pad and never invent digits.** Both month and day in the output MUST be exactly two digits (`MM-DD`) with a leading zero for single-digit values (month `8` → `08`, day `3` → `03`). Never emit a single-digit or empty month/day segment like `2026-0-12` or `2026-8-3` — if you can only read one month digit, that digit IS the month and must be padded (a lone `8` → `08`), never rendered as `0` or left partial.
  - **Do NOT transpose or pad-up a day.** A written day of `3` is `03`, NOT `31`. Never turn a single-digit or leading-zero day into a two-digit day by appending or reversing digits (e.g. `8/3` → `2026-08-03`, NEVER `2026-08-31`). Read the day exactly as written; if the day digit is ambiguous, emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> — please verify against slip"`.
