...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Well-formedness (always enforce):** the resolved `delivery_date`/`invoice_date` MUST be a valid, fully zero-padded `YYYY-MM-DD` where month is `01`–`12` and day is `01`–`31`. Never emit a month of `0`, `00`, or a single-digit / truncated month (e.g. `2026-0-28` or `2026-5-28` are both invalid). If your best reading of the month cannot form a valid two-digit month in `01`–`12`, do NOT guess a value — set `delivery_date` and `invoice_date` to null, lower confidence to ≤ 0.6, and add a `source_warning`: `"delivery_date month illegible/invalid — please read from slip"`.
