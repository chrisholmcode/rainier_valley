...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **Digits and single spaces ONLY.** `quantity_raw` is a raw transcription, NOT an interpretation. Do NOT include annotation words ("circled", "boxed", "crossed out"), markup symbols ("/", parentheses around numbers), or pattern commentary — those belong in `notes`. A cell showing a circled 51 → `quantity_raw = "51"` (NOT `"51 (circled)"`, NOT `"50 (circled) / 66"`); put "50 crossed to 51, 51 circled as final" in `notes`.
    - **Transcribe complete multi-digit numbers — never drop a leading (or any) digit.** Before writing, re-read the cell left-to-right and confirm every digit is captured; a cell reading "145" must produce `quantity_raw = "145"`, not `"45"`. Apply the digit-boundary caution below.
    - If, after transcribing, the cell contains only a stray mark, dot, or unreadable scribble with no confident numeral, treat the cell as blank (quantity_raw = null) rather than transcribing an artifact.
