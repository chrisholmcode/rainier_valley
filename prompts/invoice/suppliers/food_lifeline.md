...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Read the value from the field LABELED "Ship Date" ONLY. These manifests often print other nearby dates (e.g., "Print Date", "Due Date", "Processed") that are typically LATER — usually one month after the Ship Date. Do NOT use those. If two candidate dates differ only in the month, prefer the earlier one that sits directly against the "Ship Date" label.
  - Sanity check: the month you output must match the month on the "Ship Date" line, not an adjacent printed/processing date. Re-read the month digit carefully before finalizing.
