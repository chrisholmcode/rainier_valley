...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **ID date-component formatting:** build the date portion of the shipment ID from the resolved delivery_date but **do NOT zero-pad the month** — the reviewer-canonical form drops the leading zero on the month while keeping the day as written. So a May-28 pickup from Safeway Genesee becomes `SWY-GEN-2026-0-28` (month rendered without its leading zero), NOT `SWY-GEN-2026-05-28`. Note: this affects ONLY the synthesized `invoice_or_order_number`; the standalone `delivery_date`/`invoice_date` fields remain fully zero-padded YYYY-MM-DD.
