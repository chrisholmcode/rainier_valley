...
- **Rows with a non-empty Pounds cell:**
  ...
  > **Per-row cell isolation (do NOT bleed values between rows).** A numeral only counts as "in this row's Pounds cell" if it is physically inside that row's cell boundary, aligned with that row's label. Do NOT carry, copy, or repeat a number from an adjacent row (above or below) into a row whose own Pounds cell is empty. If a value appears to span or sit on a gridline between two rows, assign it to exactly ONE row (the one it aligns with) and leave the other blank per the empty-cell rules. On densely-filled forms the same number appearing on multiple consecutive rows is almost always a mis-read — verify each cell independently.
  > **Read the FULL number, including leading digits.** Before emitting, re-scan the left edge of the cell for a leading digit you may have clipped (e.g., "145" mis-read as "45", "156" as "56"). Grocery-rescue weights are commonly 3 digits (100–300 lb); a suspiciously low 2-digit reading in a busy cell should be re-examined for a dropped leading "1". If unsure, report both readings in `quantity_raw` and lower confidence.
