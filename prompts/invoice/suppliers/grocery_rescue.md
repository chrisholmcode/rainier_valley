...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` MUST be `"grocery_rescue"` — NEVER `"food_lifeline"`.** The "FOOD LIFELINE" logo printed on the slip is the *broker's* branding, not the supplier value. Do not copy the on-slip brand into the `supplier` field. The `food_lifeline` bucket is reserved exclusively for printed Food Lifeline **Agency Order** manifests; every handwritten grocery-rescue donation form (this file) is always `supplier = "grocery_rescue"`, regardless of the logo.
