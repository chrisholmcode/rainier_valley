...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED for a non-empty cell — but it is a strict VERBATIM transcription, never a derived or resolved value.** Copy every visible digit/number from **THIS row's own Pounds cell** exactly as written, in the order written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "151 123 108 40" → `quantity_raw = "151 123 108 40"`; a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Do NOT collapse a multi-number tally down to the single resolved value — the resolved value belongs only in `approx_weight`/`quantity`; `quantity_raw` must retain the full raw sequence. **Anti-fabrication / anti-bleed guard:** never copy, borrow, or infer a number from an adjacent row, from another line's running tally, or from the Total box. A numeral may be extracted for this row ONLY if it is clearly written inside this row's own Pounds cell. If this row's Pounds cell contains no writing of its own — even if a neighbouring cell or a tally artifact overlaps into view — treat it as an empty cell: `quantity_raw = null`, `approx_weight = null`, `quantity = null` (per the empty-cell rule). Do not fabricate a value to satisfy the skeleton "emit all 10 rows" requirement. Even a lone clean number that IS in this cell must be emitted here.
