...
- donor_org: Capture the donor/source code printed on the manifest verbatim (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC). This is the retail donor code identifying who originally donated the goods — do NOT confuse it with the `Sold To` destination. Only set `donor_org = null` if no donor/source code is present on the document.
