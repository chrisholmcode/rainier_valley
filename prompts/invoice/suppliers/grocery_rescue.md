...
- **Rows with a non-empty Pounds cell:**
  ...
  - approx_weight = the final/accepted pounds, parsed per the **Running-tally rule** below. **NEVER leave `approx_weight` blank when any numeral is legible.** If the number is hard to read, extract your best guess, lower `confidence` to ≤ 0.6, and add a `source_warning`. Only set `approx_weight = null` when the cell contains **absolutely no writing**.
    - **Read-verification (do this before emitting — two common error directions on this supplier):**
      - **Don't drop leading digits.** Count the digits you're transcribing and re-read the cell to confirm nothing precedes your first digit. A leading "1" is easy to miss on hand-filled forms (e.g. "145" mis-read as "45", "156" as "56"). Grocery-rescue pallet/bin weights are commonly 3 digits (100–200 lb); if your reading is a suspiciously small 2-digit number, re-check for a dropped hundreds digit before committing, and confirm the units digit exactly.
      - **Don't fabricate a value.** Confirm the number you're reading is actually written in the **Pounds (lb)** column of *this* row — not bleed-through, a stray mark, a struck-out artifact, or a value that belongs to the (ignored) Temp columns. If, on second look, the Pounds cell has no real writing of its own, emit `approx_weight = null` (and quantity/quantity_raw = null) rather than inventing a number; add `source_warning "Pounds cell may be empty — value uncertain, please verify"` if you're unsure.
