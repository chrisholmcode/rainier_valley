...
- donor_org: capture the donor source code printed on the manifest verbatim, including its hyphenated suffix (e.g., "SWY-GEN", "QFC-MI"). These identify the originating retail donor (e.g., SWY = Safeway, QFC = QFC) with a location/program suffix. Only set `donor_org = null` if no such donor code appears on the document.
