...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). Read ONLY the field literally labeled "Ship Date" (ignore any "Print Date", "Due Date", "Order Date", or system timestamp printed elsewhere on the page). The Ship Date is printed as M/D/YYYY (US month-first order). Transcribe the MONTH exactly as printed — do NOT shift, increment, or round it. Sanity-check the month position before emitting: a recurring error on this supplier is emitting the following month (e.g., writing 2026-08-07 when the slip actually reads 7/7/2026, which must be 2026-07-07).
