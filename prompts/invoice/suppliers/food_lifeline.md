...
- Pounds column => quantity_raw (verbatim numeric value, e.g. a handwritten "24" => quantity_raw = 24). If a slip has a Pounds column, ALWAYS populate quantity_raw from it — never leave quantity_raw blank when a pounds figure is legible. (Note: if the document is the handwritten Donor / Address / Agency / Date form, it should be routed to `grocery_rescue`; but if it is still being processed here, capture the Pounds value into quantity_raw rather than emitting nothing.)
