...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED for a non-empty cell — but numerals ONLY.** Copy the numbers from this row's Pounds cell exactly as written, preserving all separate numbers including crossed-out ones and earlier tally values, separated by single spaces (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **Do NOT write annotation words or glyphs** — no "(circled)", "(boxed)", "(crossed out)", no "/" or other separators. Only digits and spaces. If a number is circled, still just write the digits (record the circling in `notes`, and use it to pick `approx_weight` per the Running-tally rule). Example: a cell showing a circled "50" next to a "66" → `quantity_raw = "50 66"`, `notes = "50 circled as final; 66 also present"`.
    - **Re-read multi-digit numbers before writing** — verify leading digits so you never drop or add one (e.g. "145" must not become "45"). If the leading digit is faint, report both readings and lower confidence to 0.6.
    - **Only transcribe numerals you can attribute to THIS row's Pounds cell.** If no legible numeral truly belongs to this row, set `quantity_raw = null` and treat the row as empty (per the blank-cell rules) — do NOT borrow a value from an adjacent row or from a stray mark.
