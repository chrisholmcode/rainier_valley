...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Read the **month exactly as printed** — do NOT add or shift a month. If the Ship Date is shown numerically (e.g., "07/07/2026") interpret it as MM/DD/YYYY, so "07/07/2026" => `2026-07-07` (July), NOT `2026-08-07`. If the month is spelled/abbreviated (e.g., "Jul"), map it directly to that month number. Transcribe the printed month digit(s) verbatim; a value like `2026-08-07` from a July Ship Date is an error.
