...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit legibility warning (off-by-one):** the DAY portion is the most-corrected part of the synthesized invoice number on this supplier — handwritten day digits are frequently misread by ±1 (e.g., a "6" read as "5", "0" as "6", or a trailing loop dropped). If the day digit is at all ambiguous, do NOT silently commit to one reading: emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be ±1 — please verify against slip"`. Since invoice_or_order_number is derived from delivery_date, this same warning covers the invoice number.
