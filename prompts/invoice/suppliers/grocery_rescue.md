...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` is ALWAYS the literal string `"grocery_rescue"` for every slip in this bucket — NEVER `"food_lifeline"`.** The prominent "FOOD LIFELINE" logo printed on the form is the *broker's* branding, not the supplier value. Do not copy the on-page logo text into the `supplier` field. The value `food_lifeline` is reserved exclusively for printed Food Lifeline **Agency Order** manifests, which these handwritten grocery-rescue forms are NOT.
