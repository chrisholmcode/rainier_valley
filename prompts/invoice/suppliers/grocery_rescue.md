...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **supplier is ALWAYS `"grocery_rescue"` for these slips — NEVER `"food_lifeline"`.** The "FOOD LIFELINE" logo printed on the form is the brokering org's branding, NOT the supplier value. Do not copy the logo text into the `supplier` field. `"food_lifeline"` is reserved for printed Food Lifeline Agency Order manifests only (a different form). If you see the FOOD LIFELINE logo on a handwritten grocery rescue donation form, `supplier = "grocery_rescue"`.
