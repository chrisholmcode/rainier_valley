...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). approx_weight comes ONLY from the printed "Gross Weight" column of the AGENCY ORDER manifest. If the document has no printed Gross Weight column — e.g., a handwritten sheet with a per-category **Pounds** column and small hand-written numbers (like 14, 16, 21, 24) — this is NOT this supplier: STOP, set `supplier = "grocery_rescue"`, and follow that prompt. Never copy a handwritten per-category Pounds value into `approx_weight` on this supplier, and never leave `approx_weight` blank when a printed Gross Weight column is present.
