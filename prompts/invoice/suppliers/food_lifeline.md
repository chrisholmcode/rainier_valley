...
- **approx_weight SOURCE GATE (this supplier only):** approx_weight comes EXCLUSIVELY from the printed "Gross Weight" column on the AGENCY ORDER manifest. If the document has no printed "Gross Weight" column but instead a **handwritten per-category "Pounds" column** (Produce/Meat/Dairy/etc. with hand-filled weights), this is NOT a Food Lifeline AGENCY ORDER manifest — STOP, set `supplier = "grocery_rescue"`, and follow that supplier's prompt. Never transcribe a handwritten Pounds figure into this supplier's approx_weight.
