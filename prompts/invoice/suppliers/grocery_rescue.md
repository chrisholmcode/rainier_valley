...
- **Rows with a non-empty Pounds cell:**
  ...
  > ⚠️ **Handwriting-legibility guard (read every digit, and don't invent one):**
  > - **Do not drop a leading digit.** Grocery rescue Pounds values are frequently in the 100–199 range. Before finalizing, ask whether a faint or cramped leading "1" could precede what you read (e.g., a value read as "45" may actually be "145", "50" may be part of a larger number). If a leading hundreds digit is plausible but uncertain, emit your best reading, set confidence ≤ 0.6, and add a `source_warning`: `"pounds value may have a leading digit — read as <X>, verify against slip"`.
  > - **Off-by-one last digit:** if the final digit is ambiguous (e.g., could be 0 or 1, 4 or 9), do not silently commit — emit your best reading and set confidence ≤ 0.7.
  > - **But do NOT fabricate a value on a blank cell.** The "never leave blank" rule applies ONLY when there is actually ink/writing in the Pounds cell. If the cell has **no writing** — even though this is a predefined rescue row and you expect a number — you MUST emit `approx_weight = null, quantity = null, quantity_raw = null` and notes "no value on form". A pre-printed skeleton row, a stray mark, or a hatched temp column is NOT a value. When unsure whether faint marks are a real number or noise, prefer `null` and add a `source_warning` rather than guessing a specific number.
