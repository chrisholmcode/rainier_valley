...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** This is a PURE VERBATIM TRANSCRIPTION of the ink in the Pounds cell — copy the digits exactly as written and nothing else.
    - **No annotations.** Never add interpretive words or symbols such as "(circled)", "(boxed)", "(crossed out)", or " / " between alternate readings. `quantity_raw` contains only the digits, spaces, and line breaks that are physically on the slip. Put any interpretation of which number is final in `notes`, not here.
    - **Transcribe every digit of each number exactly — do not drop or add a leading digit.** If the cell shows "145", write "145", never "45". If you are unsure whether a mark is a digit, keep it and lower confidence rather than silently omitting it.
    - **Do not invent or merge candidate values.** If only one number is written, emit exactly that one number (e.g., "51" → `quantity_raw = "51"`); do not report a second guessed value.
    - Preserve genuinely-present crossed-out or earlier tally numbers separated by their real spaces/line breaks (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`).
    - **If the Pounds cell has absolutely no writing, `quantity_raw = null` — never hallucinate a value or an annotation for an empty cell** (this row then follows the empty-cell rule below).
