...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` is ALWAYS `"grocery_rescue"` for these handwritten forms — never `"food_lifeline"`.** The "FOOD LIFELINE" logo printed upper-left is the form vendor's branding, NOT the supplier value. Do NOT copy the logo text into `supplier`. `food_lifeline` is reserved exclusively for printed Food Lifeline **Agency Order** manifests, which this handwritten grocery-rescue form is not.
