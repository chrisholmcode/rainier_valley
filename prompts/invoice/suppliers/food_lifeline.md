...
- item_name_raw must be copied EXACTLY as printed on that line — do NOT truncate multi-word or slash-joined labels (e.g., print "Canned/Dry Goods", not "Canned"). Copy the complete text of the Description/label cell including any `/` or trailing words.
- Do NOT emit category/section-band headers (e.g., "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Meat") as their own line items. If a row is only a category header/subtotal band with no Item No., Quantity, or Gross Weight of its own, leave `item_name_raw` blank (do not fabricate a name from the band header). Only apply the band label as a category classifier to the real line items beneath it.
