...
- **Rows with a non-empty Pounds cell:**
  ...
  > **Digit-read discipline (grocery_rescue Pounds cells are the #1 source of corrections):**
  > - **Read each digit position deliberately.** A faint or short leading stroke is usually a real "1" — grocery rescue quantities are typically 2–3 digit poundages, so a bare "45" or "50" is more often "145"/"150" than not. When a leading digit is faint or the number looks implausibly small for a full pallet/bin, prefer the fuller-magnitude reading, transcribe every stroke you see into `quantity_raw`, and set confidence ≤ 0.7 with a source_warning: `"Pounds value leading digit uncertain: read as <value> — please verify"`.
  > - **One cell, one row — never borrow a number from an adjacent row.** A value belongs only to the row whose Pounds cell physically contains it. If a row's own Pounds cell is truly empty, it is blank (null) even if the row above/below has a tally — do NOT carry or infer a value into it. Do not merge numbers across row boundaries the way the running-tally/summed-weighings rules merge numbers *within a single cell*.
  > - "Never leave blank" applies ONLY when a numeral is actually legible **in that row's own cell**. It does not license inventing a value for an empty cell.
