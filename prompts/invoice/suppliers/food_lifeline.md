...
- If the slip has a **Pounds** column (as on the hand-filled grocery-rescue-style form), copy each line's Pounds value VERBATIM into `quantity_raw` (e.g. Pounds "24" => `quantity_raw = "24"`). Never leave `quantity_raw` blank when a Pounds value is written. Note: the printed AGENCY ORDER manifest has no Pounds column — for it, use the Quantity column for `quantity` and Gross Weight for `approx_weight` as described above.
