...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - The Ship Date is printed **day-first (DD/MM/YYYY)**, e.g. "07/07/2026" = 2026-07-07. Do NOT interpret it as MM/DD; when day and month are ambiguous, treat the FIRST number as the day and the SECOND as the month.
  - Read ONLY the field literally labeled "Ship Date". Ignore any other nearby date (print date, due date, received date), which may carry a different (often later) month and has caused month-off-by-one errors.
  - Sanity check: if your parsed month is one greater than the printed second number, you have transposed day/month — re-read.
