...
- document_type = "manifest". supplier = "food_lifeline". is_donation = true. donor_org: extract the retail/donor source code printed on the manifest (e.g., "SWY-GEN" = Safeway General, "QFC-MI" = QFC). It typically appears as a donor/source label near the line items or in the Item No. source suffix — capture it verbatim (uppercase, preserving the hyphen). Only set donor_org = null if no such source code is present on the document.
