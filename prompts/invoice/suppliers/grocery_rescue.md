...
- **Always emit one line item per predefined row — all 10, every time, in the order above.** This gives the reviewer a pre-populated skeleton to correct if the extractor missed a value, so they never have to manually add a row. Never skip a row.
  - ⚠️ **Row-alignment guard (prevents off-by-one category errors):** The 10 rows in the template are a *checklist*, NOT a guarantee that the printed form lists categories in this exact order or with no gaps. Bind each Pounds value to the category by reading the **label printed on that same horizontal row of the slip** — trace across from the written number to the leftmost Product/Description cell and read *that* label, then map it via the table. Do NOT assign a category based on the row's position in this list. Before finalizing, sanity-check adjacency: because `Dairy/Juice/Alt. Dairy` (dairy), `Frozen Foods` (frozen), and `Meat` (meat_protein) sit next to each other, a value read one line too high or too low will silently land in the neighboring category. If a written weight does not clearly line up with a printed label, lower confidence to ≤ 0.6 and add a `source_warning: "row/category alignment uncertain — please verify which printed label each weight belongs to"`.
