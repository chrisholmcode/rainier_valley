...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). Read the Ship Date carefully digit-by-digit as month/day/year and re-emit in strict ISO 8601: exactly four year digits, a two-digit month (01–12), and a two-digit day (01–31), zero-padded. The month MUST be a valid 2-digit value between 01 and 12 — never output a single-digit or empty month segment like `2026-0-07` or `2026--07`. Do not confuse the day field for the month; if the parsed month is not in 01–12, re-read the field before emitting.
