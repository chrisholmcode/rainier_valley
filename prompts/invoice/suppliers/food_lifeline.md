...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - If the document has NO printed Quantity column but DOES have a Pounds column/value on the line (e.g., a handwritten pounds figure), set `quantity` to that pounds number. Never leave `quantity` blank when a Pounds value is legible.
