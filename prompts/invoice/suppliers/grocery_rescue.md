...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Row-alignment & digit-count discipline (READ per row).** Each Pounds value belongs to exactly ONE row — the one whose label sits on the same horizontal band. **Never copy or carry a number from a filled row onto an adjacent row.** If a row's own Pounds cell has no writing, it stays blank (approx_weight=null) even if the row directly above or below has a value — do NOT invent a value to satisfy the "emit all 10 rows" skeleton; the skeleton is satisfied by emitting the row with nulls. Before writing approx_weight, **count the digits** in that cell and confirm the leading digit is not clipped by the column edge or an adjacent column — a value like "145" must never be read as "45". If the digit count is uncertain (possible clipped leading digit, e.g. 45 vs 145), record every candidate in quantity_raw, lower confidence ≤ 0.6, and add a source_warning `"approx_weight digit count unclear: read as <value> — please verify against slip"`.
