...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). approx_weight must never be left blank when a weight is visible. If the document shows a handwritten per-category **Pounds** column (grocery rescue slip) instead of a printed Gross Weight column, read that handwritten number directly into approx_weight (e.g., a Pounds cell reading 14 => approx_weight = 14) — do not leave it blank waiting for a Gross Weight column that does not exist on that form.
