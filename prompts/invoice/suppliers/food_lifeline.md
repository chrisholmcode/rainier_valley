...
- Description => item_name_raw verbatim. Copy the ENTIRE Description cell exactly, including slashes, spaces, and all tokens — e.g. "Canned/Dry Goods" (not just "Canned"). Do NOT truncate at a "/" or first word.
  - Some manifests print category/section header banners that span the row (e.g., a standalone "Bakery", "Canned", "Produce" line with no Item No., Unit, Quantity, or Gross Weight). These are grouping labels, NOT line items — do NOT emit them as `item_name_raw`; skip them entirely. Only rows with an actual Item No./Quantity are line items. If a row's only content is such a banner AND it also appears in a real Description cell, use the full Description text there. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
