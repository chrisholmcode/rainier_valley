...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - The Ship Date is printed as M/D/YYYY (e.g., "7/7/2026"). Read the MONTH digit carefully — do not confuse a "7" (July) for "8" (August). Transcribe to ISO by zero-padding both month and day to two digits: "7/7/2026" => `2026-07-07`.
  - Sanity check before emitting: the output MUST be exactly `YYYY-MM-DD` with a two-digit month (01–12) and two-digit day (01–31). Never emit a single-digit or empty month/day segment (e.g., `2026-0-07` and `2026-8-7` are invalid).
