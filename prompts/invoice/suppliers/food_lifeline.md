...
- **Pounds column**: If a line item lists its amount in a "Pounds" column (rather than a "Quantity" column), map that pounds value into `quantity_raw` verbatim (e.g., "24" => quantity_raw = 24). Never leave `quantity_raw` blank when a Pounds figure is present — the pounds value IS the quantity for that line. (This applies only when the document remains classified as `food_lifeline`; a fully handwritten Donor/Address/Agency/Date slip should already have routed to `grocery_rescue` per the note above.)
