...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - approx_weight comes ONLY from a **printed "Gross Weight" column** on this manifest. If the document instead has a hand-filled per-category **"Pounds" column** (small whole numbers like 14, 16, 21, 24), STOP: this is the handwritten `grocery_rescue` slip, not this supplier — set `supplier = "grocery_rescue"` and follow that prompt. Never copy handwritten Pounds values into this supplier's approx_weight.
  - If a printed line item has no Gross Weight value, leave approx_weight blank; do not infer or transcribe a number from anywhere else.
