...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right, captured verbatim. This is usually formatted "ACR-XXXXXX", but some manifests instead print a source/store-prefixed order code (e.g., "QFC-MI-2026-07-10"). Capture whichever order/invoice identifier is printed on the document — do NOT leave this blank just because it isn't in the "ACR-" format. Only leave null if no order/invoice number appears anywhere on the slip.
