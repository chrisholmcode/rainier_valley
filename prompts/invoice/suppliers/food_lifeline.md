...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - approx_weight comes ONLY from the printed **Gross Weight** column on the AGENCY ORDER manifest. If the document has no printed "Gross Weight" column and instead shows a **handwritten "Pounds" column** (with values written per category/row), this is NOT this supplier — STOP and route to `supplier = "grocery_rescue"`. Do not guess, copy, or leave a placeholder approx_weight from a handwritten Pounds figure on this manifest.
