...
- Category **section headers / grouping labels** (e.g., "Bakery", "Produce", "Canned/Dry Goods", "Frozen") are NOT line items. If a row is only a category label with no associated Description, Unit, Quantity, or Weight, do NOT emit it as a line item and leave `item_name_raw` blank for that row. When a category label IS the full content of the Description cell for a real line, capture it verbatim including any slash and second word — "Canned/Dry Goods" stays "Canned/Dry Goods", not "Canned". Never truncate a multi-word or slash-separated cell to just its first word, and never substitute a neighboring category's label (do not write "Bakery" for a "Canned" row).
