...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - FALLBACK when there is NO distinct "Quantity" column but the line has a Pounds / Gross Weight figure (e.g., 16, 14, 170, 21): populate `quantity` with that pounds value AND set `approx_weight` to the same figure. NEVER leave `quantity` blank when a pounds value is present.
