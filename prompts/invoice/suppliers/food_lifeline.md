...
- If the line has NO printed **Gross Weight** column but instead carries a handwritten **Pounds** value (small integers, e.g., 14, 17, 24, 70, 79), read that handwritten number into `approx_weight` (pounds). Never leave `approx_weight` blank when any pounds/weight figure — printed or handwritten — is legible on the line.
