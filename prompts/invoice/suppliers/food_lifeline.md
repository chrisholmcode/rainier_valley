...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - approx_weight comes ONLY from the rightmost **Gross Weight** column. Do NOT read it from the adjacent Quantity, Cubic Feet, Unit Fee, or Total Fee columns — these sit close together and are easy to confuse.
  - Transcribe the printed Gross Weight number **digit-for-digit exactly as it appears**. Do not transpose digits (e.g., 73 is not 7, 17 is not 117), do not drop or add leading/trailing digits, and do not round. If a digit is ambiguous, prefer the exact glyph over a "rounder" number.
  - Sanity check: read the value once, then re-read it against the printed row before committing. If the number looks physically implausible relative to Quantity (e.g., a single case reporting hundreds of pounds), re-examine which column you read from.
