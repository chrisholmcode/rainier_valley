...
- donor_org: These manifests carry a donor/source code identifying the retail donor (e.g., "SWY-GEN" = Safeway General, "QFC-MI" = QFC Mercer Island). Capture this code VERBATIM into `donor_org`, including its suffix. It typically appears near the Donor/Source field or alongside the item source. Only set `donor_org = null` if no such donor code appears anywhere on the document.
