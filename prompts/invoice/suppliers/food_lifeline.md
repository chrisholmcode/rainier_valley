...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - **Read the printed month literally; do NOT infer or shift it.** These manifests print the Ship Date in `MM/DD/YYYY` (US) order. Transcribe the FIRST number as the month and the SECOND as the day — do not swap them, and do not roll the month forward. A recurring error on this supplier is reading July (07) as August (08); verify the month digit carefully before emitting.
  - Example: a Ship Date printed as `07/07/2026` (or "Jul 07, 2026") => `invoice_date = "2026-07-07"` and `delivery_date = "2026-07-07"`. Do NOT output `2026-08-07`.
