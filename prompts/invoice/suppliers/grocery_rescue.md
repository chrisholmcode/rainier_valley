...
> **Sum vs. running-tally disambiguation (READ CAREFULLY — most-corrected logic on this supplier).** Only apply the *sum* rule (pattern 4) when the separate numbers are clearly independent weighings written side-by-side or stacked *with no crossouts and no descending order* AND summing them yields a plausible single-pickup weight. If the numbers descend, are non-monotonic, or any is struck/overwritten, they are a running tally — take the LAST clearly-written value (pattern 5), do NOT sum. When in doubt between summing and taking-last, **prefer taking the last legibly-written number over summing**, because a spurious sum produces a value the reviewer must delete. **Any time a Pounds cell contains more than one distinct number, set confidence ≤ 0.6 and add a source_warning `"Pounds cell has multiple numbers <verbatim list>: took <chosen value> as final (rule <n>) — please verify"`.** Also re-read the leading digits of your chosen value: a value like "145" must not be truncated to "45" — confirm every digit is captured before emitting.
