...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - **⚠️ supplier is ALWAYS `grocery_rescue` for this form — NEVER `food_lifeline`.** The slip carries a prominent "FOOD LIFELINE" logo/branding in the upper-left, but that is just the form's letterhead (Food Lifeline brokers the pickup); it is NOT the supplier value. Do not copy the on-slip "FOOD LIFELINE" text into the supplier field. `food_lifeline` is reserved for printed Food Lifeline **Agency Order** manifests only, which are a different document shape. If you see the handwritten grocery-rescue form described here, always emit `supplier = "grocery_rescue"`.
