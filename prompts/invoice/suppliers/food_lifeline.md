...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - **Read the Ship Date MONTH literally from the printed characters; do NOT round or shift it.** On these manifests the month is frequently misread as one greater than it should be (e.g., a July "07" date transcribed as "08"). Before emitting, re-check the month digits/token against the slip: if the printed month reads "07" or "Jul", output `07`, not `08`. Never adjust the month to be adjacent to the day value.
