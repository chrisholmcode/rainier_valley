...
- **Row-alignment guard (prevents off-by-one category drift).** The `category` for each line comes STRICTLY from the fixed predefined-rows table above, matched to the printed `item_name_raw` on that same physical row — never inferred from neighboring rows. When reading the Pounds cell for a row, read the weight from the row whose printed label you are emitting, not the row above or below it. Do NOT let your position in the Pounds column drift ahead of or behind the label column. If the form's rows appear shifted, misaligned, or a value seems to belong to an adjacent row, re-anchor by reading the printed row label horizontally across to its own Pounds cell for each of the 10 rows independently, and add a `source_warning`: `"row alignment uncertain — verify category↔weight mapping against slip"`. As a quick self-check, confirm that Frozen Foods→`frozen`, Meat→`meat_protein`, and Dairy/Juice/Alt. Dairy→`dairy` are each paired with their OWN row's weight, not the neighboring row's.
