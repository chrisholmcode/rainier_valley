...
- donor_org: capture the retail-donor source code printed on the manifest verbatim (e.g., "SWY-GEN" = Safeway general, "QFC-MI" = QFC). It typically appears near the donor/source field. Only set `donor_org = null` if no such code appears anywhere on the slip.
