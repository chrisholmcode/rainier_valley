...
- FALLBACK (handwritten Pounds slip): If this is the handwritten Donor / Address / Agency / Date form with a **Pounds** column (which should normally be rerouted to `grocery_rescue`) but it was NOT rerouted, take the handwritten **Pounds** number for each line as that line's `quantity`. Never leave `quantity` blank when a legible Pounds figure is present (e.g., Pounds "16" => quantity = 16).
