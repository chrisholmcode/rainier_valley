...
- donor_org: capture the printed donor-source code verbatim (uppercase, keep the hyphenated suffix), e.g., "SWY-GEN" (Safeway General), "QFC-MI" (QFC). This code identifies the originating donor. Only set `donor_org = null` if no such code appears on the document.
