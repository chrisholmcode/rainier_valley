...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - approx_weight comes ONLY from the printed **Gross Weight** column of the AGENCY ORDER manifest. On a genuine manifest this cell is always printed — do not leave it blank.
  - ROUTING GUARD: If the document has no printed Gross Weight column and instead shows a hand-filled **Pounds** column (typically several small per-category numbers like 24 / 16 / 14), STOP — this is the handwritten grocery-rescue slip, not food_lifeline. Set `supplier = "grocery_rescue"` and follow that prompt. Never derive food_lifeline approx_weight from a handwritten Pounds column.
