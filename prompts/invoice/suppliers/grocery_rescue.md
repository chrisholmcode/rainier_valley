...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is ambiguous in ANY way — could be read as more than one digit (e.g. "7" vs "8", "5" vs "6", "5" vs "8", "0" vs "6/8"), ink smudged, only partially formed, or overwritten — do NOT silently pick one. Emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`. This applies to every month digit, not just the 7/8 case; when a curved/looped single digit could plausibly be another month, always flag rather than commit confidently.
