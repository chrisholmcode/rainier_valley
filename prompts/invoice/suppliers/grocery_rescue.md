...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Transcribe ONLY the literal digits you see in the Pounds cell, exactly as written, preserving all numbers including crossed-out ones and earlier tally values, separated by spaces (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **quantity_raw must contain digits and separators ONLY.** Do NOT add interpretive words or symbols such as "(circled)", "(boxed)", or "/". Those observations belong in `notes` (e.g., notes = "final value 51 is circled"), NOT in quantity_raw. A circled "51" → `quantity_raw = "51"`, `approx_weight = 51`, `notes = "51 circled as final count"`.
    - **Read multi-digit numbers fully — do not drop leading/trailing digits.** If part of the digit run is faint or ambiguous, transcribe your best full reading (e.g., "145", not "45"), lower confidence to ≤ 0.6, and add a source_warning.
    - **Do not invent a value.** Only emit quantity_raw for the cell belonging to THIS row. If the Pounds cell for this row is actually empty, follow the empty-cell rule (all three fields null) — never carry a number over from an adjacent row.
