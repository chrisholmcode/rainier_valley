...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Transcribe the Ship Date digits EXACTLY as printed; do not infer or "correct" the month. The month glyph on these manifests is frequently misread as 08 (August) when it actually reads 07 (July) — zoom in and confirm each digit before emitting. Output a well-formed `YYYY-MM-DD` string with a real two-digit, zero-padded month (01–12); never emit a truncated month such as `0` or `2026-0-07`.
