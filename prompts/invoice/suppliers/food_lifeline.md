...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **If this document has a Pounds column but NO separate printed Quantity column** (e.g., a hand-filled slip that was NOT re-routed to `grocery_rescue`), it is not the printed AGENCY ORDER manifest: the number in the Pounds column IS the line quantity. Set `quantity` to that Pounds value (e.g., a Pounds cell reading "16" => quantity = 16) and `unit = "pound"`. Never leave `quantity` blank when a Pounds value is present.
