...
- Predefined category rows (visible on every form):
  ...
  | Produce | produce | |
- **Row-alignment guard (prevents off-by-one category drift).** The category for a line item is fixed by its ROW LABEL, so the only way to get category wrong is to attach a Pounds value to the wrong row. Before finalizing each row, trace the horizontal gridline from the Pounds cell back to the printed Product/Description label on the SAME line — do not borrow a value from the row above or below. The rows appear in a fixed vertical order (shelf-stable rows → Dairy/Juice/Alt. Dairy → Frozen Foods → Meat → Nonfood → Non-Meat Protein → Prepared/Perishable → Produce); if a handwritten weight sits between two labels, pick the label whose printed baseline the digits actually rest on, not the nearest one visually. If you cannot confidently tell which of two adjacent rows a value belongs to, place it on your best-guess row, lower confidence to ≤ 0.6, and add a source_warning `"row alignment ambiguous between <row A> and <row B> — please verify which row this weight belongs to"`. Never emit a category that does not match the label you actually read for that line.
