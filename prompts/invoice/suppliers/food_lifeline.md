...
- **If the document has no printed "Quantity" column but instead has a "Pounds" column** (e.g., a handwritten grocery-rescue slip that has been routed here), write the numeric Pounds value into `quantity` — do NOT leave `quantity` blank. Also set `unit = "pound"` and `approx_weight` = the same Pounds value. (When the Donor/Address/Agency/Date + Pounds form is present, prefer re-routing to `supplier = "grocery_rescue"` per the note above; but if you remain on this supplier, always populate quantity from Pounds rather than emitting a blank.)
