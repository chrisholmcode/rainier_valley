...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. **When expanding a two-digit year, expand it into the decade of the year in `Today's date` — e.g., with a 2026 upload, "26" → 2026, NOT 2020. Do not anchor on the literal century "20" and drop the written year digits.** If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. **Never invent or default to a static/stale year (e.g. 2020) when the slip has no year — the year MUST be read from `Today's date`; if `Today's date` is absent, leave the date null and let a reviewer fill it in rather than guessing.** Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  - **Year sanity check (mirrors the month check below):** After resolving, compare the year to the year in `Today's date`. A resolved year more than 1 year away from the upload year is almost certainly a misread digit (e.g., "6" read as "0", yielding 2020 instead of 2026) or a dropped year — re-examine the slip, prefer the year consistent with `Today's date`, set confidence ≤ 0.7, and add a `source_warning`: `"date year resolved as <your reading> but differs from upload year — please verify against slip"`. Only keep a year >1 away from the upload year when a full 4-digit year is unambiguously printed on the slip. Because this year flows into the synthesized `invoice_or_order_number` (`<donor_org>-<delivery_date>`), verify the ID's year matches the upload year before emitting it.
  - **Format is always M/D (month first, then day) — never D/M.** If the two numeric parts could be read either way (e.g., "7/8" — both ≤ 12), always treat the **first** as the month and the **second** as the day. Do NOT swap. Example: "7/8" → `2026-07-08`, NOT `2026-08-07`.
  ...
  - Sanity check: if the extracted month differs materially from the month in `Today's date` (the user message), re-examine the source — you have likely transposed month and day. Grocery rescue slips are almost always filled the day of pickup, so the extracted month should match or be adjacent to (±1) the upload month. When in doubt, prefer the reading consistent with `Today's date`.
