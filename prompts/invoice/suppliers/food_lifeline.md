...
- Category/section header rows: Some manifests print grouping bands (e.g., "Bakery", "Canned", "Canned/Dry Goods") that span the columns and have NO Item No., Quantity, or Gross Weight of their own. These are NOT line items — do NOT emit a line and do NOT put them in `item_name_raw`; leave `item_name_raw` blank for any row lacking its own Item No./Quantity. Only when such a phrase is the actual per-row Description of a real line item do you capture it, and then copy it in FULL, verbatim — never truncate (capture "Canned/Dry Goods", never just "Canned").
