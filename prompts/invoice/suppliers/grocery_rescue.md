...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Row-alignment & leading-digit check (do this per row before writing approx_weight):**
    - **Alignment:** Confirm the number you're reading is physically on THIS row's Pounds cell. Handwritten values often drift up/down between ruled lines. Do NOT copy a value read for one row onto an adjacent row. If a Pounds cell has no writing of its own, it is BLANK → `approx_weight = null` even if the row above/below has a number. Never duplicate the same weight onto two rows to avoid a blank.
    - **Leading digit:** Grocery-rescue weights are commonly 3 digits (100–199). Before finalizing a 2-digit reading (e.g., "45", "51"), check for a faint or cramped leading "1" that would make it "145"/"151". If plausible, prefer the 3-digit reading, put both in `quantity_raw`, and lower confidence to ≤ 0.7.
    - It is correct to leave a genuinely empty cell null. The "never leave blank" rule applies only when a numeral is actually legible IN THAT cell — it is not a mandate to invent a value for an empty row.
