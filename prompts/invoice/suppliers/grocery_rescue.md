...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. **Build the `<delivery_date>` portion by copying the already-resolved `delivery_date` string VERBATIM. Do NOT independently re-derive, re-pad, or "correct" any segment of the date when embedding it — in particular, do NOT zero-pad or reformat the month (e.g., if the resolved date carries a single-character month like `0`, keep it as `0`, never rewrite it to `05`). The date inside the ID must match the `delivery_date` field character-for-character.** If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
