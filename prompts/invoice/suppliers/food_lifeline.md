...
- donor_org: Extract the donor/source code printed on the manifest verbatim (e.g., "SWY-GEN" for a Safeway General donation, "QFC-MI" for a QFC donation). These codes identify the originating retail donor. Only set `donor_org = null` if no such donor code is present on the slip.
