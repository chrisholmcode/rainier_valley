...
- invoice_or_order_number: Use the printed order/reference number in the upper-right. This is usually the **Agency Order No** in `ACR-XXXXXX` form, but some Food Lifeline slips instead print a store/PO reference code in the form `QFC-MI-YYYY-MM-DD` (e.g., "QFC-MI-2026-07-10"). Capture whichever code is present verbatim. Never leave this field blank if any such order/reference/PO number is visible on the slip.
