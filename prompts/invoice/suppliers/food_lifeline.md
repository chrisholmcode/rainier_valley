...
- If there is no printed **Gross Weight** column but the slip has a handwritten **Pounds** value for the line (a small integer, e.g., 14, 24, 70), populate `approx_weight` with that number. Never leave `approx_weight` blank when any weight/pounds figure is legible on the slip.
