...
- document_type = "manifest". supplier = "food_lifeline". is_donation = true.
  - donor_org: capture the donor source code printed on the manifest verbatim (uppercase abbreviations like "SWY-GEN" (Safeway) or "QFC-MI" (QFC); look near the line items, funding/source column, or a Donor field). Only set `donor_org = null` if no donor code appears anywhere on the document.
