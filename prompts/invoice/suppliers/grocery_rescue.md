...
- Common fields for every row:
  ...
  - unit = "lb".
  - category = look up the **verbatim row label you actually read on that line** in the "Predefined category rows" table, then use that label's category. **Derive category ONLY from the label text — NEVER from the row's position in the emit order.** The 10 rows are emitted in a fixed order for reviewer convenience, but grocery-rescue slips are hand-filled and rows are sometimes written out of order, shifted, or skipped, so the Nth line on the slip is NOT guaranteed to be the Nth label in the table. Before assigning category, re-read the label on the line and match it exactly (Bakery→shelf_stable, Dairy/Juice/Alt. Dairy→dairy, Frozen Foods→frozen, Meat→meat_protein, etc.). If the label you read and the position disagree, TRUST THE LABEL. If a label is illegible, set category=null, lower confidence to ≤0.6, and add a source_warning `"category unresolved: row label illegible"` — do not guess from position.
