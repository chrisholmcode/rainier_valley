...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). Read the Ship Date digit-by-digit from the printed field — do NOT infer or round the month. The month digits in particular are frequently misread (e.g., a printed "07"/July has been transcribed as "08"/August). Transcribe the exact month shown on the slip; if the month digit is ambiguous, re-examine it rather than guessing an adjacent value.
