...
- **Never leave `quantity` blank if any numeric line quantity is legible.** If the document has a handwritten/printed **Pounds** column instead of a Quantity column (or the Quantity cell is blank but a Pounds value is present), read that Pounds figure into `quantity` and set `unit = "lb"`. Transcribe every digit carefully — handwritten figures are easily misread (e.g., a "9" clipped to "0"); if a trailing digit is ambiguous, re-examine rather than dropping it or defaulting to a round number.
