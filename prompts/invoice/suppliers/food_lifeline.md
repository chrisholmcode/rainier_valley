...
- Category/section header rows are NOT line items. A cell containing only a bare grocery category word or phrase — e.g., "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Dairy", "Meat", "Frozen" — with no specific product name, unit, or per-line quantity is a section header, not a description. Do NOT emit a bare category label as `item_name_raw`; leave `item_name_raw` blank for that row. Only capture the actual product Description text. If a category label legitimately belongs in a field, transcribe it in FULL and verbatim (e.g., "Canned/Dry Goods", never the truncated "Canned"). NOTE: if the document groups quantities under category labels like "Bakery" / "Canned/Dry Goods" with a Pounds column rather than a Description column, this is the handwritten form — set `supplier = "grocery_rescue"` and follow that prompt instead.
