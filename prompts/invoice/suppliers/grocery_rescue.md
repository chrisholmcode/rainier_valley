...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **Emit month and day as exactly two digits each, zero-padded (e.g., "5/28" → `2026-05-28`, not `2026-5-28`). Never drop or alter the month digit; the resolved month must correspond exactly to the first numeric part of the handwritten Date.** Reviewers have repeatedly re-touched the month field on this supplier — double-check that the month you emit is the one actually written on the slip and is correctly two-digit zero-padded before finalizing. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
