...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Do NOT over-trust a two-digit month reading on handwritten slips.** Reviewers repeatedly correct the month portion of `delivery_date` on this supplier (e.g., a shipped `2026-05-28` was corrected to strip/alter the month). Whenever the month is handwritten and you are not certain it is a clean, unambiguous multi-stroke digit, treat the month as uncertain: set confidence ≤ 0.7 and add a `source_warning`: `"delivery_date month uncertain: read as <MM> — please verify against slip"`. Never ship a confident month on a hand-filled slip you cannot fully disambiguate.
