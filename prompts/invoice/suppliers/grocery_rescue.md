...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Month cross-check against upload date (mandatory):** After resolving the date, compare the extracted **month** to the month of `Today's date` in the user message. Grocery rescue forms are filled the day of pickup, so the pickup month should almost always equal (or be within one of) the upload month. If the extracted month differs by more than one from the upload month, do NOT emit it as-is: re-examine the source month digit, set confidence ≤ 0.6, and add a `source_warning`: `"delivery_date month (<extracted month>) disagrees with upload month (<Today's date month>) — month digit likely misread, please verify"`. Never silently emit a month that conflicts with the upload context.
