...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **The date segment MUST be the fully-resolved `delivery_date` verbatim — validate it before embedding.** The embedded date MUST match the exact regex `\d{4}-\d{2}-\d{2}` (four-digit year, TWO-digit month, TWO-digit day). If your resolved date has a single-digit or missing month/day component (e.g. you were about to emit `SWY-RB-2026-0-12`), you have misread a digit — re-read the Date field, zero-pad, and set confidence ≤ 0.7 with a `source_warning` noting the ambiguous digit. Never emit an invoice_or_order_number whose date part is malformed or short.
  - **Guard against day/month transposition in the ID.** Because the ID copies `delivery_date`, a transposed day propagates silently (e.g. reading day "03" as "31" produced the wrong `SWY-GEN-2026-08-31` where the slip meant `SWY-GEN-2026-08-03`). Before finalizing, confirm the DD segment matches the day actually written on the slip (not the month, not a reversed reading). If the day digits are ambiguous or could be read reversed, emit your best reading, lower confidence to ≤ 0.7, and add a `source_warning`: `"invoice_or_order_number date unverified: DD may be transposed — please verify against slip"`.
