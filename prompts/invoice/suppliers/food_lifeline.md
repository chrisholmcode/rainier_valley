...
- Description => item_name_raw verbatim. Copy the ENTIRE Description cell exactly, including slashes and multi-word category labels (e.g., "Canned/Dry Goods" — do NOT truncate to "Canned" at the first word or at the "/").
  - Do NOT create a line item from a bare category/section heading row that has no accompanying item detail (e.g., a standalone "Bakery" grouping label). Leave `item_name_raw` blank / skip such header rows. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
