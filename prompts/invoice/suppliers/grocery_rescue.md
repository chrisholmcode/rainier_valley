...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **Date component must mirror the resolved `delivery_date` exactly — do not independently re-pad or reformat the month/day.** Build the ID by literally concatenating `donor_org`, `-`, and the same `delivery_date` string you emitted; never regenerate the date from scratch. If your read of the handwritten month conflicts with what you'd otherwise zero-pad (e.g., you wrote `SWY-GEN-...-05-...` but the slip's month digit is actually a single ambiguous stroke), trust the slip's actual digit, keep the ID consistent with `delivery_date`, and add a `source_warning` noting the month uncertainty rather than emitting a key whose month segment the reviewer has to correct.
