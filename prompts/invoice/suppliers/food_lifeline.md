...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Read the Ship Date digit-by-digit from the Ship Date field only — do NOT pull from a print/posted date elsewhere on the page. The month digit on these manifests is easily misread: a "7" (July, month `07`) is frequently mistaken for an "8" (August, month `08`). Before emitting, re-check the month glyph specifically. Emit a full, valid `YYYY-MM-DD` with a two-digit month (e.g., `2026-07-07`, never `2026-0-07`).
