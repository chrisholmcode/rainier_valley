...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — the RESOLVED accepted number as a clean numeric string, and it must equal `approx_weight`** (e.g., `approx_weight = 145` → `quantity_raw = "145"`). Do NOT dump every mark in the cell and do NOT add annotations like "(circled)", slashes, or the earlier/crossed-out tally values — those belong in `notes`, not here. When there are multiple readings, apply the Running-tally rule first, then put ONLY the final chosen value here (e.g., a cell showing "50 (circled) / 66" that resolves to 51 → `quantity_raw = "51"`, with the tally described in `notes`). Take care not to drop a leading digit (e.g., read "145", not "45"). Emit `quantity_raw = null` on any row whose Pounds cell is genuinely blank.
