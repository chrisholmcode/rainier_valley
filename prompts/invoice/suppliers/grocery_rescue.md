...
- **Category must match the ACTUAL row label / goods, not an assumed row position.** Before assigning `category`, re-read the printed row label for THAT line and confirm it against the table below. Do not carry a neighboring row's category down or up the list — the rows sit in a fixed perishability order (shelf_stable → dairy → frozen → meat_protein → produce) and it is easy to shift a value onto the adjacent row. If a row label reads as a chilled/frozen/meat item (e.g., "Frozen Foods" → `frozen`, "Meat" → `meat_protein`, "Dairy/Juice/Alt. Dairy" → `dairy`), emit that row's own category — never the category of the row above or below it. If the label you read and the pounds value seem to belong to a different row than the one you're filling, re-align to the correct printed row before emitting.
