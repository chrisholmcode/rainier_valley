...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Year sanity check:** the resolved 4-digit year must be within ±1 of the year in `Today's date` (these forms are filled the day of pickup). When a two-digit year is written, expand it against `Today's date` — e.g., with `Today's date` in 2026, a written "26" resolves to `2026`, NEVER `2020`. If your resolved year ends up several years off from the upload year (e.g., you produced 2020 while `Today's date` is 2026), you have almost certainly misread a year digit — re-read it and prefer the reading consistent with `Today's date`. Never silently emit a year more than ±1 from the upload year; if the slip genuinely appears to show a far-off year, emit your best reading, lower confidence ≤ 0.7, and add a source_warning `"invoice_date year <read> differs from upload year — please verify"`.
