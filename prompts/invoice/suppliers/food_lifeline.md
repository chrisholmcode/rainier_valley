...
- Description => item_name_raw verbatim. **Category/section header rows are NOT line items.** Some manifests print grouping headers in the Description column (e.g., "Bakery", "Canned/Dry Goods", "Produce", "Meat") that have no Item No./Quantity/Weight of their own; skip these entirely — do NOT emit a line and do NOT set `item_name_raw` from them. (Never truncate such labels, e.g., "Canned/Dry Goods" must not become "Canned".) When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
