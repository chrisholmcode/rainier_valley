...
- **Anchor each row to its PRINTED label, not to a counted position.** The 10 labels are pre-printed on the form in the fixed order above. For each line item, read the label text printed on that physical row and copy it verbatim — do NOT infer the label by counting rows from the top, because a blank, hatched, or overwritten row can shift your count and cause every subsequent label to slip one slot (a common error is emitting the label of the row *just above* the intended one, e.g. writing "Frozen Foods" where "Meat" belongs, or "Dairy/Juice/Alt. Dairy" where "Frozen Foods" belongs). Before finalizing, verify each item_name_raw against the printed label physically adjacent to the Pounds value you extracted; if a Pounds value sits between two printed labels or the gridline alignment is ambiguous, pick the label whose printed row the ink clearly falls in, lower confidence to ≤ 0.7, and add a source_warning: `"row label alignment uncertain — verify item_name_raw against slip"`.
