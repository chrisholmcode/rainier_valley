...
- **Rows with a non-empty Pounds cell:**
  ...
  - approx_weight = the final/accepted pounds, parsed per the **Running-tally rule** below. **NEVER leave `approx_weight` blank when any numeral is legible.** If the number is hard to read, extract your best guess, lower `confidence` to ≤ 0.6, and add a `source_warning`. Only set `approx_weight = null` when the cell contains **absolutely no writing**.
    - **Transcribe every digit — do NOT drop a leading digit.** Grocery-rescue weights are commonly 100–200 lb, so a value read as a 2-digit number (e.g., "45", "50") is a red flag that a faint/short leading "1" may have been missed (true value "145", "51→…"). Before finalizing, re-scan the LEFT edge of the cell for a partial leading stroke. If a leading digit is plausible but uncertain, emit your best full reading, set `confidence` ≤ 0.6, and add a `source_warning`: `"Pounds leading digit uncertain: read as <value> but a leading digit may be present — please verify against slip"`.
    - **Do NOT fabricate a value to satisfy the 'always emit' rule.** The "never leave blank when any numeral is legible" instruction applies ONLY when you can actually see writing in THIS row's Pounds cell. If a numeral you're tempted to write is really bleed-through, a stray tally mark, or belongs to an adjacent row, treat the cell as empty (`approx_weight = null`, per the empty-cell rules) rather than emitting a guessed number. When genuinely unsure whether the cell is empty or holds a faint value, emit your best reading, set `confidence` ≤ 0.6, and add a `source_warning` — never silently commit a value or a blank on an ambiguous cell.
