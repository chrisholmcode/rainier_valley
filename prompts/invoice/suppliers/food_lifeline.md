...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - CAREFUL DIGIT READING: The Gross Weight cell is a common misread. Read every digit of the number in the Gross Weight cell for the SAME row as the line item — do not drop a leading digit (e.g., 117 is not 17) and do not add a phantom leading digit (e.g., 7 is not 73). Trace horizontally from the Description across to the Gross Weight column to confirm you are on the correct row before recording the value.
  - SANITY CHECK: approx_weight should be roughly consistent with quantity × a plausible per-case weight for that item. If your read implies an implausible per-case weight, re-read the digits rather than guessing. If the printed value is genuinely ambiguous/illegible, transcribe the digits you can see as-is and do not fabricate additional digits.
