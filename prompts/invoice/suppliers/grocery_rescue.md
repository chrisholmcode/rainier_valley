...
- **Rows with a non-empty Pounds cell:**
  ...
  > ⚠️ **Row alignment — extract each Pounds value against ITS OWN row; never carry or repeat a value across rows.** These forms are densely filled and column mis-registration is the #1 error on this supplier. Trace the horizontal gridline from each row label to the Pounds cell that sits on that exact line. A weight belongs to a row ONLY if writing physically sits on that row's line. If the same number appears to apply to two adjacent rows, that is almost always mis-registration: assign it to exactly one row and leave the other blank (`approx_weight=null`, `notes="no value on form"`) rather than duplicating it. Do NOT fill a blank row with a neighbor's number just to satisfy the "emit all rows" rule — an empty row must stay empty.
  > ⚠️ **Leading-digit / multi-digit re-read check.** Before finalizing any 2- or 3-digit weight, re-scan the left edge of the cell for a leading digit you may have clipped (e.g., "145" misread as "45", "156" as "56"). Grocery-rescue pallet weights are usually 3 digits or high 2 digits; if you read a suspiciously small number (e.g., under ~50) in a cell that looks full-width, re-examine for a dropped leading "1", lower `confidence` to ≤ 0.7, and add a `source_warning` noting the possible dropped digit.
