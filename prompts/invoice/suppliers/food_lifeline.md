...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - FALLBACK: If there is no printed "Quantity" column but the line carries a **Pounds** value (this happens on handwritten grocery-rescue slips that were routed here), NEVER leave `quantity` blank — set `quantity` to that Pounds number (e.g., a "Pounds" figure of 16 => quantity = 16) and set `unit = "lb"`. (First reconsider whether the document is actually a handwritten Donor/Address/Agency/Date slip that belongs to `grocery_rescue`; if so, reroute. But if it is processed under this supplier, capture the Pounds value into quantity.)
