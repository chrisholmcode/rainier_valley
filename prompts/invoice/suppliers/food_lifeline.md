...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Read ONLY the field literally labeled **Ship Date**. Do NOT use any other nearby date (e.g., a Print Date, Order Date, Due Date, or footer/print timestamp) — these are often one month later than the Ship Date and must be ignored.
  - Transcribe the month EXACTLY as printed; do not shift or round it forward. If the Ship Date reads month 07 (July), the output month MUST be 07, not 08. Re-read the printed month digit-for-digit before emitting the date.
