...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - WEIGHT FALLBACK: If there is no "Gross Weight" column but the document has a "Pounds" column (including a handwritten one), read the numeric pound value from that column into `approx_weight`. Never leave `approx_weight` blank when any weight/pounds value is legible — extract the number as written (e.g., a handwritten "14" => approx_weight = 14).
