...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - approx_weight fallback: if there is NO printed "Gross Weight" column (e.g., the weight appears only as a handwritten number in a "Pounds" column or written next to the line/category), read that handwritten pounds figure into `approx_weight`. Never leave `approx_weight` blank when any legible weight number is present on the slip. These are typically small whole-number pounds (e.g., 14, 17, 24, 70, 79).
