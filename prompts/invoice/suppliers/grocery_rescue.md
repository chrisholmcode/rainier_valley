...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). The `<delivery_date>` portion MUST be a byte-for-byte copy of the exact string you placed in the `delivery_date` field — build the ID by string-concatenating `donor_org` + `-` + the resolved `delivery_date` value. Do NOT re-read the slip, re-derive, or re-pad the month/day when building the ID; if the month digit in the ID differs from the month digit in `delivery_date`, that is a bug — re-copy from `delivery_date`. This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
