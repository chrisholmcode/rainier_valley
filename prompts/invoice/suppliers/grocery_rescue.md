...
- **Row-alignment check (prevents off-by-one label/value mismatches).** The 10 predefined rows are fixed and always appear in the printed order shown in the table above. For each Pounds value you read, confirm it sits on the SAME horizontal row as the printed row label you are pairing it with — trace the value straight left to its printed label, do not associate it with the row above or below. Common failure: emitting a value under `Dairy/Juice/Alt. Dairy` that actually belongs to `Frozen Foods`, or `Frozen Foods`↔`Meat`, or `Coffee Kiosk`↔`Dairy/Juice/Alt. Dairy` — these adjacent rows get shifted when handwriting crowds cell borders. Because `item_name_raw`/`item_name_normalized` come from the FIXED printed order, if your extracted value seems to land on the wrong category, re-check the row alignment rather than changing the label. If two rows still look ambiguous after re-checking, lower confidence to ≤ 0.7 and add a `source_warning`: `"row alignment uncertain near <row label> — please verify value is on the correct row"`.
