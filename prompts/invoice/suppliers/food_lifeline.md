...
- **Quantity fallback for Pounds-column slips:** If this document has no printed `Quantity` column but instead shows a `Pounds` figure per line (e.g., a handwritten Pounds entry like 16, 14, 170, 21), populate `quantity` with that Pounds number and set `unit = "lb"`. NEVER leave `quantity` blank when a Pounds value is present. (Note: the handwritten Donor/Address/Agency/Date Pounds form should normally be routed to `grocery_rescue` — but if it is being processed here, still fill `quantity` from Pounds rather than emitting blank.)
