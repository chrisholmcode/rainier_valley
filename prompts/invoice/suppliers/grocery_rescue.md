...
- **Rows with a non-empty Pounds cell:**
  ...
  - approx_weight = the final/accepted pounds, parsed per the **Running-tally rule** below. **NEVER leave `approx_weight` blank when any numeral is legible.** If the number is hard to read, extract your best guess, lower `confidence` to ≤ 0.6, and add a `source_warning`. Only set `approx_weight = null` when the cell contains **absolutely no writing**.
    - **Verify digit count, not just digit value.** Before committing, count the digits you see and ask whether a leading digit could be lost to a faint stroke or a mistaken word-space (e.g., "145" mis-read as "45", "156" as "56"). If a leading "1" or other digit is plausibly present, prefer the higher-digit reading, put BOTH readings in `quantity_raw`, and add a `source_warning`: `"approx_weight digit-count uncertain: read as <value> but leading digit may be missing — verify against slip"`.
    - **Do not let a value bleed across rows.** A number physically written in one row's Pounds cell belongs ONLY to that row. If the same number seems to appear on an adjacent row, or you are inferring a value from a neighboring cell rather than reading ink inside THIS row's cell, treat this row as blank (approx_weight = null) rather than duplicating the neighbor's value.
    - **When genuinely torn between "blank" and "a value," do not silently commit either way.** If a row's Pounds cell has faint/ambiguous marks, emit your best reading with `confidence ≤ 0.6` and a `source_warning`: `"approx_weight uncertain: cell may be blank or may read <value> — verify against slip"`. Reserve `approx_weight = null` for cells with truly no ink.
