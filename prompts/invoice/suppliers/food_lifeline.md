...
- Quantity fallback: if a line item has NO printed Quantity column but DOES have a Pounds column (or a per-line pounds/weight number is the only quantity present), populate `quantity` with that pounds value and set `unit = "lb"`. Never leave `quantity` blank when a Pounds/weight number is present for the line.
