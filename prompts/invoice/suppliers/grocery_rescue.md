...
- **Row-label alignment (avoid off-by-one shifts).** The 10 predefined rows appear in a FIXED printed order and each has a FIXED label — never re-derive a row's label from the value written next to it. Read each Pounds value on the SAME horizontal line as its printed label; trace left along the row gridline from the number to the pre-printed Product/Description label before assigning it. If rows are unevenly spaced, some are blank, or handwriting drifts between lines, do NOT slide values up or down to the nearest row. Concretely: a weight belongs to `Dairy/Juice/Alt. Dairy` only if it sits on the Dairy line — not the Frozen line above or below it. If you cannot confidently match a number to its own row label, lower confidence to ≤ 0.6 and add a source_warning `"row/value alignment uncertain — verify Pounds values map to correct category rows"`. `item_name_raw`/`item_name_normalized` must always be the fixed label for that row position, never inferred from neighboring content.
