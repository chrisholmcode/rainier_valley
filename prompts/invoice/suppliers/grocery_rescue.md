...
- **donor_org**: Read the **Donor** field at the top. Grocery rescue only picks up from **5 fixed locations**. Map whatever is written on the slip to one of these exact short codes — do not invent new ones.
  ...
  - **⚠️ Two different Safeway stores — do NOT default to `SWY-GEN`.** `SWY-RB` (Safeway **Rainier Beach**) and `SWY-GEN` (Safeway **Genesee**) are distinct locations that differ ONLY by the store suffix after `SWY-`/`Safeway-`. Read that suffix explicitly before mapping: `RB`/`Rainier`/`Rainier Beach` → `SWY-RB`; `G`/`GEN`/`Genesee` → `SWY-GEN`. Never assume a generic "Safeway" or a faint suffix means Genesee. If the Safeway suffix is smudged, partial, or you cannot clearly read `RB` vs `GEN`, set donor_org=null, lower confidence, and add a source_warning `"Safeway store suffix unclear: read as <your reading> — RB (Rainier Beach) vs GEN (Genesee), please verify"`. This misread flows straight into `invoice_or_order_number`, so getting the suffix right matters.
