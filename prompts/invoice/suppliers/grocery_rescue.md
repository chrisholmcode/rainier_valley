...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is ambiguous (could be "7" or "8", **or "6" vs "8"** — a very common confusion on these slips — ink smudged, only partially legible), do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`.
  - **Handwritten "8" is frequently mis-read as "6" on this supplier's forms.** If your reading gives month "06" (or any month EARLIER than the upload month in `Today's date`), re-examine the digit before committing — a curved/looped top often means it is actually "8". These forms are filled the day of pickup, so the invoice_date month should match (or be the same month as) the upload date; a resolved month that lands *before* the upload month is a strong signal you read an "8" as a "6". Prefer the later reading when the stroke supports it.
  - Sanity check: if the extracted month lands EARLIER than the month suggested by the upload context (`Today's date`) — e.g., you read "06" but the upload is in August — re-examine the source. You have likely either transposed month and day or mis-read an "8" as a "6". When in doubt, prefer the reading consistent with (or later than) the upload date.
