...
- donor_org = the retail donor source code printed on the slip, captured verbatim and uppercased (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC). It is a hyphenated code: a store/chain prefix (SWY, QFC, ...) plus a program/region suffix. Only set donor_org = null if no such donor code appears on the document.
