...
- **donor_org**: Read the **Donor** field at the top. Grocery rescue only picks up from **5 fixed locations**. Map whatever is written on the slip to one of these exact short codes — do not invent new ones.
  ...
  - **Safeway disambiguation (SWY-RB vs SWY-GEN) — do NOT guess, and never default to `SWY-GEN`.** These are the two most-confused donor codes. There are exactly two Safeway locations: `SWY-RB` = **Rainier Beach**; `SWY-GEN` = **Genesee**. `SWY-GEN` is NOT a generic "Safeway" fallback — it means *Genesee specifically*. Match ONLY on an explicit, legible suffix: `RB` / `Rainier` / `Rainier Beach` → `SWY-RB`; `GEN` / `G` / `Genesee` → `SWY-GEN`. A bare `SWY`, a bare `Safeway`, or an illegible/ambiguous suffix is NOT enough to pick either one — do NOT default to Genesee (or Rainier Beach). In that case set donor_org=null, lower confidence, and add source_warning `"Safeway store suffix ambiguous: <verbatim value> — cannot distinguish RB vs GEN"` so a reviewer can pick the correct Safeway. Because `invoice_or_order_number` is synthesized as `<donor_org>-<delivery_date>`, leaving donor_org null here also correctly leaves the shipment ID null for review.
