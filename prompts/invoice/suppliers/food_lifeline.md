...
- If the document has a **Pounds** column (whether printed or hand-filled) and NO separate "Quantity" column, populate `quantity` from the Pounds value (numeric only, e.g., "170 lbs" => 170). Do NOT leave `quantity` blank because the header says "Pounds" instead of "Quantity" — the pound figure IS the quantity for this line. Also set `unit = "lb"` and `approx_weight` to the same value in that case.
