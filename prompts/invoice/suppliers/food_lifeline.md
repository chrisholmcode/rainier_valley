...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - The Ship Date is printed as MM/DD/YYYY. Transcribe it digit-by-digit exactly as printed — do not infer, adjust, or round the month or day. Re-read the month digits before committing; a printed "07" is July (`07`), NOT `08`.
  - Emit a strict, zero-padded `YYYY-MM-DD` where the month is between `01` and `12` and the day between `01` and `31`. Never output a single-digit or empty month (e.g., `2026-0-07` or `2026-8-07` are invalid).
