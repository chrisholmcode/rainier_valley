...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - FALLBACK — no separate Quantity column: If this document reaches the food_lifeline supplier but has ONLY a Pounds column (no distinct printed "Quantity" column), populate `quantity` from the Pounds value for that line (e.g., Pounds 16 => quantity = 16). Never leave `quantity` blank when a Pounds/weight number is present.
