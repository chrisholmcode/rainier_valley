...
- Common fields for every row:
  ...
  - category = per the table above.
    - **Anchor category to the printed row label, NOT to list position.** Before assigning `category` for any row that carries a Pounds value, re-read the Product/Description text printed on that exact line and match it to the label table. Do not assume the Nth value from the top belongs to the Nth predefined row — rows can be skewed, and a weight written between two printed lines is a common source of off-by-one errors. When two adjacent rows are both plausible (e.g., Dairy vs. Frozen Foods, Frozen Foods vs. Meat, or the Bakery/Canned/Coffee shelf_stable block vs. Dairy below it), trace the horizontal gridline from the Pounds cell back to its printed label to confirm which row it belongs to. If the alignment is ambiguous, lower confidence to ≤ 0.7 and add a `source_warning`: `"row/category alignment uncertain: <value> may belong to <label A> or adjacent <label B> — please verify against slip"`.
