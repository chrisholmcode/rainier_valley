...
- If the document has a **Pounds** column instead of a printed Quantity column (e.g., the handwritten Donor/Address/Agency/Date slip), put the numeric value from the **Pounds** column into `quantity` and set `unit = "pounds"`. Never leave `quantity` blank when a Pounds figure is written for the line — e.g., a line reading "16" in the Pounds column => quantity = 16.
