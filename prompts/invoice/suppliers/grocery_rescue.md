...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to strict ISO-8601 `YYYY-MM-DD` with EXACTLY four year digits, two month digits, and two day digits (zero-padded). If only two digits are given for the year, assume 20YY (yielding a full 4-digit year). Never emit a 3-digit year — e.g., `202-08-06` is invalid and must be `2020-08-06`. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
