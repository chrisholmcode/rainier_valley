...
- FALLBACK: If the document has a **Pounds** column instead of a printed Quantity column (i.e., a handwritten Pounds slip), this is `grocery_rescue` — reroute per that prompt. If for any reason it is processed here, the **Pounds** value IS the line quantity: set `quantity` to that number (e.g., a slip showing 16 lbs => quantity = 16). NEVER leave `quantity` blank when a numeric Pounds value is present.
