...
- donor_org: capture the **donor/source code** shown on the manifest verbatim (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC). This appears as a Donor/Source field (often near the header or per-line source column). Only set `donor_org = null` if no donor/source code is present anywhere on the document.
