...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** This is a PURE VERBATIM transcription of the ink in the Pounds cell — digits and separating spaces ONLY. Copy every visible number exactly, preserving crossed-out ones and earlier tally values (e.g., "70 79" → `quantity_raw = "70 79"`; a clean "24" → `quantity_raw = "24"`).
    - **Transcribe each number in FULL — never drop a leading digit.** If the cell reads "145", quantity_raw is `"145"`, not `"45"`. Multi-digit numbers must keep every digit.
    - **NO interpretation, annotations, or symbols.** Do NOT write "(circled)", "(boxed)", slashes, "or", or alternate readings into quantity_raw. Which number is final, and any alternate/uncertain reading, belongs in `notes` — NOT here. (e.g., a cell with a circled "51" → `quantity_raw = "51"`, and note "51 circled = final".)
    - If the cell is genuinely empty or the row label is struck through, quantity_raw = null — do not invent a value.
