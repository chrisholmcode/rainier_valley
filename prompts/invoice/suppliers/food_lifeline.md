...
- Description => item_name_raw verbatim. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
  - `item_name_raw` must come from an actual line-item Description, NOT from a category section header/grouping label (e.g., "Bakery", "Canned", "Canned/Dry Goods", "Produce"). Do NOT emit a category heading as an item name. If a category label has no line item under it (no Description text), leave `item_name_raw` blank for that row.
  - When you do capture a Description, transcribe the ENTIRE printed label verbatim — do not truncate it. If the printed text reads "Canned/Dry Goods", capture "Canned/Dry Goods", not "Canned".
