...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell. Emit ONLY the literal digits you read — never add prose annotations.** Do NOT write words like "(circled)", "(boxed)", "(crossed out)", and do NOT use separators like "X / Y" to show alternative readings. Any circling/crossing-out information belongs in `notes`, never in `quantity_raw`.
    - When the cell contains ONE final intended value (a lone number, or a circled/boxed number, or a clean number with earlier values crossed out), `quantity_raw` = that single number's digits exactly (e.g., a cell showing "50 66" with 51 circled → `quantity_raw = "51"`; a clean "24" → `quantity_raw = "24"`). Preserve multiple distinct numbers ONLY for genuine running tallies with no circle/crossout that resolve them (pattern 5), e.g. "151 123 108 40".
    - **Read every digit — do not drop a leading digit.** Before writing, count the digits: grocery-rescue pounds are commonly 3 digits (100–200+). If a value looks like a 2-digit number but the cell width or ink suggests a leading digit, re-examine (a "45" is often "145"). When unsure of digit count, lower confidence to ≤ 0.6 and add a source_warning.
    - **Row alignment:** only transcribe a number if it sits clearly inside THIS row's Pounds cell. If a number appears to float between rows or belongs to an adjacent/absent row, do not attach it here — leave this row blank per the empty-cell rules and add a source_warning about ambiguous row alignment.
