...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible **number** from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **Digits and separators ONLY — no annotation words.** Never put descriptive text such as "(circled)", "(boxed)", "crossed out", or an editorial "/" into `quantity_raw`. Those observations belong in `notes`. E.g., a cell showing a circled 50 above a 66 → `quantity_raw = "50 66"` (NOT `"50 (circled) / 66"`); put "50 circled, 66 below" in `notes`.
    - **Preserve every digit of each number — never drop a leading digit.** If you read a number, transcribe all of its digits (e.g., "145" → `"145"`, never `"45"`). If you are unsure whether a leading mark is a digit, report both readings in `quantity_raw` and lower confidence per the digit-boundary caution.
    - **Consistency with approx_weight:** the accepted value you chose for `approx_weight` must appear verbatim among the numbers in `quantity_raw`. If, after review of the marks, you conclude the cell holds no real weight (only a stray/annotation mark, not a legible number), set this row to the blank-cell treatment (all three null) rather than emitting the mark.
