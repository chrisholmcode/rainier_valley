...
- If a line's item label is a printed category bucket (e.g., "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Meat/Protein"), copy the ENTIRE label into `item_name_raw` VERBATIM — never abbreviate or split a multi-word/slash label (write "Canned/Dry Goods", not "Canned"; write "Canned", not "Bakery"). If that category row has no quantity/weight entry (an empty bucket), leave `item_name_raw` blank rather than inventing or carrying over a neighboring category name. (If the whole document is the handwritten Donor/Address/Agency/Date Pounds form, it is `grocery_rescue`, not this supplier — see "Not this supplier".)
