...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If there is no "ACR-XXXXXX" agency order number, capture whatever order/reference/source number IS printed on the slip instead — including store- or donor-coded identifiers of the form `STORE-XX-YYYY-MM-DD` (e.g., "QFC-MI-2026-07-10"). Never leave `invoice_or_order_number` blank when any such reference number is present; only leave it null if no order/reference number appears anywhere on the document.
