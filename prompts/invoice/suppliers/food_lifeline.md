...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - CAREFUL DIGIT TRANSCRIPTION: This layout has several adjacent numeric columns (Quantity, Cubic Feet, Unit Fee, Total Fee, Gross Weight). `approx_weight` comes ONLY from the rightmost **Gross Weight** column — do not pull it from Quantity or any fee column, and do not transpose values between line items. Transcribe the Gross Weight value digit-for-digit exactly as printed: preserve every digit (e.g., "73" is 73, not 7; "117" is 117, not 17), and do not add a leading digit that is not printed (e.g., "7" is 7, not 12). Re-read the cell before finalizing and confirm each digit is aligned to the Gross Weight header.
