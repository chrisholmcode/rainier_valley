...
- document_type = "manifest". **supplier = "grocery_rescue" — ALWAYS, no exceptions.** is_donation = true.
  - ⚠️ **Do NOT set supplier to "food_lifeline".** These slips display a "FOOD LIFELINE" logo upper-left, but that is only the brokerage's branding on the form — it is NOT the supplier value. For every slip routed to this file, `supplier` is the literal constant `"grocery_rescue"`, regardless of any "Food Lifeline" text or logo printed on the page. `food_lifeline` is a separate bucket reserved for printed Agency Order manifests and must never be emitted here.
