...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **`quantity_raw` is a LITERAL transcription — NOT your interpretation.** Do the transcription FIRST, before any running-tally reasoning. Then apply the tally rule only to `approx_weight`/`quantity`, leaving `quantity_raw` untouched.
      - **Never add annotations or interpretive words to `quantity_raw`.** Do NOT write "(circled)", "(boxed)", "circled", "/", or which value you chose. Those observations belong in `notes` only. E.g., a cell where "66" is crossed out and "51" is circled → `quantity_raw = "66 51"` (raw digits, in the order/layout seen), NOT `"51 (circled)"` and NOT `"50 (circled) / 66"`. Put "51 circled as final; 66 crossed out" in `notes`.
      - **Transcribe every digit of each number — do not truncate.** Read multi-digit numbers whole before deciding anything: "145" is `"145"`, never `"45"`. If a leading digit is faint, still transcribe it and lower confidence rather than dropping it.
      - If a numeral is legible, `quantity_raw` must reflect it — never leave it blank when digits are present (e.g., a legible "56" → `quantity_raw = "56"`).
