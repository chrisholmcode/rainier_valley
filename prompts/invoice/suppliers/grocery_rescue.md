...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Month must be a valid two-digit value 01–12.** Never emit a malformed date (e.g., `2026-0-28`, `2026-00-28`, or a single-digit month). Always zero-pad. If the month is smudged, partially cut off, or reads as "0"/blank so you cannot confidently pick a value in 01–12, do NOT invent one: set `delivery_date = null` and `invoice_date = null`, lower confidence to ≤ 0.6, and add a `source_warning`: `"delivery_date month illegible/invalid — reviewer must supply month; day appears to be <DD>"`. This routes the slip to a reviewer instead of committing a guessed month.
