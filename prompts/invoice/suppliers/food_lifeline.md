...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Read the Ship Date's MONTH digit carefully — on these manifests a "7" (July) is frequently misread as "8" (August). Transcribe the month exactly as printed; do not round or infer. If the month glyph is ambiguous between 7 and 8, prefer 07 and note the ambiguity in `notes`. Always emit a valid zero-padded two-digit month (01–12) — never a single digit like "0" or "8" without verifying it matches the printed date.
