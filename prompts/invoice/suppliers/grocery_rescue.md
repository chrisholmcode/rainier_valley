...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Validity + well-formedness (hard rule):** the resolved date MUST be a real calendar date formatted `YYYY-MM-DD` with a two-digit month `01`–`12` and a two-digit day `01`–`31`. NEVER emit a value with an empty, zero, or single-character month or day (e.g. `2026-0-12` is invalid). If any part is illegible, emit your best full zero-padded reading and lower confidence — do not emit a malformed value. If you truly cannot form a valid date, set delivery_date=null, lower confidence, and add a source_warning `"delivery_date illegible: <verbatim value>"`.
  - **Single-digit day legibility warning:** handwritten days are often written without a leading zero (a bare "3" means the 3rd → `03`). Do NOT pad or inflate a lone "3" up to "31" or otherwise turn one digit into two. **Dates have NO "prefer the larger value" rule** — unlike the Pounds column, the larger reading is never more plausible for a date. If the day digit(s) are ambiguous, emit your best reading, set confidence ≤ 0.7, and add a source_warning `"delivery_date day digit unclear: read as <your reading> — please verify against slip"`.
