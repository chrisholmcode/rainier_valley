...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Format strictly as zero-padded `YYYY-MM-DD`: the month MUST be a two-digit value between `01` and `12`, and the day a two-digit value `01`–`31`. Never emit a single-digit or `0` month (e.g., `2026-0-07` is invalid) — re-read the month glyph if the parse yields anything outside `01`–`12`.
  - CAUTION: on these printed manifests the month digit is frequently misread — do not assume `08`; a printed `07` (July) has been repeatedly mistranscribed as `08`. Read the month digit carefully and prefer the exact printed value.
