...
- If a line has NO printed Quantity but shows a **handwritten Pounds** figure (common when a grocery-rescue-style Pounds column is present on this doc), populate `quantity` with that pounds number — never leave `quantity` blank when a legible pounds value exists. Transcribe every digit and re-check the final digit of handwritten numbers (e.g. read "79", not "70").
