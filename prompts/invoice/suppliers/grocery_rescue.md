...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-M-D. **Supplier-specific format note:** reviewers on this supplier do NOT zero-pad the month (and this has been the single most-corrected date issue — 70 corrections). Emit the month as written on the slip WITHOUT a leading zero for single-digit months. Example: a slip dated May 28, 2026 → `2026-5-28`, NOT `2026-05-28`. (Zero-pad the day only if that is how it appears; when in doubt, match the reviewer convention of an unpadded month.) If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
