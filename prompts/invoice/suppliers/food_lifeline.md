...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). approx_weight comes ONLY from the machine-PRINTED "Gross Weight" column of this manifest. If the weights appear in a HANDWRITTEN "Pounds" column instead of a printed "Gross Weight" column, STOP — this is not a Food Lifeline AGENCY ORDER; re-route to `supplier = "grocery_rescue"` and use that prompt. Never hand-transcribe a Pounds column into approx_weight under this supplier: if you cannot find a printed Gross Weight value, leave approx_weight blank rather than guessing.
