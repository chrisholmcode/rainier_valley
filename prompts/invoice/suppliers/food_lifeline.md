...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). The Ship Date is printed as **M/D/YYYY** (US month-first). Transcribe the printed month EXACTLY as shown — do NOT round up, offset, or shift the month. Read the month digit(s) before the first slash as-is (e.g., a Ship Date printed "7/7/2026" is `2026-07-07`, NOT `2026-08-07`). If the month is unclear, prefer the literal printed digit over any inferred value.
