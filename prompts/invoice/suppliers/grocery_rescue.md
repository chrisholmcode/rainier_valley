...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<YYYY>-<M>-<D>`, where `<M>` and `<D>` are the month and day of the delivery date **with leading zeros stripped** (single-digit where applicable) — NOT the zero-padded ISO form. Example: delivery_date `2026-05-28` → `SWY-GEN-2026-5-28`; delivery_date `2026-07-01` → `QFC-MI-2026-7-1`. (Note: `delivery_date` and `invoice_date` themselves stay in full padded `YYYY-MM-DD` form; only this ID uses the un-padded month/day.) This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
