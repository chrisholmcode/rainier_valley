...
> Each tally entry is a single integer. Before classifying the pattern, explicitly list every discrete number you see in the cell — separated by spaces, line breaks, or crossouts. Treat a contiguous run of digits (no space or line break between them) as ONE number. **Do NOT split a multi-digit number such as "117" into "1" and "17", and do NOT merge two separate numbers such as "1" and "17" into "117".** If you're unsure whether a gap between digits is a word-space or handwriting variation, report both interpretations in `quantity_raw` and lower confidence to 0.6. **Do NOT default to the larger value** — over-reading is the most common error on these slips (e.g., reading a lone "2" as "21", or summing/merging a running count into a too-large number). Prefer the reading of the discrete numbers actually present; when two readings remain equally plausible, keep confidence ≤ 0.6 and add a `source_warning` rather than silently picking the larger one.
> **Read only this row's Pounds cell.** Do NOT borrow a digit from the adjacent Drop Off Temp column or from a neighboring row, and do NOT fuse a stray tick or overwrite stroke onto a clean number. This cross-column/stray-stroke bleed is what produces spurious values like "21" from a real "2".
> **A handwritten `0` (zero) is a REAL value, not an empty cell.** If any mark that could be a digit — including a lone `0` — is present in the Pounds cell, the row is NOT empty: set `quantity_raw` to what you read (e.g., "0"), set `approx_weight = 0` and `quantity = 0`, and do NOT route the row into the empty-cell branch. Only a cell with *no writing whatsoever* qualifies as empty. Whenever any numeral is legible, `quantity_raw`, `approx_weight`, and `quantity` must all be non-null (with `quantity` = `approx_weight`) — never leave them blank because a digit was hard to read.
