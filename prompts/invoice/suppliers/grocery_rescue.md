...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is ambiguous in ANY way (e.g. could be "7" or "8", ink smudged, overwritten, partially formed, or a glyph you cannot confidently resolve to a specific digit), do NOT silently pick a clean value — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`. Never upgrade an uncertain month to a confident value (e.g. do not commit to "05" when the month glyph is smudged/partial) — a smudged or partial month is exactly the case a reviewer must double-check, so flag it rather than resolving it silently.
