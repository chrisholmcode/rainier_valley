...
- **Row-alignment check (prevents off-by-one label drift):** The 10 rows above are a fixed *template order*, but you MUST still confirm that each `item_name_normalized` you emit matches the row you are actually reading on the slip — do NOT let the emitted label drift by one row relative to the source (a common error is labeling the Frozen Foods row as "Dairy/Juice/Alt. Dairy", the Meat row as "Frozen Foods", or the Dairy row as "Coffee Kiosk"). Before finalizing, re-scan top-to-bottom and confirm each emitted label sits on the same horizontal line as the pounds value you extracted for it. Anchor each label on the printed row text visible on THAT physical line rather than assuming template position.
