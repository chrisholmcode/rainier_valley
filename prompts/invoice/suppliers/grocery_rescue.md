...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **Emit a strictly valid, fully zero-padded date: exactly four year digits, a two-digit month (01–12), and a two-digit day (01–31), each separated by a single hyphen (e.g., `2026-05-28`).** Before emitting, self-validate: the month component MUST be two digits in the range 01–12 and the day MUST be two digits in the range 01–31 — never emit a single-digit or out-of-range month/day (e.g., never `2026-0-28` or `2026-5-28`). If your resolved value cannot be expressed as such a valid date, treat the date as unresolved (set delivery_date=null, lower confidence, add a source_warning) rather than emitting a malformed string. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
