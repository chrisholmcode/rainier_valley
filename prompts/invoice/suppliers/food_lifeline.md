...
- **Pounds column => quantity_raw.** If the line/slip carries a **Pounds** value (typed or handwritten), copy that number verbatim into `quantity_raw` — never leave `quantity_raw` blank when a Pounds figure is legible. On the printed manifest the Quantity column normally feeds `quantity`; the Pounds/weight figure still feeds `quantity_raw`. Only reroute the whole document to `grocery_rescue` if it is unmistakably the handwritten Donor / Address / Agency / Date form — otherwise still populate `quantity_raw` from the Pounds value.
