...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - **`supplier` is ALWAYS the literal string `grocery_rescue` for every slip in this bucket — NEVER `food_lifeline`.** The "FOOD LIFELINE" logo printed on the form is the broker's letterhead, NOT the supplier value. Do not copy the on-slip brand name into `supplier`. `food_lifeline` is a separate supplier reserved for printed Agency Order manifests and must never be emitted here.
