...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - Transcribe the Gross Weight EXACTLY as printed, digit by digit. Do not add, drop, or transpose digits and do not "round to a plausible number". Preserve the exact digit count (e.g., a printed "117" stays 117, never 17; a printed "17" stays 17, never 117). If a digit is genuinely unreadable, keep the digit count you can see and do not invent extra digits. Sanity-check against Cubic Feet and Quantity: a single case of a dense item is typically tens of pounds, so a two- vs three-digit reading should be consistent with the line's quantity and cubic feet.
