...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - **supplier is ALWAYS the literal string `"grocery_rescue"` for every slip in this bucket — never `"food_lifeline"`.** The "FOOD LIFELINE" logo printed on the form is the broker's branding, NOT the supplier value. Do not copy the logo, header, or any printed brand name into the `supplier` field. `food_lifeline` is a separate bucket reserved for printed Agency Order manifests and must never be emitted here.
