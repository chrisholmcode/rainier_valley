...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "ACR-XXXXXX" Agency Order No is present, look for a store/pickup identifier in the format `QFC-XX-YYYY-MM-DD` (e.g., "QFC-MI-2026-07-10") — capture it verbatim as `invoice_or_order_number`. Never leave this field blank when either identifier is visible.
