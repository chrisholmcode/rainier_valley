...
- **⚠️ supplier is ALWAYS the literal string `grocery_rescue` — NEVER `food_lifeline`.** The printed "FOOD LIFELINE" logo on this form is the *broker's* letterhead, NOT the supplier value. Do not copy the logo text into the `supplier` field. `food_lifeline` is reserved exclusively for printed Food Lifeline **Agency Order** manifests; a hand-filled grocery rescue donation form is ALWAYS `grocery_rescue`, regardless of the logo shown.
