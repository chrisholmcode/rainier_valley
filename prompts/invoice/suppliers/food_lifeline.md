...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **approx_weight comes ONLY from the printed "Gross Weight" column.** If the document has no printed "Gross Weight" column but instead a handwritten "Pounds" column (per-category totals on a hand-filled slip), this is NOT this supplier: STOP, set `supplier = "grocery_rescue"`, and follow that prompt. Never map a handwritten "Pounds" value into `approx_weight` under `food_lifeline`, and never leave `approx_weight` blank on a genuine manifest line that has a Gross Weight value.
