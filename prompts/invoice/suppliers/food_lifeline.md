...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "Agency Order No" label is present, fall back to any store/reference order code printed on the slip — including a store-prefix + date form such as `QFC-MI-2026-07-10` (store code, sub-code, then YYYY-MM-DD). Capture it verbatim. Never leave this field blank when such a reference code is visible.
