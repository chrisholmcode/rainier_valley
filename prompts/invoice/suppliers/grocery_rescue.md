...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  - **Month must be a valid, zero-padded value between `01` and `12`, and the day between `01` and `31`.** Never emit a malformed month (e.g., `0`, a single unpadded digit, or a value outside 01–12). If the month digit cannot be resolved to a valid 01–12 value (smudged, ambiguous, or the source appears to show something like a bare "0"), do NOT guess or emit a malformed date — set `invoice_date`/`delivery_date` to null, lower confidence to ≤ 0.6, and add a `source_warning`: `"invoice_date month unreadable/invalid — please verify against slip"`.
