...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **`quantity_raw` is a LITERAL digit-only transcription of THIS row's Pounds cell — numbers and spaces only.** Do NOT embed annotation words or markers such as "(circled)", "(boxed)", "circled", slashes joining alternate readings, or arrows — those belong in `notes` only. A circled "51" is recorded as `quantity_raw = "51"` (record the circle in `notes`, e.g. "51 circled = final"), NOT `quantity_raw = "51 (circled)"` or `"50 (circled) / 66"`.
    - **Read only the ONE cell aligned to this row.** Never carry a number seen in an adjacent row's Pounds cell into this row. If the value you're tempted to write actually sits on the line above or below, this row's cell is blank — treat it as an empty cell (quantity_raw = null).
    - **Transcribe every digit of a multi-digit number — do not drop a leading digit.** If a value looks like it could be "45" or "145", read the full width of the ink; when genuinely ambiguous, emit your best full reading, lower confidence to ≤ 0.6, and add a `source_warning`.
