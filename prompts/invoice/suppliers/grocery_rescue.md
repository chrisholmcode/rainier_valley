...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **Transcription fidelity (most-corrected failure mode on this supplier):** Before writing `quantity_raw`, read the number digit by digit and confirm the leading and trailing digits. Do NOT drop a leading digit (e.g., "145" mis-read as "45") — grocery-rescue pounds are frequently 3-digit, so a suspiciously small 2-digit value on a wide cell is a flag to re-check for a leading digit. Transcribe ONLY marks you can actually see: never append "(circled)", "(boxed)", or a second tally number unless that annotation/number is genuinely visible on the slip. If you are inventing structure to justify a value, stop and re-read.
    - **"REQUIRED" applies only to cells that actually contain writing.** If the cell has NO writing, it is a blank row — set `quantity_raw = null` per the blank-row rule; do NOT fill in a guessed number to satisfy the "never blank" instruction. If a cell has writing but you truly cannot read it, emit your best digit reading, set confidence ≤ 0.6, and add a `source_warning` — but never fabricate a clean value like "51 (circled)" for an empty or illegible cell.
