...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **`quantity_raw` contains DIGITS and separators ONLY (spaces, line breaks, slashes for genuinely separate written numbers).** Do NOT write annotation words or markup such as "(circled)", "(boxed)", "(crossed out)" into `quantity_raw` — that descriptive interpretation belongs in `notes`. A cell where "51" is circled → `quantity_raw = "51"` (NOT `"51 (circled)"`), and `notes` = "51 circled as final". Reviewers blank the field when it contains annotation text instead of a clean numeric transcription.
    - **Preserve every digit of a single multi-digit number — do not truncate.** If a value looks like "145", emit `"145"`, never `"45"`. When a leading digit is faint or you are tempted to drop one, re-examine: dropping a leading digit is a recurring miscorrection. If genuinely unsure how many digits, report both readings in `quantity_raw` (e.g., `"45 / 145"`), lower confidence to 0.6, and add a `source_warning`.
