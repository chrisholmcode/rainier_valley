...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). Gross Weight numerals on these manifests are frequently misread (dropped/added/transposed digits, e.g. 117 vs 17, 7 vs 73). Before recording, sanity-check: Gross Weight should be roughly consistent with (Quantity × a plausible per-case weight). If the read weight implies an implausible per-case weight (e.g. <1 lb or >100 lb per case), re-read the digits carefully and prefer the reading consistent with the quantity. Transcribe every visible digit; do not truncate leading or trailing digits.
