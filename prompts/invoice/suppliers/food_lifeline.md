...
- **Handwritten Pounds column => quantity_raw.** If this slip has a handwritten (or stamped) "Pounds" / "Lbs" column, transcribe each row's numeric value verbatim into `quantity_raw` for that line — NEVER leave `quantity_raw` blank when a Pounds figure is legible. Read the full number including the trailing digit (e.g., "79", not "70"); if a digit is ambiguous, transcribe your best single reading rather than dropping it. Do NOT round, sum, or convert — copy the digits exactly as written.
