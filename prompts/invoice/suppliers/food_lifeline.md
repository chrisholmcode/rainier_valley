...
- **Pounds column => `quantity_raw`:** If this slip has a handwritten/printed **Pounds** column (grocery-rescue style) rather than the manifest's Quantity/Gross Weight columns, copy the Pounds value **verbatim as an integer** into `quantity_raw` (e.g., Pounds "24" => quantity_raw = 24). Never leave `quantity_raw` blank when a Pounds value is legible. (Preferred routing is still `grocery_rescue`; this rule is the fallback for slips that land on `food_lifeline`.)
