...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "Agency Order No" field is present, capture any other printed order/invoice identifier on the slip verbatim instead — including store invoice numbers that do NOT start with "ACR" (e.g., a QFC store code with a date suffix like "QFC-MI-2026-07-10"). Never leave invoice_or_order_number blank when any order/invoice number is legible.
