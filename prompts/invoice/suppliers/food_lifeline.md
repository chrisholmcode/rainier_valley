...
- If this slip has NO printed "Quantity" column but instead a **Pounds** column (the handwritten grocery-rescue-style variant), take the numeric value written in the Pounds column for each line and populate `quantity` with it (e.g., a row showing Pounds = 16 => quantity = 16). Do NOT leave `quantity` blank just because the printed "Quantity" column is absent. Also set unit = "lb" for these rows.
