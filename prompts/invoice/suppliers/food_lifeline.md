...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - Read the Gross Weight numeral digit-by-digit and transcribe it EXACTLY — do not add, drop, or transpose digits. These are the most common errors on this supplier: reporting a 1-digit weight as 2 digits (or vice versa), dropping a leading/trailing digit (e.g., 117 vs 17), and swapping similar values (e.g., 104 vs 125). Re-read the printed cell and confirm the digit count before committing.
  - Sanity check: approx_weight should be roughly `quantity × typical case weight`. If your reading implies an implausible per-case weight (e.g., a full case of canned goods reading as ~7 lb, or a single item reading as >100 lb), re-examine the Gross Weight cell before finalizing.
