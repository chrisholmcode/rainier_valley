...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - **⚠️ supplier is ALWAYS `"grocery_rescue"` for this form — NEVER `"food_lifeline"`.** The "FOOD LIFELINE" logo printed on the slip is the *broker*, not the supplier value. Do not copy the on-slip branding into the `supplier` field. Only the printed **Agency Order** manifest maps to `supplier = "food_lifeline"`; the handwritten grocery rescue donation form (Donor | Address | Agency | Date + temp/pounds table) is always `"grocery_rescue"`.
