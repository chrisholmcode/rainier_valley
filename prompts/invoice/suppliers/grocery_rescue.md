...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Align to the row before extracting, and read the WHOLE number.** Before writing any weight, confirm the numeral sits horizontally in-line with THIS row's label — the Pounds column is dense and values from an adjacent row bleed across. If the cell for this row has no ink of its own, it is blank (approx_weight/quantity/quantity_raw = null) even if the row above or below has a value; do NOT carry a neighbor's number down. When a cell IS filled, read every digit including a faint/partly-cut leading digit (e.g., "145" not "45", "56" not "6") — grocery-rescue weights are commonly 3 digits; if the value looks implausibly small for a rescue pallet (< ~20 lb) re-check for a dropped leading digit, set confidence ≤ 0.6, and note it. When you genuinely cannot tell whether a mark is this row's value or bleed-over from an adjacent row, prefer leaving it null with a source_warning rather than guessing — a false weight is worse than a blank a reviewer can fill.
