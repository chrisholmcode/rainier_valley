...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in. Because this ID mechanically embeds `delivery_date`, any uncertainty in the date propagates here verbatim. Before synthesizing, re-apply the Single-digit-month legibility warning: if the month digit is smudged, partial, or could plausibly be read more than one way, do NOT zero-pad and commit to a guess (e.g., writing `05` for a partially-legible mark). Instead treat delivery_date as unresolved, leave invoice_or_order_number null, set confidence ≤ 0.7, and emit the `delivery_date month digit unclear` source_warning so a reviewer resolves the month rather than correcting the shipment ID after the fact.
