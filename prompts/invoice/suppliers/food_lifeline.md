...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Transcribe the Ship Date **digit-by-digit exactly as printed** — do NOT infer, round, or shift any digit. In particular, read the MONTH field character-by-character: a printed "07" (July) must become month `07`, not `08`. Do not turn a 7 into an 8. If a month digit is ambiguous or obscured, prefer the literal reading of the visible glyph over a "nearby" value, and never emit a malformed month (e.g., `0`, `00`, or a single-digit month) — the month must be a valid two-digit value 01–12.
