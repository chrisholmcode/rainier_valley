...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning (any month, not just 7/8):** if the handwritten month digit is ambiguous (could be more than one value, ink smudged, overwritten, or only partially legible), do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"invoice_date/delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`.
  - **Output must be a syntactically valid YYYY-MM-DD.** Month is always a zero-padded two-digit value 01–12 and day a zero-padded two-digit value 01–31. Never emit a malformed month such as `2026-0-28` or `2026-5-28`. If your best reading would yield an out-of-range or malformed month/day, treat it as an ambiguous read: emit your best valid guess, lower confidence to ≤ 0.6, and add the month-digit `source_warning` above.
