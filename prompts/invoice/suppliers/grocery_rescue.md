...
- **`supplier` is ALWAYS the literal string `grocery_rescue` for every slip in this bucket — NEVER `food_lifeline`.** The "FOOD LIFELINE" logo printed upper-left on the form is the broker's branding, NOT the supplier value. Do not copy the on-slip logo/brand text into the `supplier` field. `food_lifeline` is a separate bucket reserved for printed Agency Order manifests and must never be emitted here.
