...
- donor_org: capture the donor source code printed on the slip verbatim (e.g., a hyphenated store/donor code like "SWY-GEN" for Safeway-General or "QFC-MI"). These codes identify the donating retailer/program and MUST be extracted. Only set `donor_org = null` if no such donor code appears anywhere on the document.
