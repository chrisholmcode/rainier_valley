...
- HANDWRITTEN Pounds column fallback: If this document instead has a per-line handwritten **Pounds** column (rather than the printed "Quantity" column), map each line's Pounds number DIRECTLY into `quantity_raw` verbatim (e.g., a line reading "24 lbs" => `quantity_raw = 24`). NEVER leave `quantity_raw` blank when a Pounds value is legible.
