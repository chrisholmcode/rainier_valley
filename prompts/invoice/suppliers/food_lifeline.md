...
- Handwritten Pounds column: if this slip carries a handwritten "Pounds" column (grocery-rescue style) and it is being processed under this supplier, map each line's Pounds value directly into `quantity_raw` verbatim (e.g., "24", "16", "170"). Never leave `quantity_raw` blank when a Pounds number is legibly written — the Pounds figure IS the quantity for these lines. Set `unit` = "lb" for Pounds-column lines.
