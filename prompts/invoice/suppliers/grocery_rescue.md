...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **Substitute BOTH resolved values literally — never emit a partial ID.** The final string MUST be the full donor code, a hyphen, and the complete resolved `YYYY-MM-DD` date (exactly 10 characters: 4-2-2). Do not stop after the donor prefix, do not leave a trailing hyphen, and never emit a placeholder, template token (`<delivery_date>`), backslash, or line-continuation character. If you find yourself writing a value that does not end in a full 4-digit-year/2-digit-month/2-digit-day date (e.g. `QFC-BWY-`), STOP: either finish resolving the date and append it (correct example: `QFC-BWY-2026-07-21`), or — if the date is genuinely unresolved (null) — set invoice_or_order_number = null. Never output a truncated string like `QFC-BWY-\` or `QFC-BWY-`.
