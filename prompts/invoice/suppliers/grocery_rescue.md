...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity = **REQUIRED — this is NOT a unit-count field on grocery rescue slips.** Mechanically copy `approx_weight`: after you set `approx_weight`, immediately set `quantity` to the *identical* number. `quantity` must NEVER be blank/null when `approx_weight` is non-null, and must NEVER differ from `approx_weight`. Do not omit it because "quantity" usually means a piece count — on these forms the Pounds cell is both the weight and the billed quantity. Full example for a row whose Pounds cell reads "17": `approx_weight = 17`, `quantity = 17`, `quantity_raw = "17"`. (Verify before emitting each row: if `approx_weight` is a number, `quantity` must equal it.)
