...
- Some manifests group line items under printed CATEGORY/SECTION headers (e.g., "Bakery", "Produce", "Canned/Dry Goods", "Frozen"). These header rows are NOT line items: they have no Item No., Unit, Quantity, or Gross Weight of their own. Do NOT emit a line for a category header and do NOT copy the header word into `item_name_raw` — leave `item_name_raw` blank / omit the row. Only rows with an actual Item No. and a real product Description are line items.
- Never truncate a multi-word Description to a single word. Capture the ENTIRE printed text of the cell verbatim, including slashes and trailing words (e.g., "Canned/Dry Goods" in full, not just "Canned").
