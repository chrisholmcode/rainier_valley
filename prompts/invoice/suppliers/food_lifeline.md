...
- FALLBACK (no printed Quantity column present): If this document instead has a handwritten/printed **Pounds** column (per-line poundage) and lacks a distinct Quantity column, map the Pounds value directly into `quantity` (the numeric weight, e.g. 16, 14, 170, 21) and set `unit = "lb"`. Do NOT leave `quantity` blank when a Pounds figure is present. (First re-check routing: a Donor/Address/Agency/Date handwritten slip should be `grocery_rescue`; only apply this fallback if the document has been classified as food_lifeline.)
