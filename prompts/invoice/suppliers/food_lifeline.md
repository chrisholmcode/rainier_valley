...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Read ONLY the field explicitly labeled "Ship Date". Do NOT read any other nearby date (e.g., a print/generated date, due date, or a delivery-window end date) — those often sit one month later and must be ignored.
  - The Ship Date is printed in **month/day/year** order. Transcribe the month exactly as printed; do NOT shift it forward by one month. When the printed date is ambiguous, prefer the earlier month rather than assuming the later one.
  - Sanity check before emitting: the resulting month must match the month printed in the "Ship Date" label, not a later date elsewhere on the page. Example: a Ship Date printed as "07/07/2026" => `2026-07-07` (NOT `2026-08-07`).
