...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. **The date portion of this ID MUST be a literal, character-for-character copy of the resolved `delivery_date` — do NOT independently re-read, "clean up", or re-derive the month/day digits when building the ID.** In particular, if the handwritten month digit was ambiguous (the single-digit month legibility warning fired, confidence ≤ 0.7), do NOT silently promote it into a confident two-digit month here (e.g., do not turn an unclear month into `05`); carry the same reading and uncertainty through. The synthesized ID must never disagree with `delivery_date`. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
