...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **The month digit in this key inherits the delivery_date reading — re-verify it before composing.** Because invoice_or_order_number embeds delivery_date verbatim, any misread month digit silently propagates into the shipment ID. Before emitting this field, re-examine the slip's month digit specifically. If the month digit is smudged, partially legible, or ambiguous (per the "Single-digit month legibility warning" above), do NOT commit to a confident single reading here: lower slip `confidence` to ≤ 0.7 and add a `source_warning`: `"invoice_or_order_number month digit derived from unclear delivery_date — please verify against slip"`. Never emit a confident synthesized key built on an uncertain month digit.
