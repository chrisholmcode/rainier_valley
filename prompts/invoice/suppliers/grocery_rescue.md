...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **The date portion of this ID MUST be copied byte-for-byte from the FINAL resolved `delivery_date` — never re-derive, re-pad, or "clean up" the month/day while building the ID.** In particular, if the handwritten month resolved to a single ambiguous digit, do NOT silently pad it to a two-digit month like "05" or "08". A common error on this supplier is emitting `SWY-GEN-2026-05-28` when the slip's month glyph is actually a lone, partially-legible single digit (correct reading e.g. `SWY-GEN-2026-0-28`). When the month digit is ambiguous (see the single-digit month legibility warning above), reflect your exact best reading in BOTH `delivery_date` and this ID, set confidence ≤ 0.7, and add the `delivery_date month digit unclear` source_warning rather than confidently normalizing to a padded two-digit month.
