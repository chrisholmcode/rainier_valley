...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **approx_weight must never be left blank when a weight is present on the document.** If there is no printed "Gross Weight" column but the slip has a handwritten "Pounds" value (as on the hand-filled rescue slips that still route here), read that handwritten number as `approx_weight` (e.g., a handwritten "14" => approx_weight = 14). Only leave approx_weight blank if no weight of any kind is legible.
