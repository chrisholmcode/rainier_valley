...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD, **strictly zero-padded**: the month and day MUST each be exactly two digits (e.g., May → `05`, not `5` or `0`; the 8th → `08`). The final value must be a syntactically valid ISO date — never emit a one-digit or empty month/day component (e.g., `2026-0-28` and `2026-5-28` are both invalid and must never be produced; use `2026-05-28`). If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
