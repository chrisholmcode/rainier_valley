...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit legibility warning:** the handwritten **day** digit is the most-corrected part of the synthesized `invoice_or_order_number` on this supplier — reviewers repeatedly bump it by one (e.g., `...-08-05` corrected to `...-08-06`). Curved digits (5/6, 3/8, 0/6, 1/7) are easily confused in these hand-filled forms. If the day digit is at all ambiguous, do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be ±1 — please verify against slip"`. This warning must also fire even though the value is only echoed into the derived invoice_or_order_number.
