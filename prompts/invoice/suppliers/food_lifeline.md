...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "ACR-"-style Agency Order No is present, capture whatever printed order/reference/PO number the slip does carry verbatim — including vendor- or store-prefixed references such as "QFC-MI-2026-07-10". Do NOT leave this field blank when any such reference number is visible; only leave it null if the document shows no order or reference number at all.
