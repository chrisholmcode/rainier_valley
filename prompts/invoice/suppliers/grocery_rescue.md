...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **Do NOT zero-pad a single-digit month on this supplier** — reviewers record the month exactly as written on the slip, so a slip reading "5/28" must be emitted as `2026-5-28`, NOT `2026-05-28`. (Day is still written as on the slip.) Apply this identically to BOTH `invoice_date` and `delivery_date`. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
