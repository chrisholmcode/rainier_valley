...
- **Rows with a non-empty Pounds cell:**
  ...
  >
  > 🎯 **Bind each Pounds value to its OWN row. Read horizontally.** A numeral only counts for a row if it sits on that row's printed line in the Pounds column. Do NOT pull a number up or down from an adjacent row to "fill" a row that looks empty — if the numeral belongs to the row above or below, THIS row is genuinely blank (approx_weight = null). Grocery rescue slips are hand-filled and values often drift vertically; before finalizing, verify each extracted weight is on the same horizontal line as its row label. The "never leave blank when a numeral is legible" rule applies ONLY to numerals written on this row's own line — it is not a license to borrow a neighbor's number.
