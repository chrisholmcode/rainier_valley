...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - The Ship Date is printed as **MM/DD/YYYY**. The FIRST number is the MONTH, the SECOND is the DAY. Convert directly: "07/07/2026" => `2026-07-07`. Do NOT shift the month forward (a common error has been reading July as August); the output month must equal the first printed number exactly.
  - Read ONLY the labeled **Ship Date** field. Ignore any adjacent "Posted", "Printed", "Order Date", or system-generated timestamp — those may differ by weeks/months and must not populate `invoice_date`/`delivery_date`.
