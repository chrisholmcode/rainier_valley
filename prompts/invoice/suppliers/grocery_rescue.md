...
- **Rows with a non-empty Pounds cell:**
  ...
  - approx_weight = the final/accepted pounds, parsed per the **Running-tally rule** below. **NEVER leave `approx_weight` blank when any numeral is legible.** If the number is hard to read, extract your best guess, lower `confidence` to ≤ 0.6, and add a `source_warning`. Only set `approx_weight = null` when the cell contains **absolutely no writing**.
    - **Leading-digit / digit-count check (do this on EVERY tally, overwritten, or crossed-out cell):** after choosing the final value, count its digits and re-scan the cell to confirm you did not clip a leading digit off the number you selected. Grocery-rescue pickups are commonly three-digit weights; a "45" sitting next to or under other marks is often actually "145", and a "50" is often "51" once an overwrite is resolved. If the leading digit is uncertain, emit the fuller/longer reading, put every candidate in `quantity_raw`, set `confidence` ≤ 0.6, and add a `source_warning`: `"approx_weight leading-digit uncertain: read as <your reading>, cell also consistent with <alternative> — please verify against slip"`.
    - **Do not force a value out of a genuinely ambiguous mark.** The "never leave blank when a numeral is legible" rule applies only when a numeral is actually legible. If you cannot tell whether a faint mark is a number or an empty/scratched cell, do NOT invent a value and do NOT silently blank it: emit your single best reading, set `confidence` ≤ 0.6, and add a `source_warning`: `"approx_weight uncertain — faint/ambiguous mark, may be blank — please verify"` so a reviewer makes the call.
