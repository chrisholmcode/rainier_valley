...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit reading warning (03 vs 31):** the DAY portion is misread far more often than the month. A leading-zero single-digit day (e.g. "03", "01", "05") is frequently mis-transcribed as its reversed two-digit form ("31", "10", "50") when the glyphs sit close together. Before finalizing, re-read the day: if you are about to emit a day in the high 20s or 30s (28–31), explicitly confirm the slip is not actually a leading-zero low day ("03" etc.). Only emit a 30th/31st if the slip clearly shows it; if genuinely ambiguous, prefer the leading-zero low-day reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be leading-zero low day (e.g. 03) — please verify against slip"`. Do NOT silently override a clearly-written 31.
  - **Never emit a malformed date.** The resolved value MUST be a complete, well-formed `YYYY-MM-DD` with a two-digit month 01–12 and a two-digit day 01–31 — never a zero, missing, or single-"0" component (e.g. `2026-0-12` is invalid). If the month digit is unreadable, do NOT drop it to "0"; instead follow the "Single-digit month legibility warning" path (best reading, confidence ≤ 0.7, source_warning). If you cannot produce a well-formed date, set `delivery_date`/`invoice_date` = null rather than emitting a partial string, and let a reviewer fill it in.
