...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - Sanity check: if the extracted month disagrees with the month implied by `Today's date` / upload context (these forms are filled the day of pickup, so the month almost always matches), re-examine the source before committing. Two common misreads: (a) you transposed month and day (e.g. read "08" when it should be July); (b) you read a single smudged handwritten digit as a clean two-digit month (e.g. read "05"/May when the digit is actually a single-digit month like a smudged "0"-ish stroke). When the month looks clean but conflicts with the upload date, do NOT trust it — lower confidence to ≤ 0.7 and add a `source_warning`: `"invoice_date month conflicts with upload date: read as <your month> — please verify against slip"`. Prefer the reading consistent with the upload date.
  - **Always zero-pad month and day to two digits** (YYYY-MM-DD, e.g. `2026-05-28`). Never emit a single-digit month or day.
