...
- **Anchor each Pounds value to its own row label — do NOT fill values top-to-bottom into the fixed skeleton.** The form frequently has blank rows interspersed between filled ones, and the printed row order on the slip may not perfectly match the canonical order above. For EACH predefined row, locate that row's label on the slip and read the Pounds cell **on the same horizontal line as that label** — trace across from the label to its Pounds cell before recording a number. Never carry a value up or down to an adjacent row. After extracting, sanity-check: the count of rows you gave a non-null `approx_weight` must equal the count of Pounds cells that actually have writing on the slip — if these differ, you have misaligned a value to the wrong row; re-trace each filled Pounds cell back to its label. If a value's owning row is genuinely ambiguous (writing sits between two labels), lower confidence to ≤ 0.6 and add a `source_warning` naming both candidate rows rather than guessing.
