...
- donor_org = the donor/source organization code printed on the manifest, captured verbatim (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC). Look for a Donor field or store-source code. Only set donor_org = null if no such code appears on the document.
