...
- If the document has NO printed **Gross Weight** column but shows a handwritten/typed **Pounds** value (a single number per line or per category), this is almost certainly the handwritten Donor / Address / Agency / Date form — re-route to `supplier = "grocery_rescue"` and follow that prompt. Do NOT leave `approx_weight` blank. If you nonetheless remain on this prompt, map the Pounds value directly into `approx_weight` verbatim (e.g., Pounds "14" => `approx_weight` = 14).
