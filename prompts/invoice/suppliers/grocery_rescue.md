...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` is ALWAYS `"grocery_rescue"` for this form — never `"food_lifeline"`.** The prominent "FOOD LIFELINE" logo in the upper-left is the pre-printed form brand, NOT the supplier value. Do not copy the logo text into the `supplier` field. `food_lifeline` is a separate, reserved bucket (printed Agency Order manifests only) and must never be emitted for these handwritten grocery-rescue slips.
