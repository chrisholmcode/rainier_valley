...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - The Ship Date is printed in **M/D/YYYY** order (month first, then day). Read the month and day digits carefully and transcribe them exactly — do NOT round or shift the month. A recurring error on this supplier is emitting the month one higher than printed (e.g., reading "7/7/2026" and writing `2026-08-07` instead of the correct `2026-07-07`). Before finalizing, re-read the month digit and confirm your output month matches the first number in the printed Ship Date.
