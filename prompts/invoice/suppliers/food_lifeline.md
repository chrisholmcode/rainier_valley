...
- HANDWRITTEN POUNDS FALLBACK: If the document you are reading is actually the handwritten Donor / Address / Agency / Date slip with a per-category **Pounds** column (not the printed Quantity/Gross Weight columns), it should be rerouted to `supplier = "grocery_rescue"`. If for any reason it is being processed here, DO capture each handwritten Pounds figure into `quantity_raw` verbatim (e.g., a row showing "24" lbs => `quantity_raw = 24`). Never leave `quantity_raw` blank when a Pounds value is legible.
