...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Transcribe the **month** digits of the Ship Date directly from the slip — do NOT infer or guess. On these printed manifests the month glyph is easily misread (e.g., a printed `07` misread as `08`); zoom in and confirm the exact printed digits before emitting.
  - VALIDATE the result: both `invoice_date` and `delivery_date` MUST be a complete `YYYY-MM-DD` with a two-digit, zero-padded month in the range `01`–`12` and a two-digit day. Never emit a truncated or single-digit month (e.g., `2026-0-07` or `2026-8-07` are invalid). If the month cannot be read confidently, re-read the Ship Date rather than approximating.
