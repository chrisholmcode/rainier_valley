...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. **Emitted dates MUST be valid, zero-padded ISO-8601 (YYYY-MM-DD) with month 01–12 and a real day — never emit a malformed month such as "0" or an unpadded component. If you cannot resolve a valid month, set the date to null and add a source_warning rather than emitting an invalid date.** If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
