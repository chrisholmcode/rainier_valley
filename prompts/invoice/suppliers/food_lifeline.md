...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Use ONLY the date explicitly labeled **Ship Date**. Do NOT use any other date on the page (e.g., a print/processed timestamp, an order-window or "deliver by" date, or a date in the page footer), which is often a later month and leads to an off-by-one-month error.
  - Read the month field character-by-character before emitting it. On these manifests the month has been repeatedly misread one month too high (e.g., emitting `-08-` when the Ship Date actually reads `-07-`). If the digit is ambiguous, prefer the lower/earlier reading and re-check against the day and year on the same line.
