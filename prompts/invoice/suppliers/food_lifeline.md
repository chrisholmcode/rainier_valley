...
- item_name_raw MUST come from an actual line item with goods on it. Do NOT put a standalone **category/section header** (e.g., "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Dry Goods") into item_name_raw when it is just a grouping label with no described product on that row — leave item_name_raw blank for header-only rows. When a category label IS the printed item text on a goods row, capture the FULL printed label verbatim (e.g., "Canned/Dry Goods", never the truncated "Canned"). Reminder: category words appearing on a hand-filled, per-category form are a strong signal this is `grocery_rescue`, not this supplier — re-check the routing rule above before extracting. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
