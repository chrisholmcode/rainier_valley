...
- **Row-alignment check (prevents off-by-one label shifts):** The 10 rows are pre-printed in the fixed order above, but you MUST anchor each `item_name_raw` to the label you actually read printed in that row on the slip — do NOT paste the memorized list top-to-bottom independently of the image. Before finalizing, walk the rows top-to-bottom and confirm each emitted label matches the printed text beside its Pounds value. A common failure is attributing a Pounds value to the row ABOVE its true row (e.g., labeling a Meat value as "Frozen Foods", or a Frozen Foods value as "Dairy/Juice/Alt. Dairy") when a hatched or skipped row shifts your count. If your labels have drifted out of sync with the visible rows, re-anchor on an unambiguous landmark row (e.g., the "Meat" row or the last "Produce" row) and recount. When any value's row label is uncertain, lower confidence to ≤ 0.7 and add a source_warning "row label alignment uncertain — verify item_name_raw against slip row".
