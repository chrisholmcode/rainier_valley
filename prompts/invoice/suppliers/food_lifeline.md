...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). If the document has no printed **Gross Weight** column but instead shows a handwritten/typed **Pounds** value (per line or per category), use that number as approx_weight verbatim — never leave approx_weight blank when a Pounds figure is legible.
