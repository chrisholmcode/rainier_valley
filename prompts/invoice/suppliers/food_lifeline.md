...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - approx_weight comes ONLY from a printed **Gross Weight** column on a genuine AGENCY ORDER manifest. If there is no printed Gross Weight column and weights instead appear in a hand-filled **Pounds** column, this is NOT a food_lifeline manifest — STOP, set `supplier = "grocery_rescue"`, and follow that prompt (do not populate approx_weight from the handwritten Pounds column here).
