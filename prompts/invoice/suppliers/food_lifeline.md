...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Transcribe the Ship Date's **month exactly as printed** and emit it as a **valid, zero-padded two-digit month (01–12)** — e.g. July => "07". Do NOT guess an adjacent month, and NEVER emit a single-digit or empty month segment (values like "2026-0-07" or "2026-8-07" are malformed and wrong). If the printed month digit is unclear, re-read it rather than defaulting to another month.
