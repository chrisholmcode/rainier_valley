...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - approx_weight is REQUIRED — never leave it blank. If there is no printed "Gross Weight" column but there IS a handwritten "Pounds" figure (typically a small integer such as 14, 17, 24, 70, 79), read that handwritten value into `approx_weight`. (If the document has a per-category handwritten Pounds column and Donor/Address/Agency/Date fields, it is a `grocery_rescue` slip — set `supplier = "grocery_rescue"` and follow that prompt, but still capture the handwritten pounds as `approx_weight`.)
