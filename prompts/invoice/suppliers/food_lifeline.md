...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **approx_weight source (strict):** Populate approx_weight ONLY from the printed **Gross Weight** column. If the document instead shows a HANDWRITTEN per-category **Pounds** column (small round numbers like 14, 16, 21, 24), this is NOT this supplier — STOP and re-route to `supplier = "grocery_rescue"` before extracting anything. Never copy a handwritten Pounds value into approx_weight here, and never leave approx_weight blank on a genuine manifest line that shows a printed Gross Weight.
