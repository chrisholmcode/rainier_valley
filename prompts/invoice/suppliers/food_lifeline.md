...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Transcribe the Ship Date digit-by-digit exactly as printed; do NOT guess or adjust the month. In this supplier's font a printed "7" is easily misread as "8" — verify the month digit carefully before emitting.
  - Both fields MUST be a valid, fully-formed calendar date with a two-digit month and day in the range 01–12 / 01–31. If your parse yields a malformed value (e.g., a missing or zero month like `2026-0-07`), re-read the Ship Date field and correct it before output — never emit a malformed date.
