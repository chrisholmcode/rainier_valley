...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value, normalized to `YYYY-MM-DD` with a zero-padded two-digit month and day.
  - CAUTION: The Ship Date **month** on these printed manifests is frequently misread. Read the month digit(s) carefully against the day — do NOT default to `08`/August. If the printed month is a single digit (e.g., "7"), it is July and must be written as `07`, giving e.g. `2026-07-07`. Re-check the month glyph before finalizing rather than guessing.
