...
- **⚠️ supplier is ALWAYS the literal string `"grocery_rescue"` for this form — NEVER `"food_lifeline"`.** The "FOOD LIFELINE" logo printed in the upper-left is the *broker's* letterhead, NOT the supplier value. Do not copy the visible brand/logo text into the `supplier` field. `food_lifeline` is a different supplier (printed Agency Order manifests only) and is always the wrong answer for this handwritten grocery-rescue donation form.
