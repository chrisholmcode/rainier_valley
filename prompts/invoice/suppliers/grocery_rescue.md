...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is ambiguous for ANY reason (could be "7" or "8", could be another digit, ink smudged, overwritten, or only partially legible), do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`.
  - **ISO validity self-check (do this last):** before populating `invoice_date` / `delivery_date`, confirm the resolved value is a real calendar date — month MUST be `01`–`12` and day MUST be `01`–`31`, always zero-padded to two digits. If your reading would produce an out-of-range or non-parseable month/day (e.g. a month of `0`, `00`, or `13+`), the digit was misread: re-examine the slip, emit your best *valid* reading, set confidence ≤ 0.6, and add a `source_warning`: `"invoice_date month/day failed ISO validity check: best reading <value> — please verify against slip"`. Never emit a date with a single-digit or out-of-range month/day.
