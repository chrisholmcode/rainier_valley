...
- donor_org: extract the printed donor/source code verbatim if one is present on the manifest. These appear as short uppercase retailer codes, often with a location/segment suffix (e.g., "SWY-GEN" = Safeway General, "QFC-MI" = QFC). Capture the code exactly as printed, including the suffix. Set `donor_org = null` ONLY when no such donor code appears anywhere on the document.
