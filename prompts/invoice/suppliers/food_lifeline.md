...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no Agency Order No is printed, fall back to any other order/reference number on the slip — including a store- or vendor-prefixed dated reference such as "QFC-MI-2026-07-10" (format: STORE-CODE-YYYY-MM-DD). Never leave this field blank when any order/reference number is visible anywhere on the document.
