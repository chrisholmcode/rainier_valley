...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "Agency Order No" is present, use whatever order/invoice/reference number IS printed on the slip — including store- or donor-sourced formats such as "QFC-MI-2026-07-10" (store banner + region + date). Capture it verbatim. Never leave this field blank when any order/reference number appears anywhere on the document.
