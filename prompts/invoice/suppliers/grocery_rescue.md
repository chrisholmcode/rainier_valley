...
5. **Sequence with descending or non-monotonic numbers and no clear circle** (e.g., "151 123 108 40") => these are running adjustments while counting, but the accepted total is **NOT reliably the last number** on grocery_rescue slips — reviewers frequently correct "last value" picks to a different entry (often the largest) or to blank. Do NOT confidently emit the last number. Instead: list every discrete number in `quantity_raw`, emit your single best reading of the accepted total in `approx_weight`/`quantity`, set confidence ≤ 0.5, and add a source_warning `"Pounds cell ambiguous multi-number tally: candidates <list all numbers> — please verify which is the accepted total"` so a reviewer resolves it.
   - **Dropped-leading-digit check (do this before committing any reading):** on this supplier the most common Pounds error is under-reading a multi-digit count — e.g., writing `45` when the cell reads `145`, or `50` when it reads `51`. Before finalizing, re-scan the cell for a faint or narrow leading digit and for a trailing digit you may have merged/dropped. If a leading/trailing digit is even partly plausible, prefer the larger multi-digit reading, lower confidence, and note both candidates in `quantity_raw`.
