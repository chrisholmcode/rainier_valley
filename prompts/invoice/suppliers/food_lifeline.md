...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - FALLBACK — never leave `quantity` blank: if the document has a **Pounds** column (or per-category pounds figures) instead of a printed "Quantity" column, this is almost certainly the handwritten slip that belongs to `grocery_rescue` (re-route if you can). If you nonetheless extract it here, set `quantity` = the numeric Pounds value for the line (e.g., 16, 14, 170, 21) and set `unit = "pound"`. Do not output a blank quantity when a Pounds number is legible.
