...
- "FOOD LIFELINE" logo upper-left. Form fields: Donor | Address | Agency | Date. Then a 4-column table: Product/Description | Pick Up Temp (F) | Drop Off Temp (F) | Pounds (lb). **The Pounds (lb) column is always the RIGHTMOST (last) column; the two Temp columns sit between the row label and Pounds. `quantity_raw`/`approx_weight`/`quantity` come ONLY from this rightmost column — never from a Temp column. The Temp columns commonly hold small numbers (e.g., a fridge/freezer reading like 1, 18, 34) that resemble weights; never copy a Temp-column number into quantity fields, and never drop the true Pounds value because a Temp cell also has ink. If handwriting drifts across cell borders, re-anchor to the rightmost column edge before reading the Pounds value.**
