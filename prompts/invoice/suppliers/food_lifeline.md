...
- **Pounds fallback:** If this slip has a **Pounds** column/value but NO printed "Quantity" column, populate `quantity` from the Pounds number (e.g., a Pounds entry of 16 => quantity = 16) and set `unit = "lb"`. Never leave `quantity` blank when a Pounds value is legible. (If the document is clearly the handwritten Donor/Address/Agency/Date form, prefer routing to `grocery_rescue` per the note above; but if it is being processed here, do not emit a blank quantity.)
