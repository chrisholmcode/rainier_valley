...
- Description => item_name_raw verbatim. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
  - CATEGORY / SECTION HEADERS ARE NOT LINE ITEMS: rows that carry only a category grouping label (e.g., "Bakery", "Canned", "Produce", "Canned/Dry Goods") with no actual product description, Item No., quantity, or weight are section headers, not products. Do NOT emit them as `item_name_raw` — leave the field blank (skip the row). Only extract `item_name_raw` from rows that name a specific product.
  - If a category label legitimately IS the value for a cell you must capture, copy it in FULL and verbatim — including slashes and multi-word forms (e.g., "Canned/Dry Goods", not "Canned"). Never truncate a label at a slash or space.
