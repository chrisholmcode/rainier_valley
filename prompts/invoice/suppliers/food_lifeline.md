...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - `approx_weight` is populated ONLY from the printed **Gross Weight** column. If the document has no printed Gross Weight column (e.g., it has a handwritten per-category **Pounds** column instead), this is NOT a food_lifeline AGENCY ORDER — re-route to `supplier = "grocery_rescue"` and do not emit food_lifeline fields. Never copy handwritten Pounds values into food_lifeline `approx_weight`, and never leave `approx_weight` blank when a printed Gross Weight value is present.
