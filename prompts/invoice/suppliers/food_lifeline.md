...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no ACR-style Agency Order No is present, fall back to any other printed order/source reference number on the slip — e.g., a store/source code like "QFC-MI-2026-07-10" (source prefix + `-MI-` + date). Capture it verbatim. Do NOT leave `invoice_or_order_number` blank when any order or reference identifier is visible on the document.
