...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` MUST be exactly `"grocery_rescue"` — NEVER `"food_lifeline"`.** The "FOOD LIFELINE" logo in the upper-left is the *broker* of the pickup, not the supplier bucket. Do not copy the logo text into the `supplier` field. `food_lifeline` is a different bucket (printed Agency Order manifests only) and must never be emitted for these handwritten grocery-rescue slips.
