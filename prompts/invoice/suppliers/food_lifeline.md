...
- Description => item_name_raw verbatim. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
  - Capture the COMPLETE printed text of the Description cell verbatim — do NOT truncate at a slash or space. Category labels like "Canned/Dry Goods" must be copied in full ("Canned/Dry Goods", not "Canned").
  - If a row is only a section/category header (e.g., a "Bakery" band with no product described on that row), leave `item_name_raw` blank rather than copying the header text as an item.
