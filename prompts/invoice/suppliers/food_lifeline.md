...
- item_name_raw comes ONLY from the printed **Description** column of the AGENCY ORDER manifest. Do NOT treat category/section labels from a per-category **Pounds** table (e.g., "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Dairy") as line items — those belong to the handwritten `grocery_rescue` form. If you find yourself reading such labels, STOP and re-route to `grocery_rescue`. When you DO capture such a category label for grocery_rescue, copy the label EXACTLY as printed in full (e.g., "Canned/Dry Goods", never the truncated "Canned"), and leave item_name_raw BLANK for any category row that has no corresponding weight entry.
