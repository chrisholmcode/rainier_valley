...
- If the document has no printed **Quantity** column but instead a handwritten **Pounds** column (and it is being processed under this prompt rather than routed to `grocery_rescue`), map each line's Pounds value into `quantity` and set `unit = "lb"`. Never leave `quantity` blank when a Pounds figure is legible on the line.
