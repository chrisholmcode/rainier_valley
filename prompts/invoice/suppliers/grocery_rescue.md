...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Row-alignment & full-number check (do this before writing any weight):** A Pounds value belongs ONLY to the row it is physically written on. Do NOT carry a value down (or up) into an adjacent row — if a row's own Pounds cell is empty, it is blank (`approx_weight = null`), even if the row above/below has a number. Handwritten grocery-rescue weights are frequently 3 digits (100–300 lb); before recording, re-read the whole cell left-to-right and confirm you captured EVERY leading digit — a value like "145" must not be truncated to "45". If a single digit is ambiguous (e.g. "50" vs "51"), record your best reading, set confidence ≤ 0.7, and add a `source_warning: "pounds digit unclear on <row label>: read as <value>"`.
