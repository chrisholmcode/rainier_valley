...
- document_type = "manifest". supplier = "grocery_rescue" (ALWAYS — a fixed constant). is_donation = true.
  - ⚠️ **Never set supplier = "food_lifeline" for these slips.** The "FOOD LIFELINE" logo printed on the form is the broker's branding, NOT the supplier value. On this handwritten grocery-rescue form the supplier field is ALWAYS the literal constant "grocery_rescue" regardless of any logo or text visible on the page. (`food_lifeline` is a separate supplier reserved for printed Agency Order manifests only.)
