...
- Category/section labels vs. line items: Some manifests group items under printed category banners (e.g., "Bakery", "Canned/Dry Goods", "Produce"). These banners are NOT line items — do NOT emit a row whose only content is the banner text, and do NOT copy a banner into `item_name_raw`. Only rows that have an actual product Description (with an Item No. and Quantity/Weight) become line items. If you do capture a category label anywhere (e.g., in `notes`), record its FULL printed text verbatim — including any slash-separated or wrapped continuation like "Canned/Dry Goods" — never a truncated fragment such as "Canned".
