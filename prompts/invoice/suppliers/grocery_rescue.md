...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Emit the single RESOLVED value from the Pounds cell as a plain number string that EQUALS `approx_weight` (e.g., `quantity_raw = "24"`). Do NOT include annotation words like "(circled)"/"(boxed)", do NOT emit slash- or space-separated alternatives, and do NOT list multiple tally values here. Read the whole cell carefully first so you don't drop a leading digit (e.g., "145", not "45"). Put any tally history, crossouts, or circle/box markers in `notes` instead (e.g., notes: "cell showed 50 circled then corrected to 51"). If, after resolving, the cell has no real accepted value (e.g., a lone circled figure that was voided/struck), treat the row as empty per the empty-cell rule and set quantity_raw = null.
