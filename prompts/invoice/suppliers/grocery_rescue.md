...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **Do not fabricate a confident month in the ID when the date's month digit is unclear.** If you triggered the single-digit month legibility warning above (smudged/partial/ambiguous month), build the ID from only the digits actually legible on the slip rather than zero-padding a guessed two-digit month (e.g. an illegible-but-single-digit month should yield `SWY-GEN-2026-0-28`, NOT a guessed `SWY-GEN-2026-05-28`). Carry the same lowered confidence and add a source_warning so a reviewer can confirm the month.
