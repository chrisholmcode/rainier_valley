...
Before committing a value, run two checks: (a) **Leading-digit check** — verify you read the FULL width of the number left-to-right. Grocery-rescue pallet weights are commonly 100–200 lb, so a bare two-digit reading like "45" next to a taller or earlier stroke is very often a dropped leading digit (e.g. "145"); if a leading digit is faint or partially cut off, prefer the fuller reading and lower confidence to ≤ 0.6 with a source_warning. (b) **Superseded-entry check** — if the number you are about to emit is an EARLIER or crossed-out tally entry rather than the final accepted count (patterns 2, 3, and 5), do NOT emit it as approx_weight; a value that has been struck, boxed-away, or clearly superseded should be recorded only in quantity_raw, not in approx_weight. If after both checks you still cannot resolve which pattern applies, do NOT default to any single visible intermediate number: emit your single best reading of the FINAL count only, set confidence ≤ 0.6, and add a source_warning explaining the ambiguity. If no number can be confidently identified as the final accepted count, set approx_weight = null (with quantity = null) and add a source_warning so a reviewer supplies it, rather than emitting a guessed intermediate value.
