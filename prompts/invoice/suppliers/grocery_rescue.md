...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Month legibility warning (applies to ANY ambiguous month digit):** if the handwritten month digit is ambiguous in ANY way — could be a different digit (e.g., "7" vs "8", "0" vs "5", "0" vs "6"), ink smudged, overwritten, or only partially legible — do NOT silently pick one and do NOT zero-pad it into a confident two-digit month. Emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`. In particular, never resolve a smudged leading month digit to "05" (May) just because it is the most common padding — treat an unclear month as a review trigger.
