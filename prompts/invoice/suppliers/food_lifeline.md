...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). Read ONLY the value on the line labeled exactly "Ship Date" — do NOT pull the date from any other field (e.g., a "Due" / "Must Ship By" / "Expires" date, which is often a later month). Transcribe the printed month digits verbatim and re-check them: a Ship Date of July must be written as month 07, never 08. This is a recurring error source — verify the month before emitting.
