...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - `approx_weight` MUST come from the printed **Gross Weight** column on this manifest. If that column shows a number for a line, always populate `approx_weight` with it — never leave it blank. If the column is truly empty for a line, leave `approx_weight` blank; do NOT copy a weight from another line or infer one.
  - BEFORE reading any weights: if the "Pounds" figures are handwritten and appear as a per-category column (multiple weights like 24 / 16 / 14 for one document), this is NOT a Food Lifeline AGENCY ORDER — set `supplier = "grocery_rescue"` and follow that prompt. A genuine AGENCY ORDER has one printed Gross Weight per printed line item.
