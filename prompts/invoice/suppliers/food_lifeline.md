...
- donor_org: Capture the printed donor/source organization code verbatim (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC). These retail-donor codes appear on the manifest and MUST be recorded — do not leave donor_org blank when a code is present. Set `donor_org = null` only if no donor code appears on the document.
