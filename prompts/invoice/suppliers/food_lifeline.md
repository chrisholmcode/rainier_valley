...
- donor_org: Capture the printed donor / source code verbatim if one appears on the slip (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC). These codes identify the store/source that donated the goods and are usually shown near the item source, a Donor field, or the source suffix area. Only set `donor_org = null` when no donor/source code is present anywhere on the document.
