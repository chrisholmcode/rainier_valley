...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** This field is a **pure verbatim OCR transcription** of the Pounds cell — NOT the interpreted result. Do the running-tally / circled-number interpretation only for `approx_weight`; here you just copy the ink.
    - Copy **every visible numeral exactly**, including all digits of a multi-digit number. Read a contiguous digit run as a whole (e.g., a cell showing "145" → `quantity_raw = "145"`, never "45" — do not drop a leading digit).
    - Preserve every discrete number, including crossed-out and earlier tally values, separated by a single space (e.g., "70 79" → `"70 79"`; clean single value "24" → `"24"`).
    - **Do NOT add annotations, labels, or interpretation.** No "(circled)", no "(crossed out)", no "/" alternatives, no prose. Circling/boxing is captured by `approx_weight` and `notes`, never here. Example: a cell where "51" is circled → `quantity_raw = "51"` (NOT "51 (circled)").
    - Only when the Pounds cell contains **absolutely no writing** may this be null — matching the same "no writing" bar used for `approx_weight`. If you would set `approx_weight = null` for an empty cell, set `quantity_raw = null` too; if there is a legible numeral, both must be populated.
