...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no ACR-style Agency Order No is present, use any other printed order/reference number on the slip — this includes store-source reference codes such as "QFC-MI-2026-07-10" (retailer/store prefix, e.g. QFC, followed by a location code and a date). Do NOT leave this field blank if any order, reference, or manifest number is visible anywhere on the document; capture it verbatim.
