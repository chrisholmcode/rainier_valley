...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **`supplier` MUST be `"grocery_rescue"` — NEVER `"food_lifeline"`.** The slip shows a "FOOD LIFELINE" logo upper-left, but that logo is NOT the supplier value for these rescue forms. Do not copy the visible logo text into `supplier`. `food_lifeline` is a different bucket (printed Agency Order manifests only) and must never be emitted for a handwritten grocery-rescue pickup form.
