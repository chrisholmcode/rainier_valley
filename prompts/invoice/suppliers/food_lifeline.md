...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Transcribe the month digit-by-digit directly from the printed Ship Date; do NOT infer or round it. This supplier's month is frequently misread — July ("07") is often mistaken for August ("08"). Re-check the month glyph before emitting.
  - The result MUST be a valid, zero-padded `YYYY-MM-DD` string with a two-digit month between `01` and `12` (e.g., `2026-07-07`). Never emit a single-digit or empty month (e.g., `2026-0-07` is invalid).
