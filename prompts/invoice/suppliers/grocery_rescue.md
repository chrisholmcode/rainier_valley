...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit transposition & validity check:** After reading the day, re-verify its digit order against the slip — a handwritten "03" is easily misread as "31" and vice versa. A leading-zero day ("03", "07") is common on these forms; do NOT reorder its digits into a larger number. If the day is ambiguous (could be "03" or "31", "0X" or "X0"), emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be transposed — please verify against slip"`.
  - **Valid-date gate — never emit a malformed date.** Every resolved date MUST be a well-formed `YYYY-MM-DD` with a zero-padded month `01`–`12` and a zero-padded day `01`–`31` (e.g. `2026-08-12`, never `2026-0-12`). Before emitting, confirm the string matches this shape. If you cannot form a valid date, set both `delivery_date` and `invoice_date` to null, lower confidence, and add a `source_warning` describing what you could read.
  - **invoice_or_order_number must reuse this validated date verbatim.** When you synthesize the shipment ID below, take the *exact* zero-padded `YYYY-MM-DD` you resolved here — do NOT re-read or re-derive the date from the slip a second time. The date embedded in `invoice_or_order_number` MUST be byte-for-byte identical to `delivery_date` (and `invoice_date`). If that date is null, leave the ID null.
