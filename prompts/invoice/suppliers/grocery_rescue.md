...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit legibility warning:** the day portion is the most-corrected part of the synthesized invoice_or_order_number (recurring off-by-one errors, e.g. reading the 6th as the 5th). If the handwritten day digit is at all ambiguous (adjacent-digit confusion like 5↔6, 0↔6, 8↔3, a stray tally mark, or a partially-formed numeral), do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be off by one — please verify against slip"`. Because invoice_or_order_number is derived directly from delivery_date, any day misread propagates into the ID — double-check the day digit before finalizing.
