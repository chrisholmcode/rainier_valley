...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Output-format validation (apply before emitting):** the resolved date MUST match `YYYY-MM-DD` with a **zero-padded, two-digit month between 01 and 12** and a two-digit day 01–31 (e.g., `2026-05-28`). Never emit a one-digit or `0` month (e.g., `2026-0-28` and `2026-5-28` are both invalid). If — after your best reading — you still cannot commit to a legal month value, set `delivery_date = null` and `invoice_date = null`, lower confidence to ≤ 0.5, and add a `source_warning`: `"delivery_date month unresolved — please enter manually"`, rather than emitting a guessed or malformed month. (Per the invoice_or_order_number rule, a null delivery_date also leaves invoice_or_order_number null for the reviewer.)
