...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity = **REQUIRED — a pure mechanical COPY of `approx_weight`, never an independent reading or judgment.** After you set `approx_weight`, set `quantity` to that exact same value in the same step. Do NOT re-read the cell, do NOT reason about "how many units" — quantity is not a separate count on rescue forms, it is literally the pounds value repeated. Two hard rules: (1) if `approx_weight` is non-null, `quantity` MUST equal it and MUST NOT be blank (this is the single most-corrected field — a legible Pounds cell with a blank quantity is always an error); (2) if `approx_weight` is null (blank/empty cell), `quantity` MUST also be null — never invent a quantity for an empty cell. Example: Pounds cell reads "17" → `approx_weight = 17`, `quantity = 17`. Empty Pounds cell → `approx_weight = null`, `quantity = null`.
