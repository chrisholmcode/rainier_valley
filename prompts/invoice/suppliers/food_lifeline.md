...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). Transcribe the Ship Date character-by-character exactly as printed — read each digit of the MONTH directly from the slip and do NOT infer, round, or shift it. Common miss: reading month `07` (July) as `08` (August); if in doubt, re-read the printed month digits rather than guessing.
