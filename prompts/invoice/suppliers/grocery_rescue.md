...
- **Row-alignment check (item_name_raw):** The predefined list order is a template, NOT a guarantee that the handwritten grid rows line up 1:1 with it. Before assigning a Pounds value to a label, physically trace across the printed row: read the pre-printed Product/Description text in that exact row and use THAT text as `item_name_raw` — never infer the label from its position in the 10-row list. A common failure is an off-by-one shift where a Pounds value gets paired with the label of the row *above* it (e.g. writing `Dairy/Juice/Alt. Dairy` when the value belongs to `Frozen Foods`, or `Coffee Kiosk` when it belongs to `Dairy/Juice/Alt. Dairy`). To guard against this, verify each label against the row directly to its left on the same horizontal line; if the printed label text and your assigned label disagree, trust the printed text and re-check the rows immediately above and below. If the grid alignment is ambiguous, lower confidence to ≤ 0.7 and add a `source_warning: "row/label alignment uncertain — verify item_name_raw matches the physical row"`.
