...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "ACR-"-style Agency Order No is present, capture the alternate order/reference number printed on the slip verbatim — these often carry a store/donor prefix and a date, e.g., "QFC-MI-2026-07-10". Do NOT leave this field blank when any such order or reference identifier appears anywhere on the document.
