...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **Never leave `quantity` blank.** If a line has a Pounds column (or a per-line Pounds value) but no separate numeric Quantity column, set `quantity` = the Pounds value for that line (e.g., Pounds "16" => quantity 16), and set unit = "lb".
