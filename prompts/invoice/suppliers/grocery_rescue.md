...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. The output MUST be a syntactically valid, zero-padded ISO date: exactly `YYYY-MM-DD` with a 4-digit year, a 2-digit month in `01`–`12`, and a 2-digit day in `01`–`31` (e.g., `2026-05-28`, never `2026-5-28` or `2026-0-28`). If you cannot resolve a valid month or day, set delivery_date/invoice_date=null and add a source_warning rather than emitting a partial or malformed date. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
