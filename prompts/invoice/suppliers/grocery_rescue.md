...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is ambiguous (could be "7" or "8", ink smudged, retraced, partially cut off, only partially legible, or a leading zero may or may not be present — e.g. could read as `5` or `05`), do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read month as <MM> — please verify against slip"`.
  - **Month is the single most-corrected part of this date.** Always zero-pad the month to exactly two digits in the final `YYYY-MM-DD` value (write `05`, never `5` and never `0`). A one-digit or truncated month is always an error.
