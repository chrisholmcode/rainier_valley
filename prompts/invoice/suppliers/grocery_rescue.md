...
- Predefined category rows (visible on every form):
  ...
  | Produce | produce | |
- **Anchor each category to the ROW LABEL printed on that same physical line — never to a row's position/order.** The `category` for a line item is determined *only* by matching the Product/Description text actually printed on that row against the table above; it must NOT be inferred from how many rows precede it. Before assigning categories, read the visible printed label of each row and confirm the label text matches the category you assign. Watch for **off-by-one drift**: if you find yourself assigning categories by counting down the fixed list (Bakery, Canned/Dry, …) instead of reading each printed label, STOP — a single skipped or tall/short row will shift every subsequent category by one tier (e.g. shelf_stable→dairy→frozen→meat_protein). If any adjacent categories look one perishability tier off from their labels, re-align by re-reading the printed labels and lower confidence to ≤ 0.7 with a `source_warning`: `"row-to-category alignment uncertain — please verify labels"`.
