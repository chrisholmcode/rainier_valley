...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). Read the **month exactly as printed on the Ship Date line** — do NOT use any other date on the page (e.g., a print/footer timestamp, an order-placed date, or a payment/due date), which is often one month later. Transcribe digit-by-digit; verify the month field matches the Ship Date and not an adjacent date before emitting. Example: a Ship Date of 07/07/2026 must be emitted as 2026-07-07, never 2026-08-07.
