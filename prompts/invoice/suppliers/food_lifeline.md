...
- Description => item_name_raw verbatim. Only capture text that is an actual per-line product Description. Do NOT emit standalone category/group headers or section labels (e.g., "Bakery", "Canned", "Produce", "Meat") as `item_name_raw` — if a row is only a category grouping header with no product description, leave `item_name_raw` blank. When a category label genuinely IS the printed line value, copy it in full exactly as printed (e.g., "Canned/Dry Goods", not a truncated "Canned"); never abbreviate or drop part of a slash-joined label.
