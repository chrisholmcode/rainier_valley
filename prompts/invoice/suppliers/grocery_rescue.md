...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **supplier is ALWAYS `"grocery_rescue"` for this form — NEVER `"food_lifeline"`.** The prominent "FOOD LIFELINE" logo in the upper-left names the *broker*, not the supplier bucket. Do not copy the logo text into the `supplier` field. `food_lifeline` is a separate bucket reserved for printed Agency Order manifests; a hand-filled grocery rescue donation form (Donor/Agency/Date header + Pick Up/Drop Off Temp table) is ALWAYS `grocery_rescue` regardless of the logo.
