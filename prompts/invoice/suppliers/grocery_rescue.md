...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
  - **The date component MUST be byte-for-byte identical to the resolved `delivery_date`** — do not re-parse or re-pad it independently here. If the delivery_date month digit was ambiguous or only partially legible, the same ambiguity applies to this key: carry over the `delivery_date` low confidence (≤ 0.7) and its `source_warning`, and do NOT silently normalize an unclear month into a confident zero-padded two-digit value. When the month digit cannot be read cleanly, emit your best reading but flag it so a reviewer can verify against the slip rather than trusting a padded default.
