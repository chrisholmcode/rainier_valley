...
- **Always emit one line item per predefined row — all 10, every time, in the order above.** This gives the reviewer a pre-populated skeleton to correct if the extractor missed a value, so they never have to manually add a row. Never skip a row.
  - ⚠️ **Row-alignment guard (do NOT shift labels):** The 10 rows are a fixed *set*, but you must still bind each `item_name_raw` / `item_name_normalized` to the **printed Product/Description text physically on that row of the slip**, and read its Pounds value from the **same physical row**. Do NOT stamp out the canonical list by position/count — that causes off-by-one shifts (e.g., emitting "Frozen Foods" for a row whose printed label actually reads "Meat", or "Coffee Kiosk" for a row that actually reads "Dairy/Juice/Alt. Dairy"). For every row, read the actual printed label first, match it to the table above, and only then emit. If the visible printed label for a row does not match the label your ordering predicts, trust the **printed label you can read**, not the position in the list. If a printed row is genuinely absent or unreadable, still emit that canonical row but pair it with `approx_weight=null` and a `source_warning` — never borrow another row's label or Pounds value to fill it.
