...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit legibility warning:** the handwritten DAY is just as error-prone as the month — commonly confused pairs are 5/6, 0/8, 1/7, 3/8. Because `invoice_or_order_number` is synthesized directly from `delivery_date`, a one-day misread silently corrupts the shipment ID. If the day digit is ambiguous or the pen stroke could plausibly read as an adjacent value (e.g., "5" vs "6"), do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be different — please verify against slip"`. When legible, transcribe the day exactly and re-check it against the written slip before synthesizing the invoice number.
