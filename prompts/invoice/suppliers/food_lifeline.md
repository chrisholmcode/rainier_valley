...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no ACR-format Agency Order No is present, capture the printed reference/order code that combines a store or donor abbreviation, a location code, and a date, verbatim (e.g., "QFC-MI-2026-07-10"). Never leave `invoice_or_order_number` blank when any such reference code appears on the slip.
