...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case). approx_weight comes ONLY from the printed **Gross Weight** column of a printed manifest. If the weight number is HANDWRITTEN, or comes from a per-category **Pounds** column, or there is no printed Gross Weight column at all, this is NOT a food_lifeline manifest — STOP, set `supplier = "grocery_rescue"`, and follow that supplier's prompt. Do NOT copy a handwritten Pounds value into food_lifeline's approx_weight, and do NOT leave approx_weight blank on a genuine printed manifest (every printed line has a Gross Weight).
