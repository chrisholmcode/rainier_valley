...
- HANDWRITTEN slips only (if this document was not re-routed to `grocery_rescue`): there is NO printed Quantity column. Read the number written in the **Pounds** column for each line and populate `quantity` with it (transcribe the digits exactly as written; never leave `quantity` blank when a Pounds value is present, even if the handwriting is faint or crossed-out — pick the final/legible value). This is a hard requirement: a Pounds value must always map to `quantity`.
