...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Date integrity — every resolved delivery_date MUST be a structurally valid, zero-padded `YYYY-MM-DD` with month `01`–`12` and day `01`–`31`.** Never emit a malformed segment (e.g., a `0`/`00` month, a dropped/blank month digit, or a day outside `01`–`31`). If your reading would produce such a value, you have misread a digit — re-examine the source and emit your best in-range reading, set confidence ≤ 0.7, and add a `source_warning` describing the ambiguity. Only leave delivery_date null when the Date field is genuinely blank/illegible.
  - **Single-digit day legibility check (symmetric to the month rule above):** if the day digits are at all ambiguous (a single-digit day that could be misread as a two-digit day, a stray mark next to a "3" that could inflate "03"→"31", smudged/partial ink), do NOT silently pick the larger/end-of-month reading — prefer the plausible single-digit reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may be different — please verify against slip"`.
  - **These date rules also govern the date embedded in `invoice_or_order_number`.** The month/day zero-padding, single-digit-month/day legibility, anti-transposition, and structural-validity checks all apply to the `<delivery_date>` component of the synthesized ID. Fully resolve and validate delivery_date FIRST, then build the ID from that validated value — never stamp a malformed or transposed date (e.g., `...-2026-0-12`) into invoice_or_order_number.
