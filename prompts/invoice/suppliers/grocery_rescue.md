...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Digit-count & stray-mark verification (most-corrected reads on this supplier):**
    - **Count the digits before committing.** Grocery-rescue weights are commonly 3-digit (100–199). A leading "1" is easy to miss — "145" gets misread as "45", "156" as "56". Before finalizing, ask: does a 2-digit reading look implausibly small for this row, or is there a faint stroke to the left? If the digit count is at all uncertain, emit your best reading, set `confidence` ≤ 0.6, and add a `source_warning`: `"Pounds digit-count unclear: read as <your reading> but a leading digit may be present — please verify against slip"`. Always record exactly what you see in `quantity_raw`.
    - **Off-by-one single-digit reads** (e.g. "50" vs "51", "56" vs "58") should not be emitted at high confidence — if the final digit is ambiguous, set `confidence` ≤ 0.7 and add a `source_warning` noting the alternate reading.
    - **Do NOT fabricate a number from a faint or stray mark.** A smudge, a stray tick, a bleed-through from the temp columns, or the printed hatching is NOT a numeral. Only extract a value when you can identify an intended handwritten digit. If the "value" is really just an ambiguous mark, treat the cell as blank per the empty-cell rules (approx_weight = null) and add a `source_warning`: `"possible stray mark in Pounds cell — treated as blank, please verify"` rather than emitting a fabricated quantity.
