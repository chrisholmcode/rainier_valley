...
- If a line has NO separate Quantity column and instead shows only a **Pounds** value (handwritten or printed), map that Pounds number directly to `quantity` (e.g., a Pounds entry of "16" => `quantity = 16`). Never leave `quantity` blank when a Pounds value is present; also set `approx_weight` to the same value in that case.
