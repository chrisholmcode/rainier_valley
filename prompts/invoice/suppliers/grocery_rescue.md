...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **quantity_raw is DIGITS ONLY — no annotations.** Emit only the numerals actually written, separated by single spaces in the order written. NEVER embed interpretive text or symbols in this field — no `(circled)`, `(boxed)`, `(crossed out)`, and no `/` separator. (e.g., a cell where "51" is circled → `quantity_raw = "51"`, NOT `"51 (circled)"`; a cell showing "50" struck and "66" written → `quantity_raw = "50 66"`, NOT `"50 (circled) / 66"`.) The circled/boxed/crossout reasoning goes in `notes`; the chosen final value goes in `approx_weight`/`quantity`. Keep quantity_raw a plain verbatim transcription.
    - **Transcribe every digit, including leading digits.** Do not drop a leading "1" on a three-digit weight (read "145", not "45"). If a transcribed number looks implausibly small for a rescue pallet/bin and the cell could plausibly carry a leading digit, re-examine the source, prefer the fuller reading, and lower `confidence`.
