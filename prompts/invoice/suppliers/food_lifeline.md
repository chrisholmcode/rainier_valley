...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - Read the MONTH digit-by-digit from the labeled **Ship Date** field ONLY. Do NOT copy from any other date on the page (e.g., a "Due"/"Delivery By"/print-timestamp date), which is often one month LATER and has caused repeated off-by-one-month errors. If two dates are visible, use the one explicitly labeled "Ship Date"; when in doubt, prefer the earlier date. Re-check that the month you output matches the Ship Date's month exactly before finalizing.
