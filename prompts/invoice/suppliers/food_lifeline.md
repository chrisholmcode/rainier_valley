...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "ACR-"-style Agency Order No is present, capture the printed store/donor reference number instead — commonly a retailer-prefixed code with an embedded date, e.g. "QFC-MI-2026-07-10". Never leave this field blank if any order/pickup/reference number appears anywhere on the slip; capture it verbatim.
