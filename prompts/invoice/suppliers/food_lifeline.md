...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - Weight fallback: if there is NO printed "Gross Weight" column but there is a handwritten **Pounds** value (a plain integer, e.g., 14, 24, 70), capture that number into `approx_weight`. Never leave `approx_weight` blank when any pounds/weight figure is legible on the slip. (If the whole document is the handwritten Donor/Address/Agency/Date form, it belongs to `grocery_rescue` — see "Not this supplier".)
