...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no "Agency Order No" is present, use the printed store/donor order or PO reference instead — these look like a store code plus a date, e.g., "QFC-MI-2026-07-10". Capture it verbatim. Never leave `invoice_or_order_number` blank when any order/PO/reference number is visible on the slip.
