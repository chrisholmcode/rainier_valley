...
- **Rows with a non-empty Pounds cell:**
  ...
  - **Anchor each Pounds value to the row label on the SAME horizontal line.** Read the form row-by-row: a number goes on the row whose printed label shares its horizontal line — never carry a value up or down to an adjacent row. Before finalizing, cross-check: the set of rows you gave a non-null `approx_weight` must be EXACTLY the set of rows that physically have ink in their Pounds cell. If a row's Pounds cell is empty, it stays blank (null) even if the row above/below it has a value; conversely, do not blank a row that clearly has a number just because neighboring rows are empty. When row alignment is unclear (e.g., a value floating between two labels, or handwriting crossing a row boundary), lower confidence to ≤ 0.6 and add a `source_warning`: `"pounds value row alignment unclear near <row label> — please verify which row it belongs to"`.
