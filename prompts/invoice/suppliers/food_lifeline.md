...
- donor_org: Extract the donor/source code printed on the manifest and capture it verbatim (e.g., "SWY-GEN", "QFC-MI"). These are grocery-chain donor source codes (SWY = Safeway, QFC = QFC, etc.), usually found in a Donor/Source field or beside the item source. Only set donor_org = null if no such code appears anywhere on the document.
