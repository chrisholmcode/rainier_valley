...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - **`supplier` is ALWAYS the literal string `grocery_rescue` for this form — never `food_lifeline`.** The "FOOD LIFELINE" logo/text printed on the slip is the *broker's* branding, NOT the supplier field. Food Lifeline merely brokers the pickup; do not copy the logo text into `supplier`. The value `food_lifeline` is reserved exclusively for printed Food Lifeline Agency Order manifests, which this handwritten rescue form is not.
