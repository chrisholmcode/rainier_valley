...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **Quantity is REQUIRED — never leave it blank.** If the line has NO printed "Quantity" column but DOES have a handwritten **Pounds** value (e.g., a per-line pounds figure like 14, 17, 24, 70), populate `quantity` with that Pounds number. Transcribe every digit carefully — handwritten quantities are easy to misread (e.g., 79 vs 70). If a Pounds value is present, `quantity` must be filled.
