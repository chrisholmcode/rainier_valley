...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value in strict `YYYY-MM-DD` format with a **zero-padded 2-digit month and 2-digit day** (never a single-digit or empty month, e.g., `2026-07-07`, never `2026-7-07` or `2026-0-07`). Transcribe the printed Ship Date digits exactly — read the **month** character carefully and do not misread a `7` as `8` (this supplier's Ship Date month is a common error). If the Ship Date is printed in a non-ISO layout (e.g., `7/7/2026`), reorder to `YYYY-MM-DD` without altering any digit.
