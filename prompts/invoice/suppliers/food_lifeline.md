...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - Transcribe the Gross Weight value DIGIT-BY-DIGIT exactly as printed. Do NOT round, estimate, drop a leading digit, or invent digits. Count the number of digits deliberately (e.g., "117" is three digits — do not collapse to "17"; "7" is one digit — do not append "3" to make "73"). If a digit is genuinely illegible, zoom in and re-read the numeral shape rather than guessing; a plausible weight for a full case is typically tens-to-low-hundreds of pounds, but let the printed characters — not this heuristic — determine the value.
