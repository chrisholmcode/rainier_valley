...
- Category/section HEADER rows are NOT line items. If a row contains only a grouping label (e.g., "Bakery", "Produce", "Canned", "Dry Goods") with no Item No., Quantity, or Gross Weight, it is a section header — do NOT emit it as an item; leave `item_name_raw` blank / skip the row.
  - When a Description IS a real item, capture the ENTIRE cell verbatim including slashes and all tokens — never truncate at a `/` or space. e.g., "Canned/Dry Goods" must be captured in full as `Canned/Dry Goods`, not "Canned". When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
