...
- HANDWRITTEN POUNDS-COLUMN VARIANT: If this document has a handwritten **Pounds** column (per-line handwritten weight numbers) instead of a printed Quantity column, read the handwritten number in the Pounds column for each line and populate `quantity` with that value. Never leave `quantity` blank when a Pounds value is written on the line. Transcribe the digits exactly as written (e.g., a written '79' is 79, not 70); if a digit is ambiguous, prefer the reading that matches the full written figure rather than truncating. Set `unit = "pound"` for these lines.
