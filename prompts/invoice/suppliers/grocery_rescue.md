...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** This is the **clean resolved numeric value as a plain digit string**, equal to `approx_weight`. Do NOT dump multiple tally values, crossed-out numbers, or alternate readings here, and do NOT include annotation words like "(circled)"/"(boxed)" or separators like "/". First resolve the cell via the **Running-tally rule** below, then emit ONLY the single chosen number (e.g., a cell showing "50 (circled) / 66" that resolves to 51 → `quantity_raw = "51"`; a clean "56" → `quantity_raw = "56"`; a clean "24" → `quantity_raw = "24"`). Put the raw tally noise, circle/box annotations, and any alternate readings in `notes` instead — never in this field.
