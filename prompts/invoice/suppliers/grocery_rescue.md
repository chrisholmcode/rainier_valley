...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate `delivery_date` with the full resolved value (YYYY-MM-DD). Populate `invoice_date` with only the **year-month** of that same date (YYYY-MM) — e.g. if the resolved date is `2026-08-31`, then `delivery_date = "2026-08-31"` but `invoice_date = "2026-08"`. Do NOT emit a day component in `invoice_date`.
