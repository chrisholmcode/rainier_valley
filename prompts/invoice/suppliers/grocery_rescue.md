...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - **⚠️ supplier is ALWAYS the literal constant `"grocery_rescue"` for this bucket — NEVER `"food_lifeline"`.** The "FOOD LIFELINE" logo printed on the slip is the broker's letterhead, not the supplier value. Do not copy the logo text into the `supplier` field. `food_lifeline` is a *different* bucket reserved for printed Food Lifeline **Agency Order** manifests; if you are following these rules, the answer is `grocery_rescue`.
