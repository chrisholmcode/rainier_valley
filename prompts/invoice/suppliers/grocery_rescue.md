...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Month validity guard (check EVERY time before emitting):** the resolved month MUST be a two-digit value in the range `01`–`12`. Never emit a month of `00`, `0`, a single bare digit, or anything outside `01`–`12`. If your best reading of the handwritten month does not land cleanly in `01`–`12`, the month digit was misread — re-examine the source, prefer the reading consistent with `Today's date`, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> — please verify against slip"`. Do not silently coerce or truncate a misread month; re-read it.
