...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - ALWAYS populate `approx_weight` when any weight figure is legible — read the printed "Gross Weight" column value; if that column is absent and a handwritten/printed "Pounds" value is present on the line instead, capture that number into `approx_weight`. Never leave `approx_weight` blank when a Pounds/Weight figure appears on the slip (typical handwritten values are small integers like 14, 17, 24, 70, 79).
