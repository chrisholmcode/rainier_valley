...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). Read the **month digits literally** from the Ship Date — transcribe exactly the month printed and do NOT advance/shift it forward by a month. Recurring failure mode on this supplier: the model reads a July (07) ship date as August (08). Before finalizing, re-check the month field against the printed Ship Date and confirm it matches character-for-character (day and year are usually correct; the month is the field that slips). Do not derive the month from any other date, print/processed timestamp, or line-item field on the page.
