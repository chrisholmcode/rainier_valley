...
- **Rows with a non-empty Pounds cell:**
  ...
  > **Per-row grounding check (read before filling any Pounds value):** Attribute each numeral to exactly ONE row by its vertical alignment with that row's label. Before writing `approx_weight`, verify (a) you have captured EVERY digit in the cell — re-scan for a leading digit you may have dropped (e.g., a value read as "45" is often "145"; "50" is often "51"); when the leading digit is faint or partly cut off, prefer the fuller/larger reading and lower confidence ≤ 0.6 with a source_warning noting the possible dropped digit; and (b) the number actually sits on THIS row and is not a value that belongs to an adjacent row or a stray/faint mark. Do NOT copy or carry a value from one row onto another. If the only marks on a row are faint, ambiguous, or appear to bleed in from a neighboring row, treat the row as blank (approx_weight = null) rather than guessing — a fabricated weight is worse than a blank the reviewer can fill.
