...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy ONLY the numerals visible in the Pounds cell, exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **quantity_raw is a verbatim digit transcription only — NOT an interpretation.** Do NOT add annotation words or symbols such as "(circled)", "boxed", "/", or "crossed out" into quantity_raw. Any note about which value is circled/final/ambiguous belongs in `notes` (and, if unclear, a `source_warning`), never in this field. Example: a cell where "51" appears circled → `quantity_raw = "51"` (put "circled" in notes), NOT `"51 (circled)"`.
    - **Transcribe multi-digit numbers as a single run — do NOT drop leading digits.** If a number reads as "145", emit `"145"`, not `"45"`. Re-check any suspiciously small value against the digit-boundary caution in the Running-tally rule before finalizing.
    - **Only transcribe digits you can actually read.** If, on close inspection, the cell contains no legible numeral (stray mark, faint smudge, or you cannot confirm a digit), treat the row as blank per the empty-cell rule (`quantity_raw = null`) rather than inventing a value.
