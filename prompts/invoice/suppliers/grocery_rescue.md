...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **VERBATIM DIGITS ONLY — never annotate.** `quantity_raw` is a pure transcription of the ink. Do NOT add interpretive words, parentheticals, or markup like "(circled)", "(boxed)", or "/" separators. A cell where "51" is circled → `quantity_raw = "51"`, NOT `"51 (circled)"`. Put all interpretation (which value is circled/final, running-tally reasoning) in `notes` and reflect the chosen value in `approx_weight`/`quantity` — never in `quantity_raw`.
    - **Do NOT drop leading digits.** Before writing quantity_raw, re-read each number digit-by-digit and count its digits. Grocery-rescue pound values are frequently 3 digits (100–199). If your reading of a number is one digit shorter than the visible ink width (e.g., you read "45" but the cell clearly holds three characters), you have likely dropped a leading "1" — re-examine and transcribe all digits (e.g., "145").
    - If the Pounds cell contains only a single circled/boxed number and no other writing, that circled number IS the value — transcribe just its digits (e.g., `quantity_raw = "51"`). If, on re-examination, the cell holds no real pound value (only a stray mark, a temp reading, or a struck-through non-value), leave `quantity_raw = null` rather than transcribing a spurious number.
