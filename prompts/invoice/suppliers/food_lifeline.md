...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If no ACR-style number is present, capture the order/reference number printed on the slip verbatim even when it uses a different prefix or format — e.g., a QFC store reference like "QFC-MI-2026-07-10" (store-code + date). Never leave this field blank when any order, reference, or store number appears on the slip.
