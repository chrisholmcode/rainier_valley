...
- **Rows with a non-empty Pounds cell:**
  ...
  >
  > **Per-row read discipline (this supplier's most common slip errors).** Before writing `approx_weight` for a row, do all three checks:
  > 1. **Full-digit check.** Read the ENTIRE contiguous run of digits in that row's Pounds cell — do not drop a leading or trailing digit. A weight of "145" must not be recorded as "45"; "151" must not be recorded as "51". If the leading digit is faint or partially cut off at the cell edge, prefer the full multi-digit reading and lower confidence.
  > 2. **Row-alignment check.** Confirm the number physically sits on THIS row's line (between this row's top and bottom rule) before assigning it. Do NOT carry a value from an adjacent row, and do NOT copy the same number onto multiple rows. If a Pounds cell for a row is actually empty, that row gets `approx_weight = null` even if a nearby row has a value — never back-fill a plausible number onto an empty row.
  > 3. **Empty-vs-legible check.** Only set a numeric value when digits are genuinely written in THIS row's cell. If the cell is blank, follow the empty-cell rule (null) — do not invent a value; if any numeral is legible in the cell, you must extract it (never blank).
