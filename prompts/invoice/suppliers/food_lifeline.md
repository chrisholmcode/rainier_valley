...
- FALLBACK (handwritten Pounds-column slip processed here): if there is NO printed "Quantity" column and instead a handwritten per-category **Pounds** value, populate `quantity` with that Pounds number (e.g., a row showing "16" in the Pounds column => quantity = 16). Never leave `quantity` blank when a Pounds value is present. (If the document is clearly the Donor/Address/Agency/Date form, prefer re-routing to `grocery_rescue` per the note above; but if it is being extracted under this supplier, map Pounds → quantity.)
