...
- FALLBACK for handwritten Pounds-column slips landing here: If the document is NOT the printed columnar manifest (no Item No./Quantity/Gross Weight columns) but instead has a handwritten **Pounds** column, prefer routing to `grocery_rescue`. If routing does not occur and this prompt is applied anyway, populate `quantity` with the Pounds value for that line (e.g., a line reading 16 lbs => quantity = 16) and set `unit = "lb"`. Never leave `quantity` blank when a Pounds figure is legible.
