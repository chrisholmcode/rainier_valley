...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD). Transcribe the **month** exactly as printed, digit-by-digit — do NOT round, guess, or drop a digit. On these printed manifests a "7" in the month position is frequently misread as an "8"; when the month glyph is ambiguous, re-read it carefully and cross-check against the Agency Order No date context. Always emit a valid, zero-padded two-digit month (01–12); never output a malformed month like `-0-` or `-8-`.
