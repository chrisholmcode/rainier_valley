...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Day-digit legibility warning (off-by-one):** the day portion of the handwritten Date is frequently misread by one (e.g., a handwritten "6" read as "5", or vice versa). Because `invoice_or_order_number` is synthesized directly from `delivery_date`, a one-day misread silently corrupts BOTH fields. Read the day digit deliberately; if the final day stroke is at all ambiguous (5/6, 0/6, 8/6, closed vs open loop), emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date day digit unclear: read as <your reading> but may differ by one — please verify against slip"`.
