...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to strict `YYYY-MM-DD` with a **two-digit, zero-padded month (01–12) and two-digit, zero-padded day (01–31)** — e.g. `2026-05-28`, never `2026-5-28` and never a single- or zero-valued month like `2026-0-28`. Before emitting, self-check that the month substring is one of `01`–`12`; if your resolved month is not a valid `01`–`12` value, do not emit a malformed date — instead set the date null, lower confidence, and add a `source_warning`: `"invoice_date could not be resolved to a valid YYYY-MM-DD — please verify against slip"`. (Note: if reviewers are systematically rewriting a well-formed date such as `2026-05-28` into a malformed `2026-0-28`, that is a downstream normalization/tooling artifact, not a reading error — the correct extracted value is the zero-padded ISO date.) If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
