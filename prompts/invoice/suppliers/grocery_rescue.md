...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Month-digit validity + cross-check:** The resolved month MUST be a zero-padded value in `01`–`12`. Never emit a month of `00`/`0` or an out-of-range month. Do NOT trust a *confidently* read month just because the digit looks clean: if the resolved month disagrees with the pickup context (`Today's date` in the user message, or a month implied by the filename), re-examine the source before emitting. If after re-examination you still cannot reconcile the month, emit your best reading, lower confidence to ≤ 0.6, and add a `source_warning`: `"delivery_date month may be misread: extracted <month> but inconsistent with pickup context — please verify against slip"`. This applies even when the individual digit *looks* legible.
