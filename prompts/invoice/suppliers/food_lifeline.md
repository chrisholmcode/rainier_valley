...
- invoice_or_order_number: Use the **Agency Order No** value in the upper-right (e.g., "ACR-XXXXXX"). If there is NO "Agency Order No" but the slip carries a store/donor reference code of the form `QFC-XX-YYYY-MM-DD` (a QFC store code followed by a date, e.g., "QFC-MI-2026-07-10"), use that code verbatim as `invoice_or_order_number`. Never leave this field blank when any such order/reference number is printed on the slip.
