...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **`quantity_raw` is a DIGITS-ONLY transcription.** Emit ONLY the numerals (with spaces/line-breaks between distinct tally entries). Do NOT write annotation words or layout descriptions into this field — never "(circled)", "(boxed)", "(crossed out)", and never comparison markers like "/ 66". A circle or box tells you which number is final (that drives `approx_weight` per the Running-tally rule), and any commentary belongs in `notes`, not in `quantity_raw`. Example: the Pounds cell shows a single "51" with a circle around it → `quantity_raw = "51"` (NOT `"51 (circled)"`), `approx_weight = 51`, `quantity = 51`, notes = "51 circled as final".
    - **Re-check multi-digit reads before emitting.** Verify you have not dropped a leading digit (e.g., reading "145" as "45") or split/merged digits. If the cell is genuinely blank for this row, leave `quantity_raw` null — do not emit a value you carried over from an adjacent row or from a circle annotation.
