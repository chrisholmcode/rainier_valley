...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values, separated by single spaces (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **quantity_raw is a DIGIT-ONLY transcription.** Do NOT embed interpretive annotations ("(circled)", "boxed", "final"), separator glyphs ("/"), or any commentary into this field — record only the numbers themselves, space-separated. The fact that a value is circled/boxed goes in `notes`; the chosen final value goes in `approx_weight`/`quantity`. Example: a cell showing a circled "51" next to a crossed-out "66" → `quantity_raw = "51 66"`, `approx_weight = 51`, `quantity = 51`, `notes = "51 circled as final, 66 earlier value"`. Never write `quantity_raw = "51 (circled) / 66"`.
    - Before writing, re-read each number digit-by-digit to avoid dropping a leading digit (e.g., don't read "145" as "45"); if a leading digit is faint, prefer the fuller reading and lower confidence.
