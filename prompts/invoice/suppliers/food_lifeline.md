...
- Do NOT put category/section headers into `item_name_raw`. Words like "Bakery", "Canned", "Canned/Dry Goods", "Produce", "Dairy" that appear as a grouping banner spanning the row (not inside the per-line Description cell) are section labels, not line items — leave `item_name_raw` blank for that banner row and read the actual item description from the lines beneath it. If the document is organized primarily by such category banners with a Pounds column rather than the Item No. | Description | Unit | Quantity columns above, it is almost certainly the hand-filled slip: set `supplier = "grocery_rescue"` and follow that prompt instead.
