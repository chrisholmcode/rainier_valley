...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** This is a *pure verbatim transcription* of the digits in the Pounds cell — NOT an interpretation. Copy every visible digit/number exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **Do NOT add annotations.** Never write "(circled)", "(boxed)", "(crossed out)", or use "/" to separate readings. Those observations belong in `notes` and drive `approx_weight` only. quantity_raw must contain digits and separating spaces/line-breaks ONLY (e.g., write `"51"`, never `"51 (circled)"` or `"50 (circled) / 66"`).
    - **Do NOT drop or add leading digits.** Re-read the full width of the cell before writing — a value like "145" must not become "45", and "51" must not become "151". If the digit count is uncertain, report both readings in `notes` and lower confidence.
    - **Do NOT invent a value.** If, on closer inspection, the mark is a stray pen stroke / smudge / partial number that does not represent a real count, treat the cell as empty (approx_weight=null, quantity=null, quantity_raw=null, notes explaining why) rather than transcribing a hallucinated number.
