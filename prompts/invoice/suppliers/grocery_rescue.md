...
- **Row-alignment guard (prevents off-by-one category shifts):** The 10 rows appear in the canonical order above, but that order is *reference only* — always resolve each row's `category` from the row label you actually read on that physical line, NOT from its position in the list. Before finalizing, walk the rows top-to-bottom and confirm the labels you read appear in the SAME sequence as the table (Bakery → Canned/Dry Goods → Coffee Kiosk → Dairy/Juice/Alt. Dairy → Frozen Foods → Meat → Nonfood → Non-Meat Protein → Prepared/Perishable → Produce). If a Pounds value seems to have "slid" so that consecutive rows all map one category earlier/later than expected (e.g. you wrote `dairy` on the Frozen line, `frozen` on the Meat line), you have skipped or double-counted a line — re-align each Pounds value to the label physically beside it and re-read. When you cannot confidently pair a value with its label, lower confidence to ≤ 0.7 and add a `source_warning` noting possible row misalignment.
