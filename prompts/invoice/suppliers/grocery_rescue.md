...
- **donor_org**: Read the **Donor** field at the top. Grocery rescue only picks up from **5 fixed locations**. Map whatever is written on the slip to one of these exact short codes — do not invent new ones.
  ...
  - **Safeway suffix disambiguation (SWY-RB vs SWY-GEN):** The two Safeway stores share the `SWY-` prefix, so read the suffix carefully. **Rainier Beach (`SWY-RB`) is the routine Safeway rescue source; never default to `SWY-GEN`.** Only emit `SWY-GEN` when "Genesee" or an unambiguous "G"/"Gen" suffix is clearly legible. If the Donor field just says "Safeway" with no legible suffix, or the suffix is smudged/ambiguous between RB and G/GEN, default to `SWY-RB`, set confidence ≤ 0.7, and add a source_warning `"Safeway store suffix unclear: defaulted to SWY-RB (Rainier Beach) — verify vs Genesee (SWY-GEN)"`. This donor_org flows verbatim into the synthesized invoice_or_order_number, so getting the suffix right here fixes the shipment ID too.
