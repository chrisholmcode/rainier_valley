...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **Single-digit month legibility warning:** if the handwritten month digit is ambiguous for ANY reason (could be one of several digits, ink smudged, only partially legible — not just "7" vs "8"), do NOT silently pick one — emit your best reading, set confidence ≤ 0.7, and add a `source_warning`: `"delivery_date month digit unclear: read as <your reading> but may be different — please verify against slip"`.
  - **Month output validation:** Before emitting `delivery_date`/`invoice_date`, validate the resolved value against `^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$`. The month MUST be two zero-padded digits in the range `01`–`12` — never `0`, `00`, or a single unpadded digit. If your best reading of the month cannot be pinned to a valid `01`–`12` value, set `delivery_date=null` and `invoice_date=null`, lower confidence ≤ 0.6, and add a `source_warning`: `"delivery_date month unreadable/invalid — please verify against slip"` rather than emitting a guess or a malformed value.
