...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - Fallback: if this document has NO printed "Quantity" column but DOES have a Pounds column (whether printed or handwritten), read the Pounds value for the line into `quantity` (as a plain number, e.g. "16" => 16) and set `unit = "lbs"`. Never leave `quantity` blank when a Pounds value is present. (If the whole document is the handwritten Donor / Address / Agency / Date slip, prefer re-routing to `grocery_rescue` per "Not this supplier" — but still populate `quantity` from Pounds if it stays on this supplier.)
