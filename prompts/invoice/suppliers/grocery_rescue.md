...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **supplier is ALWAYS `grocery_rescue` — NEVER `food_lifeline`.** The "FOOD LIFELINE" logo/branding at the top of this form is the broker's letterhead, NOT the supplier value. Do not let the printed logo drive the `supplier` field. `food_lifeline` is reserved exclusively for printed Food Lifeline **Agency Order** manifests, which this hand-filled grocery-rescue donation form is NOT. If you see the "FOOD LIFELINE" logo on a hand-filled rescue form with the Donor/Address/Agency/Date fields and the Product/Temp/Pounds table, set `supplier = "grocery_rescue"`.
