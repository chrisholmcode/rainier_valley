...
- invoice_or_order_number: synthesize a shipment ID as `<donor_org>-<delivery_date>` (e.g. `QFC-MI-2026-07-13`, `SWY-GEN-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in. **The synthesized ID inherits the delivery_date's uncertainty.** If the delivery_date month or day digit was ambiguous (i.e. you lowered confidence and added a `delivery_date month digit unclear` source_warning), do NOT emit a clean-looking synthesized ID at high confidence — set the slip confidence ≤ 0.7 and add a source_warning `"invoice_or_order_number date component uncertain: synthesized from an unclear delivery_date digit — verify the month/day in the ID against the slip"`. The store-code portion is reliable; the date portion is what reviewers most often correct here (e.g. an ID like `SWY-GEN-2026-05-28` is usually wrong in the MONTH digit, not the store code).
