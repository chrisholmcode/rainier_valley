...
- Category / grouping header rows: Some manifests group line items under a printed category label (e.g., a row reading only "Bakery", "Canned", or "Canned/Dry Goods" with NO Item No., Unit, Quantity, or Gross Weight). These are section headers, NOT line items. Do NOT emit them as a line and do NOT put them in `item_name_raw` (leave item_name_raw blank / omit the row). Only rows that have an actual product Description with an accompanying Item No. / Quantity become line items. If a category label is captured at all, transcribe it verbatim and completely (e.g., "Canned/Dry Goods", not truncated to "Canned").
