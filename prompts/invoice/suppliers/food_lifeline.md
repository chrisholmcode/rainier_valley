...
- Quantity column => quantity. Unit column => unit (lowercase "Case" => "case"). Gross Weight => approx_weight (TOTAL pounds for the line, not per-case).
  - **Always populate `quantity` — never leave it blank.** If there is no separate printed "Quantity" column and the per-line count appears under a "Pounds"/count column instead, use that number as `quantity` (as an integer). Do NOT reroute to `grocery_rescue` solely because a Pounds column exists once you've already confirmed this is a Food Lifeline AGENCY ORDER (printed logo/header, Agency Order No, Sold To). The `grocery_rescue` reroute applies only to the fully handwritten Donor / Address / Agency / Date form.
