...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — never leave blank for a non-empty cell.** Copy every visible digit/number from the Pounds cell exactly as written, preserving all numbers including crossed-out ones and earlier tally values (e.g., a cell showing "70 79" → `quantity_raw = "70 79"`; a clean single value "24" → `quantity_raw = "24"`). Even a lone clean number must be emitted here.
    - **quantity_raw is a LITERAL DIGIT TRANSCRIPTION ONLY.** Emit digits, spaces, and (for crossed-out runs) the raw numbers — nothing else. **Never** write annotation words or symbols into quantity_raw: no `(circled)`, no `(boxed)`, no `/`, no "or", no notes. Any interpretation of which number is circled/final belongs in `approx_weight` and `notes`, not here. Example: a cell with "51" drawn inside a circle → `quantity_raw = "51"` (NOT `"51 (circled)"`); a cell you read as "50" circled next to "66" → `quantity_raw = "50 66"` and explain the circle in `notes`.
    - **Re-read every digit of multi-digit numbers before emitting.** Grocery-rescue weights are commonly 2–3 digits (often 100+). Do not clip a leading digit: if a value could be "45" or "145", count the digit strokes and prefer the fuller reading, and if still unsure emit your best reading, lower confidence to ≤ 0.6, and add a `source_warning` noting the ambiguity (e.g., `"pounds value leading digit unclear: read as 145 but may be 45 — please verify"`). Do not silently drop a hundreds digit.
