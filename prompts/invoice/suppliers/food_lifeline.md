...
- Description => item_name_raw verbatim. When normalizing for item_name_normalized, strip the leading source-program prefix and the trailing `FB` markers — "TEFAP FB Chicken Drumsticks (1115795) FB" => "Chicken Drumsticks". The number in parentheses is a USDA item code; keep it out of the normalized name.
  - **Category/section header rows:** Some manifests group items under printed category labels (e.g., "Bakery", "Canned/Dry Goods", "Produce"). These header rows have NO Item No., Unit, Quantity, or Gross Weight of their own — they are section separators, not line items. Do NOT emit a line item for a header row and do NOT copy the category label into `item_name_raw`; leave `item_name_raw` blank (null) for that row. Use the header only to inform the `category` of the real item lines beneath it.
  - When a category label legitimately IS the cell content you must capture, copy the ENTIRE label verbatim including any slashes and multiple words — "Canned/Dry Goods", not "Canned"; "Canned", not a partial token. Never truncate at a space or `/`.
