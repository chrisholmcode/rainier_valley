...
- invoice_or_order_number: synthesize a shipment ID as `<id_stem>-<delivery_date>`, where `<id_stem>` is derived from donor_org via this table (the ID stem is NOT always identical to donor_org):
  ...
  For Safeway locations the stem is the bare chain prefix `SWY-` (the location suffix is dropped), so the synthesized value keeps a trailing hyphen before the date — e.g. donor_org `SWY-GEN` on 2026-08-07 → `SWY--2026-08-07`, NOT `SWY-GEN-2026-08-07`. For non-Safeway stores the stem equals donor_org (e.g. `QFC-MI-2026-07-13`, `HG-2026-07-01`). This is the unique key for one grocery rescue pickup — only one shipment per store per day, so the store+date combination is guaranteed unique. If either donor_org or delivery_date is unresolved (null), leave invoice_or_order_number null and let a reviewer fill it in.
