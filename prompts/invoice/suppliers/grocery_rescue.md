...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - **⚠️ `supplier` is ALWAYS the literal string `"grocery_rescue"` for every slip in this bucket — NEVER `"food_lifeline"`.** The "FOOD LIFELINE" logo printed upper-left is the broker's branding on the form template; it is NOT the supplier value. `food_lifeline` is a separate, reserved supplier used only for printed Agency Order manifests. Do not copy the on-page logo text into the `supplier` field.
