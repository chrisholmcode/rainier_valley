...
- donor_org: capture the donating retailer/source code printed on the manifest VERBATIM (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC). This code identifies the retail donor (SWY = Safeway, QFC = QFC) plus a source/store suffix. Only set donor_org = null when no such donor/source code appears anywhere on the slip.
