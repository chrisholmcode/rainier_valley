...
- Handwritten Food Lifeline slip with Donor / Address / Agency / Date fields and a per-category Pounds column → `supplier = "grocery_rescue"` (separate prompt). If for any reason a line is extracted under this config, the handwritten number in the **Pounds** column IS the line quantity: set `quantity_raw` to that number verbatim (e.g., 24, 16, 170). Never leave `quantity_raw` blank when a Pounds value is legibly present.
