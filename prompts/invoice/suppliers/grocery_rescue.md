...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to strict YYYY-MM-DD, where MM is always a two-digit, zero-padded month in the range 01–12 and DD is a two-digit, zero-padded day (e.g., May 28 → `2026-05-28`, NOT `2026-5-28` and NOT `2026-0-28`). Never emit a single-digit or "0" month/day — a month of "0" or "00" is invalid and indicates a parsing error; re-read the slip. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
