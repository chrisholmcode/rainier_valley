...
- delivery_date and invoice_date: Food Lifeline AGENCY ORDER manifests carry a single **Ship Date** field in the upper-left. Populate BOTH `invoice_date` and `delivery_date` with that value (YYYY-MM-DD).
  - **Month read caution:** Read the Ship Date's MONTH digits directly from the printed field — do NOT round up, shift, or infer the month. A recurring error on this supplier is writing the month one higher than printed (e.g., outputting `2026-08-07` when the slip reads `2026-07-07`). Before finalizing, re-verify each of the year, month, and day segments character-by-character against the printed Ship Date.
  - If the printed date's field order is ambiguous (e.g., `07-07` or a dotted/two-line date), treat the smaller/upper positioned value per the printed label and prefer the plain-as-printed reading over any assumed regional ordering.
