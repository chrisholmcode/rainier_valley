...
- **Rows with a non-empty Pounds cell:**
  ...
  - quantity_raw = **REQUIRED — the single RESOLVED final number as plain digits, exactly equal to `approx_weight`.** Do NOT include annotations, slashes, the word "circled", or crossed-out/earlier tally values here — put that full verbatim transcription in `notes` instead. Examples: a cell showing "50 (circled) / 66" that resolves to 51 → `quantity_raw = "51"` (and `notes` records "reads 50 circled / 66, resolved to 51"); a running tally "151 123 108 40" → `quantity_raw = "40"`; a clean single value "24" → `quantity_raw = "24"`. Read multi-digit numbers carefully so you do not drop a leading digit (e.g., "145", not "45"). If, after resolving, this row has no legitimate accepted weight (value belongs to another row, or is not actually a Pounds entry), set `quantity_raw = null` along with `approx_weight`/`quantity`.
