...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. The output MUST be a strictly valid ISO-8601 date: four-digit year, then a **zero-padded two-digit month 01–12**, then a **zero-padded two-digit day 01–31**, e.g. `2026-05-28`. Never emit a single-digit or empty month/day component (e.g. `2026-0-28` or `2026-5-28` are invalid). If your resolved month/day would not be a valid two-digit value, re-read the slip; if still unresolved set delivery_date=null and add a source_warning. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
