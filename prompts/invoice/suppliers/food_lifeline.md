...
- HANDWRITTEN SLIP FALLBACK: If the document is (or is being processed as) a hand-filled Food Lifeline slip with a **Pounds** column instead of a printed Quantity column, map the handwritten **Pounds** value directly to `quantity` (one number per line item). Do NOT leave `quantity` blank — every row with a written pounds figure has a quantity. Read the handwritten digits carefully (e.g., a trailing 9 vs 0, or 79 vs 70); if two digits are written, capture both. If the number is truly illegible, transcribe your best single reading rather than blanking the field.
