...
- FALLBACK for handwritten Pounds-column slips: If the document has NO printed "Quantity" column but DOES have a handwritten "Pounds" column (the grocery_rescue-style Donor / Address / Agency / Date form), the number in the Pounds column IS the quantity — set `quantity` to that Pounds value (e.g., 16, 14, 170, 21) and `unit = "lb"`. Never leave `quantity` blank when a Pounds value is present. (Ideally such slips route to `grocery_rescue`, but if this slip is being processed under food_lifeline, do NOT drop the Pounds number.)
