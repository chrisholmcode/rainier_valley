...
- document_type = "manifest". supplier = "grocery_rescue" (ALWAYS — never "food_lifeline"). is_donation = true.
  - ⚠️ **The "FOOD LIFELINE" logo printed on this form is the BROKER's branding, NOT the supplier value.** For this bucket the supplier field is ALWAYS the literal string "grocery_rescue". Do NOT copy "food_lifeline" (or any text off the logo) into `supplier`. The value `food_lifeline` is reserved exclusively for printed Food Lifeline **Agency Order** manifests, which this handwritten grocery-rescue form is not.
