...
- donor_org: capture the donor/source code printed on the manifest **verbatim** (e.g., "SWY-GEN" for Safeway General, "QFC-MI" for QFC). This is a short alphanumeric source code, usually near the donor/source line or item source columns. Do NOT translate it to a full company name. Only set `donor_org = null` if the document truly has no donor/source code.
