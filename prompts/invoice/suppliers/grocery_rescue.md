...
- delivery_date and invoice_date: The handwritten Date (see swap note above). Convert to YYYY-MM-DD. If only two digits are given for the year, assume 20YY. If no year is written at all (bare M/D like "7/1"), use the year from `Today's date` in the user message — these forms are filled the day of pickup, so year-boundary edge cases (e.g., a bare "12/28" seen in early January) should use the previous year. Populate BOTH `invoice_date` and `delivery_date` with the resolved value.
  ...
  - **NEVER leave delivery_date blank, empty, "\", or any placeholder.** If the Date field is illegible, smudged, or appears blank, do NOT emit an empty/backslash value — fall back to `Today's date` from the user message (these forms are filled the day of pickup, so the pickup date is Today's date), set confidence ≤ 0.6, and add a `source_warning`: `"delivery_date unreadable on slip — defaulted to Today's date <value>; please verify"`. Always resolve to a real YYYY-MM-DD value and populate BOTH `invoice_date` and `delivery_date` with it.
