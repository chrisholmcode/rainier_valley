...
- document_type = "manifest". supplier = "grocery_rescue". is_donation = true.
  - ⚠️ **The "FOOD LIFELINE" logo printed on this form is NOT the supplier.** It is the broker's letterhead on a grocery-rescue pickup form. `supplier` MUST always be `"grocery_rescue"` here — **never** emit `supplier = "food_lifeline"` for this handwritten form. `food_lifeline` is reserved exclusively for printed Food Lifeline **Agency Order** manifests, which this is not.
